...
- kici feedback prints how to report a docs/behaviour discrepancy, and a new guide tells coding agents what a report needs and that a human decides to file it
- New KICI_RUNNER_USER agent option runs the bare-metal runner child under a dedicated uid
- Orchestrators install in hybrid mode by default, and kici-admin source redeliver replays deliveries lost to an outage
- Schedule orchestrator database backups with kici-admin db backup --install-timer
- Orchestrator retention windows for run history, audit logs and provenance are now settable per cluster and prune the database directly
- kici-admin orchestrator upgrade now takes a database dump and drains in-flight jobs before it restarts, and refuses a rollback the database schema is ahead of
- kici-admin db migrate --to reverts the schema to a named migration, which is what makes a rollback across a schema change possible
- A dynamicJob generator can declare named git credentials, and every job it generates inherits them
- Auto-scaler network policy gains hostAccess: a CIDR and port vocabulary for what a sandbox may reach on the orchestrator host
- The CI trust settings page warns while fork pull requests are being dropped, and offers hold for approval in one click
- kici-admin token create --subject records the intended holder of an orchestrator admin token, and the dashboard reports RBAC drift between those tokens and organization membership
- kici-admin trust-policy directory shows how old the approval directory is, and warns when the Platform connection is down so it cannot refresh
- New orchestrator metric kici_orch_trust_directory_age_seconds reports how far behind the cached approval directory is, with a matching dashboard panel
- The infrastructure page, `kici-admin orchestrator status`, and `kici report` now show where each orchestrator's config files live.
- The dashboard sidebar and login page now link to the documentation, the public status page, and kici.dev

### Fixes

- Job-originated git credential and container-registry secret requests are now authorized against the job's lock declaration, the named context's protection rules and the run's trust tier
- Organization-wide workflows register their jobs' declared git credentials, so a credential the workflow names is no longer refused as undeclared
- Re-runs and scheduled fires carry a job's declared git credentials, so naming one no longer fails with Unknown git credential
- A git credential declared with an inline runtime value is admitted again, checked against the job's lock like any other declared credential
- Multi-line secrets (private keys, kubeconfigs, service-account JSON) are now masked in run logs
- A step or hook that hits its timeout now aborts its own subprocesses instead of being left running
- Dependency installs in .kici/ now run with lifecycle scripts disabled by default (KICI_ALLOW_INSTALL_SCRIPTS to opt out)
- Nested job containers now get RFC1918 and cloud-metadata egress filtering by default
- Workflow filters, matrix and dynamic-job generators now evaluate in a dedicated child process without the agent's environment
- A failed step now shows its real error on the run timeline instead of [object Object]
- A git credential a workflow declared is no longer refused when a job asks for it immediately after dispatch
- Generic webhook deliveries are now resolved against the organization in the URL, so a source another organization registered under the same routing key can never receive them
- The Platform now rejects a source registration whose generic routing key names an organization other than the one the orchestrator authenticated as
- Dashboard: the run payload viewer's Plain view now HTML-escapes payload content, so attacker-controlled webhook data can no longer inject markup
- Dashboard: the dashboard origin now serves a Content-Security-Policy that restricts scripts, framing and network destinations
- Dashboard: sign-in no longer requests offline access, so the browser's refresh token dies with the session instead of staying valid for 30 days
- A `g`- or `y`-flagged regex in a trigger, comment bodyMatch or runsOn selector no longer skips every second match
- A `d`- or `v`-flagged regex in `commitMessage` or a `requires` content filter now compiles into the lock file correctly instead of silently never matching
- The orchestrator honours `KICI_HOST` for its listener, and refuses to start when `KICI_AGENT_AUTH=none` is paired with a routable bind address
- A workflow's `contentHash` now covers the whole `.kici/` tree, so editing a file the workflow imports invalidates the source cache instead of silently running the old code
- The source cache is now org-scoped and content-addressed, and a restored `.kici/` tree is verified and replaces the clone instead of being unpacked over it
- Editing an in-repo `workspace:` sibling package now invalidates the dependency cache instead of restoring its stale built output
- An attached `kici run --local` no longer fails to seed its source when the local plane is attached to a Platform organization
- Outbound webhook deliveries are sent exactly once, and buffered webhooks are no longer discarded under load
- A source registration no longer unroutes an orchestrator's other sources, and deleting a source stops routing to it
- Orchestrator service commands can address a macOS instance after install
- Upgrading an org that already has a subscription now opens the billing portal instead of creating a second subscription
- @kici-dev/compiler no longer prompts during install; run `npx kici init` to scaffold a project
- Published orchestrator and agent images no longer ship a build-time .npmrc or the private root package.json
- Orchestrator migrations no longer inherit the hot-path statement timeout, so an upgrade on a database with real history cannot boot-loop
- Orchestrator LISTEN subscriptions now reconnect after a database failover instead of going permanently deaf while reporting healthy
- Firecracker orphan sweep no longer deletes a running VM's TAP device
- kici compile no longer writes generated secret declarations into a nested .kici/.kici/types/ directory when run from inside .kici/, which poisoned the lock file's contentHash and made every agent reject the run as a stale lock file
- Scaler isolation is now enforced. Three behaviour changes an upgrading operator will notice, each because a policy that was previously ineffective now takes effect: - **`networkPolicy.denyAll` denies.** The per-agent drop used to sit behind the host baseline's unconditional internet accept, so `denyAll` granted full egress. Outbound calls that worked under a `denyAll` label set will now fail — which is the configured policy. An `allowlist` entry inside a denied range is still reachable. - **Firecracker VMs cannot reach each other.** Every VM's TAP is enslaved with bridge port isolation, so VM-to-VM traffic — which is switched at layer 2 and never reaches a packet filter — is blocked. The orchestrator refuses to start on a host kernel that rejects the flag. Staging and production Firecracker hosts need re-provisioning to gain it, which the orchestrator does itself on startup. - **`globalMaxAgents` and per-scaler `maxAgents` bind.** Both count caps used to read a ledger written after the check, so a burst of queued jobs could provision well past them. An operator whose caps were previously overshot will see fewer concurrent agents. Also fixed: the orphan sweeps no longer destroy live workloads (a running VM's TAP, another orchestrator's containers or its agents' isolation rules, or an arbitrary host process reached through a recycled PID), per-agent nftables rules are reconciled so a recycled address cannot inherit a dead job's allowlist, an idle-reaped warm agent stops receiving dispatches while it is being destroyed, bare-metal container mode applies the resource limits and network isolation it was configured with, and the serial-console tail is bounded so guest output cannot exhaust the orchestrator's memory. Agent containers on both container spawn paths now carry a 4096-process ceiling. They keep the default capability set: unless a job declares its own image, the workflow's steps run inside the agent container, so dropping capabilities there would break `apt-get`, `chown` and `sudo` in ordinary builds. The capability drop and `no-new-privileges` continue to apply to the nested job container, which is the boundary that confines customer code.
- Only an interactive login can mint an unscoped user token, and a scoped credential caps what it mints
- Connecting Slack or linking GitHub must be completed in the browser that started it
- Suspending a member or disabling an organization now revokes live log streams and agent access, not just dashboard reads
- Master-key rotation no longer strands the signing key, dashboard-encryption key, run ephemeral keys, or secret outputs
- OIDC identity tokens now distinguish a fork pull request from a trusted push, and carry the event context a cloud trust policy needs
- Job-container egress drops now apply from network creation, so a job image's own entrypoint can never reach the host private network or the cloud metadata endpoint
- A Firecracker host re-provision no longer removes the job-container egress drops installed by a container-mode agent on the same host
- A re-run of a pull request presents the pull-request OIDC subject instead of the colliding branch-shaped one
- Suspending an organization member now revokes their CI approval trust and team approval authority in the trust policy pushed to your orchestrator, while keeping their identity link so open pull requests still resolve to them.
- Removing a member from an organization now revokes their CI approval authority in your orchestrator immediately: their identity link and CI trust level leave the pushed trust policy as soon as the removal lands, instead of persisting until an unrelated change triggered the next push.
- Removing a member from an organization, or a member leaving one, now also clears their team memberships and immediately re-pushes the trust policy, so they lose CI approval authority on your orchestrator at once.
- Deleting a user account, or linking and unlinking a provider account, now immediately re-pushes the trust policy, so the change reaches your orchestrator at once instead of waiting for an unrelated event.
- Granting, revoking, editing or deleting a role now immediately re-pushes the trust policy, so a CI trust level you change through a role reaches your orchestrator at once instead of waiting for an unrelated event.
- Unlinking a provider account in the dashboard, and linking one through GitHub OAuth, now immediately re-push the trust policy to every organization you belong to.
- The orchestrator refuses to start when KICI_AGENT_AUTH=none is paired with a wildcard bind (0.0.0.0 or ::), which is the default KICI_HOST — agent authentication disabled on a listener every interface accepts
- The orchestrator's loopback guards now read an IPv4-mapped IPv6 address (`::ffff:127.0.0.1`, and the `::ffff:7f00:1` form a URL parser produces) by the IPv4 address it carries, so a mapped loopback bind starts and a mapped loopback storage endpoint is caught
- Scaled agents now reach only the orchestrator port, DNS and the object-storage endpoint on the host, instead of every port on every host address
- Job containers now reach no service on the agent host except DNS, closing a path from workflow code to host-local services
- Worker-role orchestrators now refuse to start with agent authentication disabled on a bind other hosts can reach, matching coordinators
- A run now records its lock-file source at the same moment it records its trust tier, so a fork pull request is never observed with a resolved tier and no lock source
- kici-admin agent register --labels now authorizes the kici:role:* namespace, so a label-scoped agent token admits the agent that presents it
- The Firecracker orphan reaper no longer deletes the chroot of a running microVM whose process it cannot positively identify, including on rootless nodes where the orchestrator cannot signal the jailer
- A missing nftables forward chain now reads as "no rules installed" instead of aborting the caller, so job-container egress drops are installed rather than skipped
- Workflow runs no longer fail the drift gate with "lock file is out of date" when nothing changed. Installing a workflow's dependencies rewrites `.kici/package-lock.json` and `.kici/.npmrc`, and those files were part of the content hash, so a run could invalidate its own hash. The set of files the hash covers is now declared in `.kici/.kiciignore`, which `kici init` seeds with sensible defaults. Existing lock files are stale after upgrading — run `kici compile` and commit the result
- Scaler: a coordinator restart no longer strands agent-capacity reservations, which could silently refuse every autoscale spawn
- The agent now names a compiler/agent compile-schema mismatch instead of reporting it as an out-of-date lock file
- An ignored fork pull request now records why it was dropped in the event log, naming the fork policy and whether the org chose it
- kici-admin trust-policy show warns that an ignore fork policy drops pull requests with no trace the contributor can see
- kici-admin held-run approve/reject accept --as <user-id>, so an independent orchestrator can answer a hold whose approval clauses name a user or a team
- The orchestrator refuses a dashboard-write policy change that would disable held_runs.approve or held_runs.reject on a Platform-attached orchestrator, where a context or reviewer hold would be left with no surface that can answer it
- The held-run dashboard-write operations name kici-admin held-run approve/reject as their operator equivalent, replacing a command name that does not exist
- Runner labels and hostnames now match case-insensitively everywhere, and both are stored and displayed lowercase. Breaking for secret scope templates: `${host}` and `${label:NAME}` render the lowercase value, so rename any secret subtree keyed on a mixed-case hostname or label value to lowercase before upgrading. Agent IDs are unaffected and still compare exactly.
- The infrastructure page no longer shows a worker peer the deployment shape of whichever coordinator reported it, so the "Run kici-admin here" command stops flipping between systemd, launchd and cmd.exe on the same row.
- The orchestrator now reads the last occurrence of a duplicated key in its service env file, matching how systemd applies EnvironmentFile assignments.
- kici-admin orchestrator status now reports the deployment shape recorded at install time rather than the init system of the host it runs on, so a compose install is described correctly on a systemd machine.
- Service lifecycle, upgrade, and scheduled-backup commands now read the service platform from the resolved install's manifest, so a Compose install is managed with Compose on a systemd host instead of reported as not installed
- kici-admin db backup --install-timer now refuses a Compose install and exits non-zero, instead of installing a systemd timer for a container it cannot reach
- kici-admin now scans every service driver the host can run when it lists instances or resolves one by name, so a Compose instance is no longer hidden from the candidate list or dropped from the instance index by a command run on a systemd host
- The infrastructure diagnostics panel now shows the instance id of a disconnected or remote-owned orchestrator instead of leaving it blank
- kici-admin no longer drops Compose instances from its instance index when the container daemon is down or hung
- kici-admin no longer drops Windows services or launchd jobs from its instance index when their registry cannot be read
- kici-admin now says on stderr when it cannot read a platform's service registry, instead of silently listing fewer instances
- Hot reload now works for an orchestrator running in independent mode. `KICI_MODE`, `KICI_PORT` and `KICI_BASE_PATH` reach the reload merge, so a reload no longer fails validation asking for a Platform token, and no longer reverts the port
- `KICI_CONFIG` no longer stops the orchestrator from starting, and a startup rejection of a reload-overlay name such as `KICI_SERVER_PORT` now names the startup variable to set instead
- kici-admin join now writes ./kici-orchestrator.env, the env file the orchestrator boots from and `orchestrator install --env-file` consumes, readable by its owner only on POSIX hosts (on Windows the permission call sets the read-only attribute and leaves the ACL unchanged); --config still writes the local YAML the orchestrator never read, and is deprecated
- kici-admin orchestrator install and agent install now write their service env file — it carries the secrets encryption key, the database URL and the agent token — readable by its owner only on POSIX hosts; on Windows the same permission call sets the read-only attribute and leaves the file's ACL unchanged
- kici-admin config set now warns when a setting is one no running orchestrator reads back, and names the command that does change a running cluster
- kici-admin orchestrator install --mode now reaches an --env-file install, and the join and its docs say the mode has to be chosen before the install rather than after it
- Firecracker VMs can now run a workflow that has a filter or any other dynamic field; the microVM image was missing the component that evaluates them
- A webhook delivery the orchestrator sheds under load now leaves a durable `shed` row in the event log, naming the shed reason, so it is distinguishable from a delivery that never arrived
- A context concurrency queue no longer deadlocks: a job waiting for a slot no longer counts as occupying one, so held jobs are released as soon as the running jobs finish instead of waiting out the hold expiry
- A context concurrency limit no longer counts one job twice. A dynamic matrix child, or any job gated after its sibling reached an agent, was measured against both the sibling's job record and the same sibling's in-pass reservation, so it was queued while a slot was free
- A context concurrency limit now counts a job from the moment it is dispatched, rather than from the moment its agent reports it running. A burst of events arriving during agent start-up used to read the context as empty and admit every one of them
- A `.kici/node_modules` that is a symlink into a shared dependency tree is now excluded from the workflow source digest, the same as a real directory. The link used to be hashed as its target string — a member the build agent could never hold, since its dependency install writes a real directory there — so every run failed the drift gate with "lock file is out of date" against source nobody had changed, and recompiling reproduced the same lock. `kici compile` now also warns about a hashed symlink the source tarball cannot carry unchanged, and the agent's drift error names the symlinks in the tree it hashed.
- The quickstart object-store image now carries the patched curl (SeaweedFS 4.46)
- A webhook shed under sustained load is no longer given up on after ten seconds. The orchestrator's durable ingest queue now takes an admission slot before it claims a delivery to replay, so an overload of any length costs that delivery no retries — the retry ceiling counts genuine failures only. Retention is bounded by age instead (`KICI_INGEST_OVERFLOW_MAX_AGE_MS`, default 15 minutes)
- A webhook shed on the orchestrator's direct-ingress route now logs the delivery id, routing key and shed reason, matching what the Platform-relay route already logged. Before this an operator saw the shed counter rise with nothing naming the delivery

### Documentation

- the Getting help page linked SECURITY.md at a repository that does not exist; it now points at kici-public on its main branch
- The generic webhook setup docs now show the Platform-route URL that actually delivers: the source id (not its name) in the last segment, the singular /webhook/ path, and your Platform organization id in the org segment
- SECURITY.md states the supported version line without a literal minor; LICENSES.md names the AGPL packages the kici CLI pulls
- Document networkPolicy.hostAccess and the narrowed host reachability defaults for agents and job containers
- Document the org-scoped source cache key layout
- Correct the CI security architecture doc: the Platform pushes the trust policy on every handshake, including to an org with no stored row
- The platform-down capability matrix and the direct-ingress page now scope the quota exemption to relayed webhooks, and name what still meters while the Platform is connected
- Operator docs state that a CI-trust or membership change reaches the orchestrator's approval directory on the Platform's next push, and how to check the lag
- The Firecracker auto-scaler page now says how much memory a VM that runs jobs needs

### Other

- The agent container image no longer ships two copies of the application tree, making it substantially smaller to pull and store
- The SDK's runtime-internal API moved to the `@kici-dev/sdk/internal` subpath; the root barrel keeps every symbol as a `@deprecated` re-export
