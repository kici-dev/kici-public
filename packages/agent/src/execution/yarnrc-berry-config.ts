...
export async function applyYarnrcBerryConfig(
  ...
  // Lifecycle scripts are off for every install, not only a private-registry
  // one: a `postinstall` in a committed package.json is customer code running
  // in whichever process performs the install.
  if (args.ignoreScripts !== false) {
