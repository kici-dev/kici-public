...
export function buildContainerHardening(opts: SandboxHardeningOptions): ContainerHardening {
  ...
  } else if (opts.jobNetworkName) {
    // Join the agent's own bridge so the container gets a predictable IP the
    // egress rules key on. Without it the container keeps the runtime's default
    // bridge, which reaches the host private network and 169.254.169.254.
    hostConfig.NetworkMode = opts.jobNetworkName;
  }
  // 'default' with no job network leaves NetworkMode unset (the runtime's own
  // default bridge) — the posture when egress filtering is off.
