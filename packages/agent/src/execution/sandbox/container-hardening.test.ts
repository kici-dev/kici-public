...
describe('job-network attachment', () => {
  const base = {
    hardened: true,
    readonlyRootfs: false,
    pidsLimit: 512,
  ...
  it('joins the kici-jobs bridge when egress filtering is on', () => {
    const { hostConfig } = buildContainerHardening({
      ...base,
      networkMode: 'default',
      jobNetworkName: 'kici-jobs',
    });
    expect(hostConfig.NetworkMode).toBe('kici-jobs');
  });

  it('leaves the runtime default bridge when egress filtering is off', () => {
    const { hostConfig } = buildContainerHardening({ ...base, networkMode: 'default' });
    expect(hostConfig.NetworkMode).toBeUndefined();
  });

  it('never overrides an explicit host network', () => {
    const { hostConfig } = buildContainerHardening({
      ...base,
      networkMode: 'host',
      jobNetworkName: 'kici-jobs',
  ...
  it('never overrides a grant that asked for no network', () => {
    const { hostConfig } = buildContainerHardening({
      ...base,
      networkMode: 'default',
      jobNetworkName: 'kici-jobs',
      grant: { network: 'none' } as never,
    });
    expect(hostConfig.NetworkMode).toBe('none');
  });
});
