...
// --- Log containment: crash tail masking + raw stdio echo gating ---

describe('fork-runner log containment', () => {
  let mockChild: ReturnType<typeof createMockChild>;
  let mockFork: ReturnType<typeof vi.fn>;
  ...
  beforeEach(async () => {
    mockChild = createMockChild();
    mockFork = vi.fn().mockReturnValue(mockChild);
    vi.doMock('node:child_process', () => ({ fork: mockFork, spawn: mockFork }));
    vi.doMock('node:fs', () => ({ existsSync: vi.fn().mockReturnValue(false) }));
    vi.doMock('./env-sanitizer.js', () => ({
      buildSanitizedEnv: vi.fn().mockReturnValue({}),
    }));
  ...
  function execOptionsWithSecret(secret: string): JobExecutionOptions {
    const opts = createMockExecOptions();
    (opts.dispatch as unknown as { secrets: Record<string, string> }).secrets = {
      DEPLOY_TOKEN: secret,
    };
    return opts;
  }

  it('does not echo the child stdio to the agent stderr by default', async () => {
    const mod = await importForkRunnerLocal();
    mod.createForkRunner({ runnerPath: '/test/runner.js', env: {} }, createMockExecOptions());

    (mockChild.stdout as unknown as EventEmitter).emit('data', Buffer.from('step said hello\n'));
    (mockChild.stderr as unknown as EventEmitter).emit('data', Buffer.from('step warned\n'));

    expect(stderrWrites.join('')).not.toContain('[workflow-runner:stdout]');
    expect(stderrWrites.join('')).not.toContain('[workflow-runner:stderr]');
  });

  it('propagates the debug flag into the child env, which the sanitizer strips', async () => {
    process.env.KICI_RUNNER_DEBUG_STDIO = 'true';
    const mod = await importForkRunnerLocal();
    mod.createForkRunner({ runnerPath: '/test/runner.js', env: {} }, createMockExecOptions());

    // buildSanitizedEnv excludes every KICI_* host var, so without the explicit
    // put-back the child's own fork-mode passthrough would stay off and the
    // flag would enable only the agent-side half of the echo.
    const forkOpts = mockFork.mock.calls[0][2] as { env: Record<string, string> };
    expect(forkOpts.env.KICI_RUNNER_DEBUG_STDIO).toBe('true');
  });

  it('leaves the debug flag out of the child env when it is off', async () => {
    delete process.env.KICI_RUNNER_DEBUG_STDIO;
    const mod = await importForkRunnerLocal();
    mod.createForkRunner({ runnerPath: '/test/runner.js', env: {} }, createMockExecOptions());

    const forkOpts = mockFork.mock.calls[0][2] as { env: Record<string, string> };
    expect(forkOpts.env.KICI_RUNNER_DEBUG_STDIO).toBeUndefined();
  });

  it('echoes the child stdio when KICI_RUNNER_DEBUG_STDIO=true', async () => {
    process.env.KICI_RUNNER_DEBUG_STDIO = 'true';
    const mod = await importForkRunnerLocal();
    mod.createForkRunner({ runnerPath: '/test/runner.js', env: {} }, createMockExecOptions());
    ...
    expect(all).toContain('[workflow-runner:stdout] step said hello');
    expect(all).toContain('[workflow-runner:stderr] step warned');
  });

  it('masks a registered secret in the crash tail', async () => {
    const mod = await importForkRunnerLocal();
    const execOpts = execOptionsWithSecret('s3cr3t-deploy-token');
    const handle = mod.createForkRunner({ runnerPath: '/test/runner.js', env: {} }, execOpts);

    (mockChild.stderr as unknown as EventEmitter).emit(
      'data',
      Buffer.from('Error: auth failed with s3cr3t-deploy-token\n'),
    );
    mockChild.simulateExit(1, null);

    const result = await handle.result;
    expect(result.error).toContain('***');
    expect(result.error).not.toContain('s3cr3t-deploy-token');

    const logged = (execOpts.onLogLine as ReturnType<typeof vi.fn>).mock.calls
      .map((c) => String(c[1]))
      .join('\n');
    expect(logged).not.toContain('s3cr3t-deploy-token');
    expect(logged).toContain('***');
  });
});

async function importForkRunnerLocal() {
  return await import('./fork-runner.js');
}

describe('resolveRunnerUser', () => {
  // The suites above vi.doMock('node:child_process'); this one needs the real
  // execFileSync to resolve a uid through `id`.
  beforeEach(() => {
  ...
  it('returns undefined when unset (the same-uid default)', async () => {
    const { resolveRunnerUser } = await import('./fork-runner.js');
    expect(resolveRunnerUser(undefined)).toBeUndefined();
    expect(resolveRunnerUser('')).toBeUndefined();
  });

  it('accepts uid:gid verbatim', async () => {
    const { resolveRunnerUser } = await import('./fork-runner.js');
    expect(resolveRunnerUser('1234:5678')).toEqual({ uid: 1234, gid: 5678 });
  });

  it('resolves a bare uid to its primary group', async () => {
    const { resolveRunnerUser } = await import('./fork-runner.js');
    const { userInfo } = await import('node:os');
    const me = userInfo();
    // `id -g <uid>` resolves the current user's primary group.
    expect(resolveRunnerUser(String(me.uid))).toEqual({ uid: me.uid, gid: me.gid });
  });

  it('throws rather than silently falling back to the agent uid', async () => {
    const { resolveRunnerUser } = await import('./fork-runner.js');
    expect(() => resolveRunnerUser('definitely-not-a-user-on-this-host')).toThrow(
      /KICI_RUNNER_USER/,
    );
  });
});
