...
describe('step timeout aborts what it times out', () => {
  /** Wire the loop the way workflow-runner does: one controller per step, and
   *  `ctx.signal` composed from it. */
  function timeoutHarness(body: (ctx: StepContext) => Promise<void>) {
    const controllers = new Map<number, AbortController>();
    const { messages, sendIpc } = collectMessages();
    return {
      messages,
      run: (steps: Step[]) =>
        executeStepLoop({
          steps,
          abortStep: (i) => controllers.get(i)?.abort(),
          getStepAbortSignal: (i) => controllers.get(i)?.signal,
          createStepContext: (stepIndex) => {
            const controller = new AbortController();
            controllers.set(stepIndex, controller);
            return stubStepContext({ signal: controller.signal });
          },
          sendIpc,
          defaultTimeoutMs: 30_000,
  ...
  it('fires ctx.signal at the timeout and still reports the timeout message', async () => {
    let observedAbort = false;
    const harness = timeoutHarness(async () => {});
    const step: Step = {
      name: 'hangs',
      timeout: 30,
    ...
    expect(complete?.error?.message).toContain("Step 'hangs' timed out after 30ms");
  });

  it('names a step that ignores its abort signal', async () => {
    const harness = timeoutHarness(async () => {});
    const step: Step = {
      name: 'stubborn',
      timeout: 20,
      // Never settles and never watches ctx.signal.
      run: async () => new Promise<void>(() => {}),
    } as unknown as Step;

    await harness.run([step]);

    const lines = harness.messages
      .filter(
        (m): m is Extract<RunnerToAgentMessage, { type: 'log.line' }> => m.type === 'log.line',
      )
      .map((m) => m.line);
    expect(lines.some((l) => l.includes("Step 'stubborn' did not stop within"))).toBe(true);
  }, 15_000);

  it('does not emit the ignored-abort line when the step unwinds', async () => {
    const harness = timeoutHarness(async () => {});
    const step: Step = {
      name: 'polite',
      timeout: 20,
    ...
    const lines = harness.messages
      ...
      .map((m) => m.line);
    expect(lines.some((l) => l.includes('did not stop within'))).toBe(false);
  });
});
