    ...
    if (parsed.success) {
      ...
      switch (msg.type) {
        ...
        case 'rate.limit.warning': {
          // The orchestrator dropped the frame this answers. Logged as what
          // it is, so a throttled agent can see its own output being
          // discarded instead of an unparseable frame.
          logger.warn('Orchestrator rate-limited this agent and dropped a frame', {
