...
import { validateGitCredentials } from './git-types.js';
...
export function dynamicJob(
  ...
  const tagged = fn as TaggedDynamicJobFn & {
    ...
    [DYNAMIC_JOB_GIT_CREDENTIALS_TAG]?: GitCredentialMap;
  };
  Object.defineProperty(tagged, DYNAMIC_JOB_GROUP_TAG, {
    value: groupName,
    enumerable: false,
  });
  if (isConfig && fnOrConfig.needs) {
...
export interface ResultAwareDynamicJobConfig {
   ...
   * Omit it for an event-only generator that declares `gitCredentials` and
   * nothing else — the generator then runs at webhook time, as the bare
   * function form does.
   */
  ...
  /**
   * Named git credentials every job this generator produces inherits.
   *
   * A generated job has no lock entry of its own, so it cannot declare its own
   * map: the definitions come back from the agent that evaluated the generator,
   * and a declaration read from there would let job code authorize itself. The
   * generator does live in the lock file, so its map is the ceiling the
   * credential relay holds every generated child to.
   */
...
// --- Generator-declared git credentials ---

const DYNAMIC_JOB_GIT_CREDENTIALS_TAG = Symbol.for('kici:dynamicJobGitCredentials');

/**
 * Read the named git credentials a generator declared.
 * Returns undefined for the bare function form, which cannot declare any.
 */
export function getDynamicJobGitCredentials(fn: DynamicJobFn): GitCredentialMap | undefined {
  return (fn as { [DYNAMIC_JOB_GIT_CREDENTIALS_TAG]?: GitCredentialMap })[
    DYNAMIC_JOB_GIT_CREDENTIALS_TAG
  ];
}

export { DYNAMIC_JOB_GIT_CREDENTIALS_TAG };
