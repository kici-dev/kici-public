...
describe('regex flag hygiene at the trigger producers', () => {
  it('toBranchPattern drops g and y', () => {
    expect(toBranchPattern(/^release-\d+/g)).toEqual({
      type: 'regex',
      pattern: '^release-\\d+',
      flags: undefined,
    });
    expect(toBranchPattern(/^release-\d+/gy)).toMatchObject({ flags: undefined });
  });

  it('toBranchPattern preserves every other flag', () => {
    expect(toBranchPattern(/^release-\d+/gi)).toMatchObject({ flags: 'i' });
    expect(toBranchPattern(/^release-\d+/dims)).toMatchObject({ flags: 'dims' });
  });

  it('comment bodyMatch drops g and y', () => {
    const cfg = comment({ bodyMatch: /^\/deploy/g });
    expect(cfg.bodyMatch).toMatchObject({ type: 'regex', flags: undefined });
  });

  it('comment bodyMatch preserves a non-stateful flag', () => {
    const cfg = comment({ bodyMatch: /^\/deploy/gi });
    expect(cfg.bodyMatch).toMatchObject({ type: 'regex', flags: 'i' });
  });
});
