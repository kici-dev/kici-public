...
describe('readConfigPaths', () => {
  it('reads the stamped env file path', () => {
    expect(readConfigPaths({ KICI_DEPLOY_CONFIG_FILE: '/etc/kici/orch.env' }, {})).toEqual({
      envFile: '/etc/kici/orch.env',
    });
  });

  it('trims surrounding whitespace off a stamped path', () => {
    // fails-when: the `.trim()` is dropped — the parsed value keeps its spaces
    // and no longer equals this literal. Asserted against a literal, never
    // against an expression derived from the input.
    expect(
      readConfigPaths({ KICI_DEPLOY_CONFIG_FILE: '  /etc/kici/orch.env \n' }, {}).envFile,
    ).toBe('/etc/kici/orch.env');
  });

  it('reports the compose file for a compose deployment', () => {
    // fails-when: resolution is moved behind readDeploymentIdentity's early
    // `compose` return — the compose file stops being reported.
    expect(
      readConfigPaths(
        {
          KICI_DEPLOY_MODE: 'compose',
      ...
      envFile: '/etc/kici/orch1.env',
      composeFile: '/etc/kici/orch1-compose.yaml',
    });
  });

  it('reports the scaler config from app config, with no stamp needed', () => {
    expect(readConfigPaths({}, { scalerConfigPath: '/etc/kici/scalers.yaml' })).toEqual({
      scalerConfig: '/etc/kici/scalers.yaml',
    });
  });

  it('falls back to the scaler config directory when only a directory is set', () => {
    expect(readConfigPaths({}, { scalerConfigDir: '/etc/kici/scalers.d' })).toEqual({
      scalerConfig: '/etc/kici/scalers.d',
    });
  });

  it('prefers the explicit scaler file over the directory', () => {
    expect(
      readConfigPaths(
        {},
        { scalerConfigPath: '/etc/kici/scalers.yaml', scalerConfigDir: '/etc/kici/scalers.d' },
      ).scalerConfig,
    ).toBe('/etc/kici/scalers.yaml');
  });

  it('omits every field a hand-run orchestrator cannot know', () => {
    expect(readConfigPaths({}, {})).toEqual({});
  });
});
