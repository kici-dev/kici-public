...
describe('InMemoryJobQueue', () => {
  ...
  it('listPending returns the oldest pending jobs first, capped at the limit', async () => {
    const queue = new InMemoryJobQueue();
    await queue.enqueue(makeInput({ jobName: 'first' }));
    await queue.enqueue(makeInput({ jobName: 'second' }));
    await queue.enqueue(makeInput({ jobName: 'third' }));

    // fails-when: the method is missing (the worker re-drive's
    // `this.queue.listPending is not a function`), or the cap is ignored.
    const capped = await queue.listPending(2);
    expect(capped.map((j) => j.jobName)).toEqual(['first', 'second']);
    // breaks-if-wrong: a limit past the queue depth must still list everything.
    expect((await queue.listPending(10)).map((j) => j.jobName)).toEqual([
      'first',
      'second',
      'third',
    ]);
