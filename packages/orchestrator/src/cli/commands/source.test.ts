...
describe('kici-admin source redeliver', () => {
      ...
      results: [
        {
          deliveryId: 2,
          guid: 'g-2',
          deliveredAt: '2026-09-01T18:00:00Z',
          event: 'pull_request',
          action: 'opened',
        ...
        },
        {
          deliveryId: 1,
          guid: 'g-1',
          deliveredAt: '2026-09-01T06:00:00Z',
          event: 'push',
          action: null,
  ...
  it('posts the normalized window to the redeliver route and prints a per-delivery table', async () => {
    ...
    expect(stdout).toContain('2 delivery(ies)');
    // One row per delivery, event.action joined, plus the original status.
    expect(stdout).toContain('pull_request.opened');
    expect(stdout).toContain('502');
    expect(stdout).toContain('push');
    expect(stdout).toContain('2 redelivered, 0 failed.');
  });
  ...
  it('reports an empty window instead of an empty table', async () => {
    const post = vi.fn().mockResolvedValue(response({ matched: 0, redelivered: 0, results: [] }));
    const { stdout } = await runCommand(['source', 'redeliver', 'github:42', ...WINDOW], {
      post,
    } as unknown as Partial<AdminApiClient>);

    expect(stdout).toContain('no deliveries in');
  });

  it('exits non-zero when GitHub refused some redeliveries', async () => {
    const post = vi.fn().mockResolvedValue(
      response({
        redelivered: 1,
        failed: 1,
        results: [{ ...response().results[0], outcome: 'failed', error: 'GitHub said no' }],
      }),
    );
    const { stdout, exitCode } = await runCommand(['source', 'redeliver', 'github:42', ...WINDOW], {
      post,
    ...
    expect(stdout).toContain('GitHub said no');
    expect(exitCode).toBe(1);
  });

  it('refuses a non-ISO --since without calling the API', async () => {
    const post = vi.fn();
    const { stderr, exitCode } = await runCommand(
      ['source', 'redeliver', 'github:42', '--since', 'yesterday', '--until', WINDOW[3]],
      { post } as unknown as Partial<AdminApiClient>,
    );

    expect(exitCode).toBe(1);
    expect(stderr).toContain('--since must be an ISO-8601 timestamp');
    expect(post).not.toHaveBeenCalled();
  });

  it('emits raw JSON with --json', async () => {
    const post = vi.fn().mockResolvedValue(response());
    const { stdout } = await runCommand(['source', 'redeliver', 'github:42', ...WINDOW, '--json'], {
      post,
    } as unknown as Partial<AdminApiClient>);
