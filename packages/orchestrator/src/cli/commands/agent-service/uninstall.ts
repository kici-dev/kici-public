...
export function registerAgentUninstall(agent: Command): void {
  agent
    .command('uninstall')
    .description('Remove the agent service registration')
    ...
    .action(async (opts: UninstallOptions) => {
      try {
        ...
        removeIndexEntry(kiciRoot, { component: 'agent', name: config.name });
