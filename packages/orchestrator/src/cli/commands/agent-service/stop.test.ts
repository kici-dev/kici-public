/**
 * Tests for the agent stop command's folder-anchored behavior:
...
describe('agent stop — folder-anchored', () => {
  let program: Command;
  ...
  beforeEach(() => {
    tmpInstanceDir = mkTmp('kici-f-stop-i-');
    tmpConfigRoot = mkTmp('kici-f-stop-c-');
  ...
  it('refuses without --instance-dir/--name and no CWD manifest', async () => {
    ...
    const emptyCwd = mkTmp('kici-f-stop-cwd-');
    ...
    try {
      ...
      await expect(program.parseAsync(['node', 'agent', 'stop'])).rejects.toThrow(
        'process.exit called',
      );
  ...
  it('exits 0 as a no-op when --name matches no installed instance', async () => {
    ...
    try {
      await program.parseAsync(['node', 'agent', 'stop', '--name', 'kici-absent']);
      expect(mockStop).not.toHaveBeenCalled();
  ...
  it('resolves via --instance-dir and calls manager.stop', async () => {
    ...
    await program.parseAsync(['node', 'agent', 'stop', '--instance-dir', tmpInstanceDir]);
