/**
 * Tests for the agent restart command's folder-anchored behavior:
...
vi.mock('../../service/index.js', async () => {
  ...
  const makeManager = (platform: ServicePlatform): ServiceManager =>
    ({
      ...
      start: vi.fn(),
      stop: vi.fn(),
      restart: op(platform, (...args: unknown[]) => mockRestart(...args)),
      status: vi.fn(),
      logs: vi.fn(),
...
describe('agent restart — folder-anchored', () => {
  let program: Command;
  ...
  beforeEach(() => {
    tmpInstanceDir = mkTmp('kici-f-rs-i-');
    tmpConfigRoot = mkTmp('kici-f-rs-c-');
  ...
  it('refuses without --instance-dir/--name and no CWD manifest', async () => {
    ...
    const emptyCwd = mkTmp('kici-f-rs-cwd-');
    ...
    try {
      ...
      await expect(program.parseAsync(['node', 'agent', 'restart'])).rejects.toThrow(
        'process.exit called',
      );
  ...
  it('resolves via --instance-dir and calls manager.restart', async () => {
    ...
    await program.parseAsync(['node', 'agent', 'restart', '--instance-dir', tmpInstanceDir]);
