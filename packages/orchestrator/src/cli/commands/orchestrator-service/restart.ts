...
export function registerOrchestratorRestart(orchestrator: Command): void {
  orchestrator
    .command('restart')
    .description('Restart the orchestrator service')
    .option('--platform <type>', 'Service platform (systemd, launchd, windows, compose)')
    .option('--instance-dir <path>', 'Deploy folder of the instance to restart')
    ...
    .action(async (opts: RestartOptions) => {
      try {
