/**
 * Tests for the orchestrator logs command's folder-anchored behavior:
...
describe('orchestrator logs — folder-anchored', () => {
  let program: Command;
  ...
  beforeEach(() => {
    tmpInstanceDir = mkTmp('kici-e4-lg-i-');
    tmpConfigRoot = mkTmp('kici-e4-lg-c-');
  ...
  it('refuses without --instance-dir/--name and no CWD manifest', async () => {
    ...
    const emptyCwd = mkTmp('kici-e4-lg-cwd-');
    ...
    try {
      ...
      await expect(program.parseAsync(['node', 'orchestrator', 'logs'])).rejects.toThrow(
        'process.exit called',
      );
