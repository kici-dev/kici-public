/**
 * Tests for the orchestrator start command's folder-anchored behavior:
 ...
 * from the manifest.
 *
 * Same partial-mock strategy as uninstall.test.ts — stub only the platform-
 ...
 * (resolveInstance, writeIndex, writeManifest).
 */
...
describe('orchestrator start — folder-anchored', () => {
  let program: Command;
  ...
  beforeEach(() => {
    tmpInstanceDir = mkTmp('kici-e4-start-i-');
    tmpConfigRoot = mkTmp('kici-e4-start-c-');
    ...
    registerOrchestratorStart(program);
  ...
  it('refuses without --instance-dir/--name and no CWD manifest', async () => {
    ...
    const emptyCwd = mkTmp('kici-e4-start-cwd-');
    ...
    try {
      ...
      await expect(program.parseAsync(['node', 'orchestrator', 'start'])).rejects.toThrow(
        'process.exit called',
      );
  ...
  it('resolves via --instance-dir and calls manager.start', async () => {
    ...
    await program.parseAsync(['node', 'orchestrator', 'start', '--instance-dir', tmpInstanceDir]);
