...
/** Shape of the `source redeliver` route response. */
interface RedeliverResponseJson {
  routingKey: string;
  since: string;
  until: string;
  dryRun: boolean;
  matched: number;
  /** The page walk hit its ceiling, so the window holds more than `matched`. */
  truncated?: boolean;
  redelivered: number;
  failed: number;
  results: RedeliverEntryJson[];
}

/** Print a `source redeliver` response as a per-delivery table plus a tally. */
function printRedeliverResult(res: RedeliverResponseJson): void {
  const window = `${res.since} → ${res.until}`;
  if (res.matched === 0) {
    console.log(`${res.routingKey}: no deliveries in ${window}.`);
    return;
  }
  // A truncated walk must never read as a complete one: the tally below counts
  // only what was gathered, and an operator recovering an outage would
  // otherwise believe the window was fully replayed.
  if (res.truncated) {
    console.log(
      `${res.routingKey}: WARNING — more deliveries exist in ${window} than could be listed. ` +
        'Narrow the window and re-run to cover the rest.',
    );
  }
  console.log(
    `${res.routingKey}: ${res.matched} delivery(ies) in ${window}${res.dryRun ? ' (dry run)' : ''}`,
  );
  console.log('');
  console.log('  DELIVERED AT             EVENT                 WAS   OUTCOME');
  for (const r of res.results) {
    const event = r.action ? `${r.event}.${r.action}` : r.event;
    console.log(
      `  ${r.deliveredAt.padEnd(24)} ${event.padEnd(21)} ${String(r.originalStatusCode).padEnd(5)} ${r.outcome}` +
        (r.error ? ` (${r.error})` : ''),
    );
  }
  console.log('');
  const partial = res.truncated ? ' (partial window — see the warning above)' : '';
  if (res.dryRun) {
    console.log(
      `  ${res.matched} would be redelivered${partial}. Re-run without --dry-run to send them.`,
    );
    return;
  }
...
export function registerSourceCommands(program: Command, getClient: () => AdminApiClient): void {
  ...
  // orchestrator holds the App private key, so it is the only tier that can
  // drive GitHub's App-level delivery endpoints. Use it to recover a window
  // whose deliveries never landed: GitHub does not retry on its own.
  src
    .command('redeliver <routingKey>')
    .description("Ask GitHub to resend a GitHub App source's webhook deliveries from a window")
    .requiredOption('--since <iso>', 'Window start, ISO-8601 (inclusive)')
      ...
      try {
        for (const [flag, value] of [
          ...
          if (Number.isNaN(Date.parse(value))) {
            console.error(`Error: ${flag} must be an ISO-8601 timestamp (got "${value}")`);
            process.exit(1);
          }
        }
        const res = await getClient().post<RedeliverResponseJson>(
          `/api/v1/admin/sources/${encodeURIComponent(routingKey)}/redeliver`,
          {
            since: new Date(opts.since).toISOString(),
            until: new Date(opts.until).toISOString(),
