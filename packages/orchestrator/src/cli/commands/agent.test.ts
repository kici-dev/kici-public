...
describe('kici-admin agent CLI', () => {
  ...
  it('register --privileged-root adds the root label to both labels and mandatoryLabels', async () => {
    ...
    expect(client.createAgentToken).toHaveBeenCalledWith({
      labels: ['linux', PRIVILEGED_ROOT_LABEL, ...resolveRoleLabels(undefined)],
  ...
  it('register --mandatory-label is repeatable and unions into the authorized labels', async () => {
    ...
    expect(exitCode).toBeNull();
    expect(client.createAgentToken).toHaveBeenCalledWith({
      labels: ['a', 'b', ...resolveRoleLabels(undefined)],
      mandatoryLabels: ['a', 'b'],
    });
  });
  ...
  it('register with no taint flags omits mandatoryLabels', async () => {
    ...
    expect(exitCode).toBeNull();
    expect(client.createAgentToken).toHaveBeenCalledWith({
      labels: ['linux', ...resolveRoleLabels(undefined)],
      mandatoryLabels: undefined,
    });
  });

  it('register --labels authorizes the role namespace the agent will advertise', async () => {
    // The documented example. An agent presenting this token advertises
    // `kici:role:*` labels it derived from its own host; a scope without them
    // is refused at registration with close code 4010.
    const client = makeMockClient();
    client.createAgentToken.mockResolvedValue({ id: 'tok-r', token: 'kat_secret' });
    const { stdout, exitCode } = await runCommand(
      ['agent', 'register', '--labels', 'linux,x64,gpu'],
      client,
    );
    expect(exitCode).toBeNull();
    const { labels } = client.createAgentToken.mock.calls[0][0];
    expect(labels).toEqual(['linux', 'x64', 'gpu', ...resolveRoleLabels(undefined)]);
    expect(stdout).toContain('agent role labels authorized implicitly');
  });

  it('register with no labels and no taint mints an unscoped token', async () => {
    // `labels: undefined` is the back-compat unscoped token the register-time
    // gate skips entirely. Authorizing only the role namespace here would turn
    // it into a scope that refuses every operator label.
    const client = makeMockClient();
    client.createAgentToken.mockResolvedValue({ id: 'tok-u', token: 'kat_secret' });
    const { exitCode } = await runCommand(['agent', 'register'], client);
    expect(exitCode).toBeNull();
    expect(client.createAgentToken).toHaveBeenCalledWith({
      labels: undefined,
      mandatoryLabels: undefined,
