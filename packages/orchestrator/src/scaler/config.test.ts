...
describe('networkPolicySchema', () => {
  ...
  it('accepts an empty hostAccess as "reach nothing on the host"', () => {
    expect(networkPolicySchema.parse({ hostAccess: [] })?.hostAccess).toEqual([]);
  });

  it('rejects a hostname in hostAccess', () => {
    // A name resolved at rule-build time goes stale with no error anywhere, so
    // the config is refused rather than producing a rule that silently rots.
    expect(() => networkPolicySchema.parse({ hostAccess: ['registry.internal:5000'] })).toThrow(
      /not an IPv4 address/,
    );
  });

  it('rejects an out-of-range port in hostAccess', () => {
    expect(() => networkPolicySchema.parse({ hostAccess: ['*:70000'] })).toThrow(/outside 1-65535/);
  });
});
...
describe('label canonicalization at scaler config load', () => {
  // Scaler config is the second orchestrator ingress into the matching domain,
  // and the declared labels flow verbatim into a spawned agent's KICI_LABELS.
  // Nothing downstream re-folds them, so these assertions are the only guard
  // that an operator writing `Docker` still gets an agent a `runsOn: ['docker']`
  // job can reach.
  it('canonicalizes labelSets labels at parse', () => {
    const parsed = labelSetConfigSchema.parse({ labels: ['Docker', 'Linux'] });
    expect(parsed.labels).toEqual(['docker', 'linux']);
  });

  it('trims surrounding whitespace while folding', () => {
    const parsed = labelSetConfigSchema.parse({ labels: ['  Docker  '] });
    expect(parsed.labels).toEqual(['docker']);
  });

  it('canonicalizes mandatoryLabels and labelSets at parse', () => {
    const config = {
      version: 1,
      scalers: [
        {
    ...
    const result = scalerFileSchema.parse(config);
    expect(result.scalers[0].mandatoryLabels).toEqual(['gpu']);
    expect(result.scalers[0].labelSets[0].labels).toEqual(['linux', 'gpu']);
  });

  it('still rejects a reserved kici: prefix in labelSets after folding', () => {
    expect(() => labelSetConfigSchema.parse({ labels: ['kici:os:linux'] })).toThrow(/reserved/);
  });

  // The `kici:` refines fold case before testing the prefix, and that is
  // load-bearing: a scaler must never be able to declare an upper-cased system
  // label, which would otherwise reach the agent registry unchecked by the
  // privileged-root honesty gate.
  it('rejects an upper-cased reserved prefix in labelSets', () => {
    expect(() => labelSetConfigSchema.parse({ labels: ['KICI:PRIVILEGED:ROOT'] })).toThrow(
      /reserved/,
    );
  });

  it('rejects an upper-cased reserved prefix in mandatoryLabels', () => {
    const config = {
      version: 1,
      scalers: [
        {
    ...
    expect(() => scalerFileSchema.parse(config)).toThrow(/reserved/);
  });
});
