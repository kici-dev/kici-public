...
describe('EventScalerBackend scale-up mandatoryLabels', () => {
  it('emits the derived gate the spawn context carries, not the configured list alone', async () => {
    // `platform:` is on the entry for documentation only: the manager resolves
    // it into the context's `mandatoryLabels`; the backend never reads it.
    const { backend, emitUp } = makeBackend({
      entry: makeEntry({ platform: { os: 'linux', arch: 'arm64' } }),
    });

    await backend.spawn(['cloud=hetzner'], 'a-1', 'ws://orch/ws', undefined, undefined, {
      platformTaints: ['arm64'],
      mandatoryLabels: ['arm64'],
    });

    const payload = emitUp.mock.calls[0][0] as { labels: string[]; mandatoryLabels: string[] };
    // fails-when: the backend reads `this.entry.mandatoryLabels ?? []` — the
    // entry configures no gate, so the payload carries [] and the agent the
    // provisioning workflow starts is registered under a gate it never sees.
    expect(payload.mandatoryLabels).toContain('arm64');
    expect(payload.labels).toContain('arm64');
  });

  it('keeps a configured-only gate exactly as configured when the context carries it', async () => {
    const { backend, emitUp } = makeBackend({ entry: makeEntry({ mandatoryLabels: ['gpu'] }) });
    ...
    // breaks-if-wrong: a pool with no platform must still publish its
    // configured gate unchanged — no taint added, none dropped.
    expect((emitUp.mock.calls[0][0] as { mandatoryLabels: string[] }).mandatoryLabels).toEqual([
      'gpu',
    ]);
  });

  it('falls back to the configured gate when no spawn context reaches it', async () => {
    const { backend, emitUp } = makeBackend({ entry: makeEntry({ mandatoryLabels: ['gpu'] }) });

    await backend.spawn(['cloud=hetzner'], 'a-1', 'ws://orch/ws');

    // breaks-if-wrong: a context-less spawn must not lose the configured gate.
    expect((emitUp.mock.calls[0][0] as { mandatoryLabels: string[] }).mandatoryLabels).toEqual([
      'gpu',
    ]);
  });
