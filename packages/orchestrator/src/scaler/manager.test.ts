...
// Hoisted so the `@kici-dev/shared` factory below can close over it: `vi.mock`
// is lifted above every import, and a plain `const` would not exist yet.
const { mockLoggerError } = vi.hoisted(() => ({ mockLoggerError: vi.fn() }));

// Partial mock: only `createLogger` is replaced, so `toErrorMessage`,
// `normalizeLabelSet` and every other shared export the manager uses stay
// real. The error fake is what the warm-pool boot cases assert on; the rest
// are inert so the suite's log output stays quiet.
...
function makeEventBackend(
  opts: {
    entry?: ScalerEntry;
    /**
     * Widened past the `fakeEmitter` double so a late-bound wrapper of plain
     * functions — the shape the backend factory builds — can be injected.
     */
...
/**
 * The narrow registry view the manager consumes. `markDraining` returns the
 * agent's `activeJobs` so the caller can back out of a destroy for an agent
 * that took a job since the idle check.
 */
function fakeAgentRegistry(
  registered: Array<{ agentId: string; activeJobs: number; registeredAt: number }>,
) {
  const draining = new Set<string>();
  return {
    findAvailable: () => registered.filter((a) => !draining.has(a.agentId)),
    isRegistered: (agentId: string) => registered.some((a) => a.agentId === agentId),
    markDraining: (agentId: string) => {
      const entry = registered.find((a) => a.agentId === agentId);
      if (!entry) return undefined;
      draining.add(agentId);
      return entry.activeJobs;
    },
    clearDraining: (agentId: string) => {
...
describe('ScalerManager', () => {
  ...
  describe('requestScale()', () => {
    ...
    it('admits exactly one of two concurrent requests against globalMaxAgents: 1', async () => {
      // The real timing: `requestScale` fires the spawn asynchronously AFTER
      // the reservation lock is released, so `getActiveCount()` still reads 0
      // while the agent waits in the spawn semaphore. A count cap reading only
      // the backend's ledger therefore admits every request in a redrive
      // batch — ten agents against a cap of one.
      const backend = createMockBackend({
        type: 'container',
        labelSets: [{ labels: ['linux', 'docker'], image: 'agent:latest' }],
        maxAgents: 20,
      ...
      const manager = createManager(
        ...
        [{ name: 'container-prod', backend }],
      );

      const results = await Promise.all([
        manager.requestScale(['linux', 'docker'], 'job-a', 'run-test'),
        manager.requestScale(['linux', 'docker'], 'job-b', 'run-test'),
      ]);

      const admitted = results.filter((r) => r.action === 'spawning').length;
      const refused = results.filter((r) => r.action === 'at-capacity').length;
      expect(admitted).toBe(1);
      expect(refused).toBe(1);
    });

    it('admits exactly one of two concurrent requests against a per-scaler maxAgents: 1', async () => {
      const backend = createMockBackend({
        type: 'container',
        labelSets: [{ labels: ['linux', 'docker'], image: 'agent:latest' }],
        maxAgents: 1,
      ...
      expect(results.filter((r) => r.action === 'spawning')).toHaveLength(1);
      expect(results.filter((r) => r.action === 'at-capacity')).toHaveLength(1);
    });
    ...
    it("returns 'spawning' and triggers async spawn", async () => {
      ...
      expect(containerBackend.spawn).toHaveBeenCalledWith(
        ...
        { boundJobId: 'job-6', runId: 'run-test', platformTaints: [], mandatoryLabels: [] },
  ...
  describe('warm pool', () => {
    ...
    it('offers a warm agent to the reaper', async () => {
      ...
      const manager = makeManager({
        config: warmConfig(1),
        backends: [{ name: 'container-prod', backend: containerBackend }],
        agentRegistry: fakeAgentRegistry(registered),
      });
      (containerBackend.spawn as ReturnType<typeof vi.fn>).mockClear();
  ...
  describe('spawn context mandatoryLabels', () => {
    it('hands the event backend the same derived gate the spawn row is stamped with', async () => {
      const stateStore = fakeStateStore();
      const emitter = fakeEmitter();
      // ONE entry object shared by `config.scalers` and the backend: the
      // manager's name-keyed `mandatoryLabels` / `platform` lookups resolve
      // against the config copy, the backend serves the other.
      ...
      // The job asks for every taint the pool gates on, or the gate refuses it
      // and there is no spawn to observe.
      const result = await manager.requestScale(
        ['github-actions', 'kici:os:linux', 'arm64'],
        'job-1',
        'run-1',
      );
      await vi.advanceTimersByTimeAsync(0);
      ...
      expect(result.action).toBe('spawning');
      const payload = emitter.emitScalerScaleUp.mock.calls[0][0] as { mandatoryLabels: string[] };
      // fails-when: the manager hands the backend no `mandatoryLabels` — the
      // payload then carries only the configured `kici:os:linux`, while the
      // spawn row (and so the registered agent) is gated on `arm64` too.
      expect(payload.mandatoryLabels).toContain('arm64');
      expect(payload.mandatoryLabels).toContain('kici:os:linux');
      const row = stateStore.upsertSpawningAgent.mock.calls[0][0] as { mandatoryLabels: string[] };
      expect([...payload.mandatoryLabels].sort()).toEqual([...row.mandatoryLabels].sort());
    });

    it('hands a warm pre-spawn the derived gate beside its taints', async () => {
      const cfg = createDefaultConfig();
      const manager = createManager(
        {
          defaults: WARM_POOL_DEFAULTS,
      ...
      const call = (bareMetalBackend.spawn as ReturnType<typeof vi.fn>).mock.calls[0];
      // fails-when: the warm path builds its context from `platformTaints`
      // alone — the gate is then undefined and an event pool's warm agent is
      // announced with the configured list only.
      expect((call[5] as SpawnContext).mandatoryLabels).toEqual(['arm64']);
      expect((call[5] as SpawnContext).platformTaints).toEqual(['arm64']);
      // breaks-if-wrong: the backend is still asked for its own declared set.
      expect(call[0]).toEqual(['linux', 'gpu']);
    });
  });
  ...
  describe('recoverState() — owner-scoped hydration', () => {
    ...
    it('stamps the reservation owner, or the scoped read recovers no usage', async () => {
      ...
      // The spawn row has to be modelled too: recovery keeps a reservation only
      // while a rehydrated spawn backs it, so a fake that dropped the row would
      // exercise the orphan path and read as an owner-scoping failure. An event
      // spawn, because a local one is released at recovery by design (its
      // container cannot outlive the process) and would read the same way.
    ...
    it('drops a reservation row no spawn backs, instead of billing it forever', async () => {
      // A reservation is released by its spawn's lifecycle: the spawn fails, the
      // agent disconnects, or `pruneStaleSpawningEntries` reaps it. All three
      // read `spawningAgents`. So a reservation row whose spawning row is gone
      // is reachable by NONE of them — `recoverState` rehydrates it on every
      // restart and it bills against the caps for the life of the deployment.
      const reservations = new Map<string, Record<string, unknown>>();
      const stateStore = fakeStateStore({
        listReservationsForOwner: vi.fn(async () => [...reservations.values()]),
        deleteReservation: vi.fn(async (agentId: string) => {
          reservations.delete(agentId);
        }),
      });
      for (let i = 0; i < 3; i++) {
        reservations.set(`scaler-container-orphan-${i}`, {
          agentId: `scaler-container-orphan-${i}`,
          scalerName: 'container-prod',
          cpus: 1,
          memBytes: 1024,
          ownerInstanceId: 'orch-a',
        });
      ...
      expect(recovery.reservationsRehydrated).toBe(0);
      expect(recovery.reservationsDropped).toBe(3);
      // The durable rows go with them, or the next restart rehydrates the same
      // three and the drop has bought exactly one boot.
      expect(reservations.size).toBe(0);
      expect(stateStore.deleteReservation).toHaveBeenCalledTimes(3);
      // The usage they were billing is released with them.
      expect(restarted.getStatus().globalUsage).toEqual({ cpus: 0, memBytes: 0 });
    });

    it('keeps a reservation whose spawn row survived the restart', async () => {
      // The complement, and the reason the drop is keyed on the spawning map
      // rather than on age: a spawn still in flight across a restart owns its
      // reservation, and dropping that one would let the caps admit an agent
      // the previous life already started.
      const { rows, overrides } = spawningRowTable();
      const reservations = new Map<string, Record<string, unknown>>();
      const stateStore = fakeStateStore({
        ...overrides,
      ...
      expect(recovery.spawningAgentsRehydrated).toBe(1);
      expect(recovery.reservationsRehydrated).toBe(1);
      expect(recovery.reservationsDropped).toBe(0);
      expect(reservations.size).toBe(1);
    });

    it('spawns again after a restart that inherited only orphaned reservations', async () => {
      // The behavioural statement of the wedge. `reserveForSpawn` compares
      // `Math.max(activeCount, reserved)` against the caps at both levels, so
      // orphan rows past either one refuse every request — silently, since an
      // at-capacity verdict leaves the job queued and logs nothing. 40 rows on
      // one scaler clears the harness's `maxAgents: 5` many times over.
      const reservations = new Map<string, Record<string, unknown>>();
      const stateStore = fakeStateStore({
        listReservationsForOwner: vi.fn(async () => [...reservations.values()]),
        deleteReservation: vi.fn(async (agentId: string) => {
      ...
      await restarted.recoverState();
      const result = await restarted.requestScale(['default'], 'job-1', 'run-1');

      expect(result.action).toBe('spawning');
    });

    it('releases a recovered local-backend spawn instead of holding its reservation', async () => {
      // A container spawn cannot outlive the process that started it — the
      // restart's orphan cleanup removes the container and `adoptSpawningAgent`
      // adopts event rows only — so rehydrating it held a slot for nothing.
      // The shape from the field: the previous life had filled a five-slot
      // scaler when it died, and the restarted one refused every request as
      // reserved capacity nothing accounted for until the stale prune.
      const { rows, overrides } = spawningRowTable();
      const reservations = new Map<string, Record<string, unknown>>();
      const stateStore = fakeStateStore({
        ...overrides,
      ...
      const previous = makeManagerWithContainerBackend({ stateStore, instanceId: 'orch-a' });
      for (let i = 0; i < 5; i++) {
        await previous.requestScale(['default'], `job-${i}`, `run-${i}`);
      }
      await vi.advanceTimersByTimeAsync(0);
      // The previous life really did fill the pool: five spawn rows, five
      // reservations, and the harness scaler's `maxAgents: 5` is exhausted.
      expect(rows.size).toBe(5);
      expect(reservations.size).toBe(5);
      expect((await previous.requestScale(['default'], 'job-x', 'run-x')).action).toBe(
        'at-capacity',
      );
      ...
      // Released at hydration, not merely tolerated: the spawn rows are gone,
      // the reservations with them, and nothing is left for the prune to find.
      expect(recovery.spawningDropped).toBe(5);
      expect(recovery.spawningAgentsRehydrated).toBe(0);
      expect(recovery.reservationsDropped).toBe(5);
      expect(recovery.reservationsRehydrated).toBe(0);
      expect(restarted.getStatus().spawningCount).toBe(0);
      expect(rows.size).toBe(0);
      expect(reservations.size).toBe(0);
      // The behavioural statement: the very next request spawns, with no
    ...
    it('still rehydrates an event-backend spawn and its reservation', async () => {
      // breaks-if-wrong: the release above must be keyed on the backend type,
      // never on "recovered". An event spawn is a cloud instance the customer
      // is paying for, its row is the only pointer to it, and a peer can still
      // adopt it — dropping that one would let the caps admit an agent the
      // previous life already started, and lose the teardown. Same fixture
      // shape as the local case, opposite verdict.
      const { rows, overrides } = spawningRowTable();
      const reservations = new Map<string, Record<string, unknown>>();
      const stateStore = fakeStateStore({
        ...overrides,
      ...
      const previous = makeManagerWithEventBackend({ stateStore, instanceId: 'orch-a' });
      await previous.requestScale(['github-actions', 'kici:os:linux'], 'job-1', 'run-1');
      await vi.advanceTimersByTimeAsync(0);
      expect(rows.size).toBe(1);
      ...
      expect(recovery.spawningDropped).toBe(0);
      expect(recovery.spawningAgentsRehydrated).toBe(1);
      expect(recovery.reservationsRehydrated).toBe(1);
      expect(restarted.getStatus().spawningCount).toBe(1);
      expect(rows.size).toBe(1);
  ...
  describe('shutdownAll()', () => {
    ...
    it('stops warm pool and shuts down all backends', async () => {
      const manager = createManager();
      manager.start();
      // `start()` no longer arms the pool tick; without this the "stops warm
      // pool" half of the claim would pass against a pool that never ran.
      manager.startWarmPools();
      // Positive control: the pool tick (and the retirement sweep) really are
      // armed, so the zero below is a teardown, not a pool that never ran.
      ...
      // fails-when: shutdownAll() leaves the pool tick (or any other periodic
      // timer start() armed) running — a leaked interval keeps spawning into
      // backends that were just shut down.
...
describe('warm pool first fill on an event backend', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    // The fake accumulates across the whole file, so a stray error logged by
    // an earlier suite must not decide these cases.
  ...
  /**
   * The coordinator's wiring, in miniature. The backend is built while the
   * emitter is still unassigned and reaches it through a late-bound provider
   * that throws until `wire()` runs — the same wrapper of plain functions the
   * backend factory builds around the coordinator's provider — so a warm fill
   * that runs too early reaches the throwing provider on the exact path the
   * real spawn takes, rather than a `fakeEmitter()` that would swallow it.
   */
  function lateBoundHarness() {
    let emitter: ReturnType<typeof fakeEmitter> | null = null;
    const provider = () => {
      if (!emitter) throw new Error('event emitter not initialized before an event-scaler emit');
      return emitter;
    };
    const entry: ScalerEntry = {
      ...eventScalerEntry(),
      // The gate label must appear inside the label set or the pool is refused
      // as unfillable, and the set must declare a shape or it is refused as
      // undeclared — either refusal would leave `startWarmPools()` with
      // nothing to spawn and turn the first case into a false green.
      mandatoryLabels: ['github-actions'],
      labelSets: [
        { labels: ['github-actions'], resources: { requests: { cpus: 1, memory: '1g' } } },
      ],
  ...
  it('fills nothing at start() and emits the first fill once the emitter is wired', async () => {
    const { manager, wire } = lateBoundHarness();

    manager.start();
    await vi.advanceTimersByTimeAsync(0);

    // fails-when: `start()` runs the pools' first deficit pass itself — the
    // spawn reaches the unwired provider and the manager logs the failure.
    ...
    expect(emitter.emitScalerScaleUp).toHaveBeenCalledTimes(1);
    // An unbound fill: the pool spawns on its own deficit, naming no job.
    expect(emitter.emitScalerScaleUp.mock.calls[0]![0]).not.toHaveProperty('jobId');
    expect(mockLoggerError).not.toHaveBeenCalled();

    await manager.shutdownAll();
  });

  it('still serves a demand spawn through the same backend after the pools started', async () => {
    const { manager, wire } = lateBoundHarness();
    manager.start();
    const emitter = wire();
    manager.startWarmPools();
    ...
    // breaks-if-wrong: splitting the pool start out of `start()` must leave
    // the demand path untouched — a job-bound request still emits.
    await manager.requestScale(['github-actions'], 'job-1', 'run-1');
    await vi.advanceTimersByTimeAsync(0);

    expect(emitter.emitScalerScaleUp).toHaveBeenCalledTimes(2);
    expect(emitter.emitScalerScaleUp.mock.calls[1]![0]).toMatchObject({ jobId: 'job-1' });
    expect(mockLoggerError).not.toHaveBeenCalled();
