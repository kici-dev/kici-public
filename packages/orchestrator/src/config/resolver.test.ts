...
describe('envKeyToConfigPath', () => {
  ...
  it('maps the startup names whose overlay spelling differs', () => {
    // The boot path reads only these names, so the overlay has to resolve
    // them or a reload lands on a different value than the process booted
    // with.
    // fails-when: STARTUP_NAME_MAPPINGS loses an entry.
    expect(envKeyToConfigPath('KICI_PORT')).toEqual(['server', 'port']);
    expect(envKeyToConfigPath('KICI_BASE_PATH')).toEqual(['server', 'basePath']);
    expect(envKeyToConfigPath('KICI_MODE')).toEqual(['instance', 'mode']);
  });

  it('maps legacy unprefixed env vars used by startup loadConfig()', () => {
    // NODE_ENV is the only unprefixed name the overlay honors — it is an
    // OS-level convention. Every other setting is KICI_-prefixed only, so
    // the bare forms below resolve to nothing.
...
describe('applyEnvOverrides', () => {
  ...
  it('overrides server.port from the startup name KICI_PORT (coerced to number)', () => {
    // fails-when: KICI_PORT stops resolving — the merged config keeps the
    // YAML value and the reload reverts the port the process booted on.
    const config = { server: { port: 4000 } };
    const result = applyEnvOverrides(config, { KICI_PORT: '5000' });
    expect((result.server as Record<string, unknown>).port).toBe(5000);
  });

  it('overrides instance.mode from the startup name KICI_MODE', () => {
    const config = { instance: { mode: 'platform' } };
    const result = applyEnvOverrides(config, { KICI_MODE: 'independent' });
    expect((result.instance as Record<string, unknown>).mode).toBe('independent');
  });

  it('overrides server.basePath from the startup name KICI_BASE_PATH', () => {
    const config = { server: { basePath: '/' } };
    const result = applyEnvOverrides(config, { KICI_BASE_PATH: '/orch' });
    expect((result.server as Record<string, unknown>).basePath).toBe('/orch');
  });

  it('lets the startup name win when both spellings are set', () => {
    // Pinned deliberately: the boot path reads only the startup name, so a
    // reload that preferred the overlay spelling would resolve a value the
    // process never booted with.
    // fails-when: the startup group stops being applied last.
    const result = applyEnvOverrides(
      {},
      {
        KICI_SERVER_PORT: '4999',
        KICI_PORT: '5000',
        KICI_INSTANCE_MODE: 'platform',
        KICI_MODE: 'independent',
        KICI_SERVER_BASE_PATH: '/overlay',
        KICI_BASE_PATH: '/startup',
      },
    );
    expect((result.server as Record<string, unknown>).port).toBe(5000);
    expect((result.server as Record<string, unknown>).basePath).toBe('/startup');
    expect((result.instance as Record<string, unknown>).mode).toBe('independent');
  });

  it('still honors the overlay-only names on their own', () => {
    // breaks-if-wrong: these names are the documented direct-mapping surface
    // and stay live.
    const result = applyEnvOverrides(
      {},
      {
        ...
        KICI_SERVER_LOG_LEVEL: 'warn',
      },
    );
    expect((result.server as Record<string, unknown>).port).toBe(4999);
    expect((result.server as Record<string, unknown>).basePath).toBe('/overlay');
    expect((result.instance as Record<string, unknown>).mode).toBe('independent');
    expect((result.server as Record<string, unknown>).logLevel).toBe('warn');
  });
...
describe('resolveFullConfig', () => {
  // The shipped deployment has no YAML file at all: `orchestrator install`
  // writes an env file, so the reload path resolves an EMPTY local config plus
  // the environment. These four cases run that shape.
  describe('an env-file-only deployment (no YAML)', () => {
    it('resolves independent mode from KICI_MODE', () => {
      // fails-when: KICI_MODE stops reaching the merge — `mode` falls back to
      // its `platform` default and appConfigSchema then demands a Platform URL
      // and token, so every reload of every independent orchestrator fails
      // validation and the process keeps its old config.
      const result = resolveFullConfig({}, null, {
        KICI_MODE: 'independent',
    ...
    it('resolves the booted port from KICI_PORT', () => {
      const result = resolveFullConfig({}, null, {
        KICI_MODE: 'independent',
        KICI_DATABASE_URL: 'pg://env',
        KICI_PORT: '5000',
    ...
    it('resolves the instance id from KICI_CLUSTER_INSTANCE_ID', () => {
      // Without this the reload generates a fresh random id, which the
      // reloader then reports as a changed restart-required field on every
      // single reload.
      const result = resolveFullConfig({}, null, {
        KICI_MODE: 'independent',
        KICI_DATABASE_URL: 'pg://env',
        KICI_CLUSTER_INSTANCE_ID: 'orch-1',
      });
      expect(result.instanceId).toBe('orch-1');
    });

    it('still rejects platform mode with no token', () => {
      // breaks-if-wrong: the fix must reach the merge, never weaken the
      // cross-field rule. Falsified through the input — the token is unset.
      expect(() =>
        resolveFullConfig({}, null, {
          KICI_MODE: 'platform',
          KICI_DATABASE_URL: 'pg://env',
          KICI_PLATFORM_URL: 'wss://platform',
        }),
      ).toThrow(/platformToken is required/);
    });
  });

  it('YAML instance.id still wins over KICI_CLUSTER_INSTANCE_ID', () => {
    // breaks-if-wrong: the cluster id is a fallback, so an existing YAML that
    // names instance.id keeps resolving to the same value.
    const result = resolveFullConfig(
      makeMinimalLocal({ instance: { mode: 'independent', id: 'from-yaml' } }),
      null,
      { KICI_CLUSTER_INSTANCE_ID: 'from-env' },
    );
    expect(result.instanceId).toBe('from-yaml');
    expect(result.mode).toBe('independent');
  });
