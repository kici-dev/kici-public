...
function flattenToAppConfig(merged: Record<string, unknown>): Record<string, unknown> {
  ...
  // Instance. `cluster.instanceId` is the fallback because KICI_CLUSTER_INSTANCE_ID
  // is the startup name for this setting — `loadConfig()` reads it and nothing
  // else — so without it a reload regenerates a random id and reports the field
  // as changed-and-restart-required on every reload.
  ...
  flat.instanceId =
    instance?.id || cluster?.instanceId || `${hostname()}-${randomUUID().slice(0, 8)}`;
