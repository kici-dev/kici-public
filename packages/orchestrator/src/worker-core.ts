...
async function initializeWorkerScaler(
  ...
  scalerManager.start();
  // The worker hosts no event backend (the factory skips every event entry
  // here), so nothing a warm fill reaches is still unwired at this point.
...
export async function bootstrapWorker(
  ...
  // Fail fast when agent auth is disabled on a listener anyone can reach. A
  // worker serves the same agent WebSocket endpoint a coordinator does, with
  // the same `agentAuthMode`, and an agent that registers is dispatched jobs
  // carrying resolved secrets — so the credential-free registration surface is
  // refused here on exactly the terms it is refused on a coordinator.
