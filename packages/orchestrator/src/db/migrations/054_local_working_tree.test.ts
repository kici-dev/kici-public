...
import { down, up } from './054_local_working_tree.js';

const ADMIN_URL = process.env.KICI_TEST_ADMIN_DATABASE_URL;
const describeDb = ADMIN_URL ? describe : describe.skip;
const TEST_DB = `kici_mig054_test_${process.pid}_${Date.now()}`;
...
describeDb('migration 054_local_working_tree', () => {
  ...
  it('adds a non-null boolean column defaulting to false', async () => {
    const col = await column('execution_runs', 'local_working_tree');
  ...
  it('down() drops and up() re-adds the column idempotently', async () => {
    await down(db);
    expect(await column('execution_runs', 'local_working_tree')).toBeNull();
    await up(db);
    await up(db);
    expect(await column('execution_runs', 'local_working_tree')).not.toBeNull();
  });
});
