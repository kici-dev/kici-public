...
describe('AgentRegistry', () => {
  ...
  describe('draining', () => {
    ...
    // survives until the WebSocket closes. Every dispatch in that window landed
    // on an agent already exiting.
    it('excludes a draining agent from findAvailable', () => {
      ...
      const available = registry.findAvailable(['linux']);
      expect(available.map((a) => a.agentId)).toEqual(['agent-2']);
    });

    it('excludes a draining agent from findAvailable with no required labels', () => {
      // The no-labels branch is a separate code path from the label-indexed one.
      registry.register('agent-1', mockWs(), ['linux']);
      registry.markDraining('agent-1');
      expect(registry.findAvailable([])).toHaveLength(0);
    });

    it('excludes a draining agent from hasMatchingAgent', () => {
      // The two choke points must reject the same dispatches, or the reroute
      // path is told a local agent can take work findAvailable will not give it.
      registry.register('agent-1', mockWs(), ['linux']);
    ...
    it('reports the active-job count at the moment of the mark', () => {
      // `listIdle` reads activeJobs a tick before the destroy is issued, so a
      // job claimed in between must be visible to the caller — which is what
      // lets it back out rather than killing an agent mid-job.
      registry.register('agent-1', mockWs(), ['linux']);
      registry.incrementActiveJobs('agent-1');
      expect(registry.markDraining('agent-1')).toBe(1);
    });

    it('makes an agent dispatchable again after clearDraining', () => {
      // A rejected destroy leaves the agent alive and working.
      registry.register('agent-1', mockWs(), ['linux']);
      registry.markDraining('agent-1');
      registry.clearDraining('agent-1');
      expect(registry.findAvailable(['linux'])).toHaveLength(1);
      expect(registry.hasMatchingAgent(['linux'])).toBe(true);
    });

    it('registers a reconnecting agent as not draining', () => {
      registry.register('agent-1', mockWs(), ['linux']);
      registry.markDraining('agent-1');
      // A reconnect is a NEW connection with new work to do.
      registry.register('agent-1', mockWs(), ['linux']);
      expect(registry.findAvailable(['linux'])).toHaveLength(1);
    });

    it('returns undefined when marking an agent that is not registered', () => {
      expect(registry.markDraining('nobody')).toBeUndefined();
    });
  });
  ...
  describe('mandatoryLabels gate (k8s-style taints)', () => {
    ...
    it('satisfies a mandatory gate that differs from runsOn only by case', () => {
      registry.register('a1', mockWs(), ['gpu', 'linux'], 'linux', 'x64', '1.0.0', 1, {
        mandatoryLabels: ['GPU'],
      });
      expect(registry.findAvailable(['gpu', 'linux'])).toHaveLength(1);
      expect(registry.hasMatchingAgent(['gpu', 'linux'])).toBe(true);
    });

    it('folds a scaler-supplied gate onto the entry it stores', () => {
      // The scaler config folds `mandatoryLabels` at load, but the value also
      // reaches here from an adopted spawn record and from the re-register
      // branch's `[...existingEntry.mandatoryLabels]`. The entry is what every
      // gate check reads, so the fold is asserted where it is stored.
      registry.register('a1', mockWs(), ['GPU', 'Linux'], 'linux', 'x64', '1.0.0', 1, {
        mandatoryLabels: ['GPU'],
      });
      const entry = registry.get('a1');
      expect(entry!.mandatoryLabels).toEqual(new Set(['gpu']));
      expect(entry!.labels).toEqual(new Set(['gpu', 'linux']));
    });
  });

  describe('label case folding', () => {
    it('finds an agent registered with mixed-case labels via a lowercase requirement', () => {
      registry.register('a1', mockWs(), ['Docker'], 'linux', 'x64', '1.0.0', 1);
      expect(registry.findAvailable(['docker'])).toHaveLength(1);
      expect(registry.hasMatchingAgent(['docker'])).toBe(true);
      expect(registry.agentSatisfies(registry.get('a1')!, ['docker'])).toBe(true);
    });

    it('finds an agent registered with lowercase labels via a mixed-case requirement', () => {
      registry.register('a1', mockWs(), ['docker'], 'linux', 'x64', '1.0.0', 1);
      expect(registry.findAvailable(['DOCKER'])).toHaveLength(1);
      expect(registry.hasMatchingAgent(['DOCKER'])).toBe(true);
    });

    it('excludes on a differently-cased exclude label', () => {
      // The fold must reach the exclusion arm too: a job excluding `Spot` from
      // an agent labelled `spot` would otherwise land on exactly the agent it
      // named — a silent widening rather than a silent no-match.
      registry.register('a1', mockWs(), ['linux', 'spot']);
      expect(registry.findAvailable(['linux'], [], ['Spot'])).toHaveLength(0);
      expect(registry.hasMatchingAgent(['linux'], [], ['Spot'])).toBe(false);
      expect(registry.agentSatisfies(registry.get('a1')!, ['linux'], [], ['Spot'])).toBe(false);
    });

    it('matches a regex pattern against a differently-cased label', () => {
      // A regex source cannot be lowercased without corrupting it, so the fold
      // rides on the `i` flag `canonicalizeMatcher` forces.
      registry.register('a1', mockWs(), ['GPU-Large']);
      const pattern = { kind: 'regex' as const, source: '^gpu-.*$', flags: '' };
      expect(registry.findAvailable([], [pattern])).toHaveLength(1);
      expect(registry.hasMatchingAgent([], [pattern])).toBe(true);
      expect(registry.agentSatisfies(registry.get('a1')!, [], [pattern])).toBe(true);
      // …and the same forced flag must reach the exclusion arm.
      expect(registry.findAvailable([], [], [], [pattern])).toHaveLength(0);
    });
  });
