 ...
 * orchestrator.
 */
export const DeferredAttestationRejectReason = {
  ...
  StatementHashMismatch: 'statement_hash_mismatch',
  /** The frozen statement disagrees with the run row on a build fact. */
  BuildContextMismatch: 'build_context_mismatch',
...
export type DeferredAttestationRejectReason =
  (typeof DeferredAttestationRejectReason)[keyof typeof DeferredAttestationRejectReason];
 ...
 */
export const deferredAttestationRejectedTotal = lazyCounter(
  'kici_orch_deferred_attestation_rejected_total',
  {
    description:
      'Deferred provenance attestations refused at capture because the frozen statement did not check out against the orchestrator run row.',
  },
);

/** Record one refused deferred-attestation capture. */
export function incDeferredAttestationRejected(reason: DeferredAttestationRejectReason): void {
  deferredAttestationRejectedTotal.add(1, { reason });
}
 ...
 */
export const webhookPipelineFailuresTotal = lazyCounter(
  'kici_orch_webhook_pipeline_failures_total',
  {
    description:
      'Webhook match-and-dispatch pipelines that threw. Labels: phase (post_ack — failed after the delivery was acknowledged)',
  },
);
...
export const ingestOverflowDroppedTotal = lazyCounter('kici_orch_ingest_overflow_dropped_total', {
  description:
    'Total overflow-buffered deliveries permanently dropped. Labels: reason (cap_full|max_attempts|max_age)',
});
 ...
 */
export const ingestOverflowReplayRefusedTotal = lazyCounter(
  'kici_orch_ingest_overflow_replay_refused_total',
  {
    description:
      'Total overflow replay reservations refused by the admission controller (the row stays buffered and costs no attempt). Labels: reason (loop_overload|queue_full)',
  },
);
...
let _trustDirectoryUpdatedAtMs: number | undefined;
 ...
 */
export function setTrustDirectoryUpdatedAt(updatedAt: Date): void {
  _trustDirectoryUpdatedAtMs = updatedAt.getTime();
}

/**
 * Seconds since the approval directory was last refreshed.
 *
 * `/kici approve` is authorized against that directory, and it carries no TTL
 * on purpose: an expiring cache would refuse approvals during exactly the
 * Platform outage it exists to survive. The cost is that a member whose CI
 * trust the Platform revoked keeps it here until the next push lands, and an
 * orchestrator that cannot reach the Platform never gets that push. This gauge
 * is how long that window has been open.
 *
 * @conditionallyEmitted An independent orchestrator receives no push, and a
 * Platform-attached one emits nothing until its first push lands.
 *
 * Deliberately unset rather than zero before that first push: zero reads as a
 * directory refreshed this instant, which is the opposite of never refreshed.
 */
defineObservableGauge(
  'kici_orch_trust_directory_age_seconds',
  {
    description:
      'Seconds since the approval directory /kici approve is authorized against was last refreshed',
    unit: 's',
  },
  (result) => {
    if (_trustDirectoryUpdatedAtMs === undefined) return;
    // Clamped, because the timestamp is the writer's clock and this is the
    // reader's: a backwards skew would otherwise report a negative age.
    result.observe(Math.max(0, (Date.now() - _trustDirectoryUpdatedAtMs) / 1000));
  },
);
 ...
 * - outcome: ok | error
 */
export const deferredIndexBuildsTotal = lazyCounter('kici_orch_deferred_index_builds_total', {
  description: 'Total post-boot deferred index builds by outcome',
});
 ...
 */
export const retentionRowsDeletedTotal = lazyCounter('kici_orch_retention_rows_deleted_total', {
  description: 'Total rows deleted by the database retention sweep, by table',
});
...
// ── Tier-2 global eval round ──────────────────────────────────────
//
// A global workflow declaring a `filter` predicate or a `DynamicJobFn` is
// decided by a pre-run agent job, so nothing the orchestrator logs explains
// what the round concluded. These five instruments are that explanation: how
// many candidates entered, what each of them was decided to be, how long the
// round took, how many jobs its generators produced, and whether the
// redelivery cache answered.
 ...
 */
export const globalEvalCandidatesTotal = lazyCounter('kici_orch_global_eval_candidates_total', {
  description: 'Global workflow candidates handed to a Tier-2 eval round',
});
 ...
 */
export const globalEvalVerdictsTotal = lazyCounter('kici_orch_global_eval_verdicts_total', {
  description: 'Per-candidate verdicts produced by a Tier-2 global eval round',
});
 ...
 */
export const globalEvalRoundDurationSeconds = lazyHistogram(
  'kici_orch_global_eval_round_duration_seconds',
  {
    description: 'Duration of one dispatched Tier-2 global eval round in seconds',
    advice: { explicitBucketBoundaries: [1, 5, 10, 30, 60, 120, 240, 600] },
  },
);
 ...
 */
export const globalEvalJobsGenerated = lazyHistogram('kici_orch_global_eval_jobs_generated', {
  description: 'Jobs generated by a Tier-2 global eval round',
  advice: { explicitBucketBoundaries: [0, 1, 2, 5, 10, 25, 50, 100] },
});
 ...
 */
export const globalEvalCacheLookupsTotal = lazyCounter(
  'kici_orch_global_eval_cache_lookups_total',
  { description: 'Tier-2 global eval round cache lookups by outcome' },
);
...
export const containerRegistryAuthContributorStrippedTotal = lazyCounter(
  ...
  {
    description:
      'Job dispatches where container-registry credentials were withheld because the contributor tier was not `trusted`',
  },
);
 ...
 */
export const dispatchOwnershipSkippedTotal = lazyCounter(
  'kici_orch_dispatch_ownership_skipped_total',
  {
    description:
      'Dispatch-queue actions skipped because the row is owned elsewhere or already resolved',
  },
);
 ...
 */
export const nonMonotonicJobStatusWritesTotal = lazyCounter(
  'kici_orch_non_monotonic_job_status_writes_total',
  {
    description:
      'execution_jobs status writes rejected because the terminal row contradicts the frame',
  },
);
...
export const unroutableFastFailedTotal = lazyCounter('kici_orch_unroutable_fast_failed_total', {
  description: 'Jobs terminalized as unroutable by the fast-fail probe (not the queue timeout)',
});
...
/**
 * Per-channel LISTEN connection state, reported by the observable gauge below.
 * A dropped subscription is otherwise invisible: the process stays up and
 * `/health` stays 200 while the channel is deaf.
 */
const _pgListenConnected = new Map<string, boolean>();

/** Record whether the LISTEN subscription on `channel` currently holds a client. */
export function setPgListenConnected(channel: string, connected: boolean): void {
  _pgListenConnected.set(channel, connected);
}

/**
 * Per-channel LISTEN connection state.
 * Labels:
 * - channel: the LISTEN channel name
 */
defineObservableGauge(
  'kici_orch_pg_listen_connected',
  { description: '1 when the LISTEN subscription on this channel holds a live connection' },
  (result) => {
    for (const [channel, connected] of _pgListenConnected) {
      result.observe(connected ? 1 : 0, { channel });
    }
  },
);
 ...
 * - channel: the LISTEN channel name
 */
export const pgListenReconnectsTotal = lazyCounter('kici_orch_pg_listen_reconnects_total', {
  description: 'Total LISTEN subscriptions re-established after a connection loss',
});
