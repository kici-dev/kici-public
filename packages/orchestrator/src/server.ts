...
import { isProductionEntry } from './entry-helpers.js';
...
const { settleSecurityCheckForDecision } = await import('./pipeline/security-hold-check.js');
...
const {
  ...
  setRejectedAttestations,
  setTrustDirectoryUpdatedAt,
...
import type { ProcessingDeps, ReadyDispatchContextRow } from './pipeline/processor.js';
import { resolveOrgId } from './pipeline/processor.js';
import { provisionRemoteSource } from './pipeline/remote-source-store.js';
import { readDeploymentIdentity, readConfigPaths } from './deployment/deployment-identity.js';
...
// Type-only: the test-only fault-injection policy. `import type` erases at
// runtime, so the `src/testing/*` module never enters the shipped bundle.
...
export async function runServer(
  ...
  await guardStartup(logger, async () => {
    ...
    const buildServerHooks = (
      ...
      return {
          ...
          onStepStatusForward: (
            ...
            const doWork = () => {
              ...
              platformClient.send({
                ...
                stepName,
                state: state as
                  'running' | 'success' | 'failed' | 'skipped' | 'pending' | 'cancelled',
                timestamp,
        ...
        onSubsystemsReady: async (sub) => {
          // Identity and RBAC state (updated by Platform push), read by the
          ...
          const applyTrustDirectory = (directory: TrustDirectory): void => {
            ...
            teamMemberships = new Map(
              directory.teamMemberships.map((t) => [t.teamName, new Set(t.memberUserIds)]),
            );
          };
          ...
          try {
            ...
            if (cached) {
              ...
              logger.info('Approval directory restored from cache', {
                orgId: cached.orgId,
                identityLinks: cached.identityLinks.length,
            ...
            logger.warn('Failed to restore the approval directory from cache', {
              error: toErrorMessage(err),
            });
          }
          ...
          const buildProcessingDeps = (): ProcessingDeps => ({
            ...
            eventRouter: sub.eventRouter,
            invokeGateDeps: sub.invokeGateDeps,
          ...
          const buildFleetDeps = () => ({
            ...
            rosterStore: sub.hostRosterStore!,
            rosterGraceMs: config.rosterGraceMs,
            resolveRunsOnAll: (workflowName: string) =>
          ...
          platformClient = new PlatformClient({
            ...
            configPaths: readConfigPaths(process.env, {
              scalerConfigPath: config.scalerConfigPath,
              scalerConfigDir: config.scalerConfigDir,
            }),
            ...
            onShedCapture: async (meta, body, reason) => {
              await recordShedBreadcrumb(
                {
                  eventLog: sub.eventLogWriter,
                  resolveOrgId: (routingKey) => resolveOrgId(sub.db, routingKey),
                },
                meta,
                body,
                reason,
            ...
            onTrustPolicyUpdate: (msg) => {
              ...
              void trustPolicyStore
                .upsertFromPlatform(msg.orgId, msg.policy)
                .catch((err: unknown) =>
              ...
              const directory: TrustDirectory = {
                ...
                memberCiTrustLevels: msg.memberCiTrustLevels,
                teamMemberships: msg.teamMemberships ?? [],
              };
              // Same fire-and-forget shape as the policy write above, and for
              // the same reason: the in-memory directory must refresh even when
              // the DB write fails. The push replaces the cached directory
              // wholesale, exactly as it replaces the in-memory one.
              void trustDirectoryStore
                .upsertFromPlatform(msg.orgId, directory)
                .catch((err: unknown) =>
                  logger.error('Failed to persist the pushed approval directory', {
                    orgId: msg.orgId,
                    error: toErrorMessage(err),
                  }),
                );
              applyTrustDirectory(directory);
              // Stamped from the push's arrival rather than from the row the
              // fire-and-forget write above will land: the in-memory directory
              // is what authorizes approvals, and it just refreshed whether or
              // not that write succeeds.
              setTrustDirectoryUpdatedAt(new Date());
            ...
            onAuthenticated: async () => {
              ...
              try {
                ...
                if (resolved) {
                  ...
                  dashboardBackendsHandler.setOrgContext(
                    resolved.customer_id,
                    resolved.routing_key,
                  );
                  dashboardFleetWriteHandler.setOrgContext(
                    resolved.customer_id,
                    resolved.routing_key,
                  );
              ...
              try {
                const replayData =
                  await sub.executionTracker.getReplayDataWithDb(replayWindowHours);
            ...
            onWebhookRelay: async (relay) => {
              ...
              // The `failed` event-log row is `processWebhook`'s own, for every
              // ingress rather than this one. A catch here would write a SECOND
              // row: `event_log` conflicts on (org_id, delivery_id), and this
              // one could only guess `__default__` for the org — a different key
              // from the resolved org the pipeline writes under, and a row the
              // org-scoped dashboard view cannot see.
          ...
          if (relayEnabled && config.ingestOverflowEnabled && sub.pgSecretStore) {
            ...
            sub.ingestOverflowReplayer.setReinjectRelay(
              buildRelayReinject({
                // No admit seam: the replayer reserves the admission slot before
                // it claims the row and holds it across this whole call.
                verify: (d, body) =>
          ...
          return {
            appDepsExtras: {
              ...
              // A JOB-scoped wait-timer hold resumes by re-dispatching the one
              // job it held — the same seam a reviewer approval takes. It has a
              // pending job context, not a pending workflow context, so it
              // cannot go through onWorkflowRelease.
              // Lets the stale detector's queued-concurrency sweep read each
              // group's configured limit before releasing into it, and arms the
              // ready-dispatch re-gate that `orchestrator-core` reads off these
              // extras.
              matchContext: (orgId: string, name: string) =>
                contextStore.matchContext(orgId, name) as Promise<ReadyDispatchContextRow | null>,
              onJobRelease: async (signal: ReleaseSignal) => {
                await dispatchReadyJob(
                  signal.runId,
                  signal.jobId,
                  sub.dispatcher,
                  ...
                  sub.db,
                  sub.invokeGateDeps,
                  // The sweep releases against a count it read before this job
                  // reached the chokepoint, so the limit is re-checked here.
                  {
                    matchContext: (o, n) => contextStore.matchContext(o, n),
                    heldRunStore,
                    accessLogWriter: sub.accessLogWriter,
            ...
            onServerStarted: async () => {
              // Wire the change-driven membership reporter now that the platform
              // client exists: a worker peer joining or leaving re-reports the
              // coordinator's cluster.membership snapshot immediately.
              sub.setMembershipReporter(() => platformClient.sendClusterMembership());
    ...
    // Fault injection comes from the caller: the test-only server-test.ts entry
    // passes a policy; the production entry passes nothing, so every seam falls
    // through to the existing config path and behavior is unchanged.
    const hooks = buildServerHooks(config, opts.faultInjection);
    await bootstrapOrchestrator(config, hooks, {
      orchestratorVersion: ORCHESTRATOR_VERSION,
      otelSdk,
...
// Production entry point: boot the orchestrator only when this module is the
// process entry — never on bare import, and never when its code has been
// inlined into the dev-only `server-test.js` entry (which boots `runServer`
// itself with a fault policy). `isProductionEntry` fires for every real
// production output name (`server.js`, `kici-orchestrator.cjs`/`.mjs`) and
// excludes only `server-test.js`. See entry-helpers.ts for the full rationale.
if (isProductionEntry(process.argv[1], import.meta.url)) {
  await runServer({});
}
