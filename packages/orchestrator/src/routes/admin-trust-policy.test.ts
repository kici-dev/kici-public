...
function makeApp(opts: {
  ...
  /**
   * Platform liveness getter. Omitted by default, which is the independent /
   * older-orchestrator shape: the route then reports no connection state.
   */
  ...
  const inner = createTrustPolicyRoutes({
    ...
    mode: opts.mode,
    ...(opts.platformConnected && { platformConnected: opts.platformConnected }),
    accessLog,
  });
  const app = new Hono();
...
describe('GET /trust-policy/directory', () => {
  ...
  it('reports the live connection state, not the configured mode', async () => {
    // `platformManaged` says a Platform owns the directory; it cannot say one
    // is reachable. The CLI escalates its staleness warning on the difference,
    // so the two must be independently observable on one response.
    const base = { mode: 'platform' as const, storedDirectory: STORED_DIRECTORY };
    const down = await getDirectory(makeApp({ ...base, platformConnected: () => false }).app);
    expect(down.body.platformManaged).toBe(true);
    expect(down.body.platformConnected).toBe(false);

    const up = await getDirectory(makeApp({ ...base, platformConnected: () => true }).app);
    expect(up.body.platformConnected).toBe(true);
  });

  it('re-reads the connection state on every request', async () => {
    // Captured once, a reconnect would need the route re-mounted to be seen —
    // and the route is mounted at boot.
    let connected = false;
    const { app } = makeApp({
      mode: 'platform',
      storedDirectory: STORED_DIRECTORY,
    ...
    expect((await getDirectory(app)).body.platformConnected).toBe(false);
    connected = true;
    expect((await getDirectory(app)).body.platformConnected).toBe(true);
  });

  it('omits the connection state entirely when there is none to report', async () => {
    // An independent orchestrator passes no getter. The field must be ABSENT
    // rather than `false`: the CLI reads absent as unknown, and a literal
    // `false` there would warn about an outage that cannot exist.
    const { app } = makeApp({ mode: 'independent', storedDirectory: STORED_DIRECTORY });
    const { body } = await getDirectory(app);
    expect(body).not.toHaveProperty('platformConnected');
  });
