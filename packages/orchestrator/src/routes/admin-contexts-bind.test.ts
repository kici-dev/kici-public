 ...
 *   envStore.getByName → selectFrom('contexts').selectAll().where().where().executeTakeFirst()
 *   existence check    → selectFrom('context_bindings').select().where×4.executeTakeFirst()
 *   insert             → insertInto('context_bindings').values(...).execute()
 */
function buildMockDb(opts: {
  ...
  const db = {
    selectFrom: vi.fn().mockImplementation((table: string) => {
      ...
      if (table === 'context_bindings') return { select: bindingSelect };
      return {};
    }),
    insertInto: vi.fn().mockImplementation((table: string) => {
      if (table === 'context_bindings') return { values: insertValues };
      return {};
    }),
  };
...
describe('POST /contexts/:name/bind — host_pattern', () => {
  const orgId = 'org-1';
  ...
  it('persists an explicit host_pattern', async () => {
    ...
    const res = await app.request('http://localhost/contexts/production/bind', {
      method: 'POST',
