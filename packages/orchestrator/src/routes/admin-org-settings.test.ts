...
vi.mock('../policy/dashboard-write-policy.js', async (importOriginal) => ({
  getDashboardWritePolicy: vi.fn(),
  setDashboardWritePolicy: vi.fn(),
  resetDashboardWritePolicy: vi.fn(),
  // The real class, not a stub: the route branches on `instanceof`, so a
  // mock that omitted it would make every error in this handler a 500.
  HeldRunLockoutRefusedError: ((await importOriginal()) as { HeldRunLockoutRefusedError: unknown })
...
describe('PATCH /org-settings/dashboard-writes', () => {
  ...
  // The refusal is a coherent conflict the operator can act on, so it must
  // reach the CLI verbatim rather than as a generic 500.
  it('surfaces a refused held-run lockout as a 409 carrying the policy message', async () => {
    vi.mocked(setDashboardWritePolicy).mockRejectedValue(
      new HeldRunLockoutRefusedError(['held_runs.approve']),
    );
    const app = buildTestApp();
    const res = await app.request('/org-settings/dashboard-writes', {
      method: 'PATCH',
    ...
    const body = (await res.json()) as { error: string; operations: string[] };
    expect(body.operations).toEqual(['held_runs.approve']);
    expect(body.error).toContain('Platform-attached');
  });

  it('threads the orchestrator mode into the policy write', async () => {
    const app = buildTestApp();
