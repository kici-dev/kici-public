...
function makeApp(opts: {
  ...
  /** Members an operator registered with `trust-policy directory-set`. */
  members?: StoredTrustDirectory['memberCiTrustLevels'];
  dbRows?: DbRows;
}) {
  ...
  const directory = {
    load: vi.fn(async () =>
      opts.teams || opts.members
        ? ({
            identityLinks: [],
            memberCiTrustLevels: opts.members ?? {},
...
describe('admin held-run routes', () => {
  ...
  // Without `--as` the approver subject is `service:<token>`, which satisfies
  // neither a `{user}` clause nor a `{team}` membership — so a clause-bearing
  // hold on an independent orchestrator could only expire.
  describe('answering as a registered member (--as)', () => {
    const userClauseHold = () =>
      makeHold({
        approval_requirement: {
          clauses: [{ user: 'alice' }],
          expiresAt: '2026-08-29T00:00:00Z',
    ...
    it('satisfies a {user} clause naming the member', async () => {
      const h = makeApp({
        userId: 'ops-token',
        members: { alice: 'write' },
        holds: [userClauseHold()],
      });
      const res = await h.request({ ...approveBody, asUserId: 'alice' });
      expect(res.status).toBe(200);
      expect(h.onJobRelease).toHaveBeenCalledTimes(1);
      expect(h.decisions[0].approver_user_id).toBe('alice');
    });

    it('satisfies a {team} clause the member belongs to', async () => {
      const h = makeApp({
        userId: 'ops-token',
        members: { alice: 'write' },
        teams: [{ teamName: 'platform', memberUserIds: ['alice'] }],
        holds: [
          makeHold({
            approval_requirement: {
              clauses: [{ team: 'platform' }],
    ...
    // The flag is a subject, so an unvalidated one would satisfy any clause by
    // naming it. `mallory` is not in the directory even though the clause names
    // her, so the refusal cannot be the clause failing to match.
    it('refuses an id the approval directory does not carry, and dispatches nothing', async () => {
      const h = makeApp({
        members: { alice: 'write' },
        holds: [
          makeHold({
            approval_requirement: {
              clauses: [{ user: 'mallory' }],
              expiresAt: '2026-08-29T00:00:00Z',
              reason: 'r',
            },
          }),
      ...
      const res = await h.request({ ...approveBody, asUserId: 'mallory' });
      expect(res.status).toBe(400);
      expect((await res.json()).error).toContain('trust-policy directory-set');
      expect(h.decisions).toHaveLength(0);
      expect(h.onJobRelease).not.toHaveBeenCalled();
      expect(h.holds[0].status).toBe(HeldRunStatus.Pending);
    });

    it('refuses every id when no directory has ever been written', async () => {
      const h = makeApp({ holds: [userClauseHold()] });
      const res = await h.request({ ...approveBody, asUserId: 'alice' });
      expect(res.status).toBe(400);
      expect(h.decisions).toHaveLength(0);
    });

    it('leaves the token subject untouched when --as is omitted', async () => {
      const h = makeApp({ userId: 'ops-token', members: { alice: 'write' } });
      const res = await h.request(approveBody);
      expect(res.status).toBe(200);
      expect(h.decisions[0].approver_user_id).toBe(adminActorSub('ops-token'));
    });

    // The self-approval gate compares `actorSub` against the triggerer's
    // subject, so it is only live once both are written in the same vocabulary.
    it('refuses a member approving a run they triggered when self-approval is off', async () => {
      const h = makeApp({
        members: { alice: 'write' },
        holds: [userClauseHold()],
        dbRows: {
          org_settings: { allow_self_approval: false },
          execution_runs: { triggered_by: stringifyActor({ type: 'user', sub: 'alice' }) },
        },
      });
      const res = await h.request({ ...approveBody, asUserId: 'alice' });
      expect(res.status).toBe(409);
    ...
    // The positive control for the refusal above: identical fixture, only
    // `allow_self_approval` differs.
    it('allows the same member when the org left self-approval on', async () => {
      const h = makeApp({
        members: { alice: 'write' },
        holds: [userClauseHold()],
        dbRows: {
      ...
      expect((await h.request({ ...approveBody, asUserId: 'alice' })).status).toBe(200);
      expect(h.onJobRelease).toHaveBeenCalledTimes(1);
    });

    it('records the token as the actor and the member in meta, never one instead of the other', async () => {
      const h = makeApp({
        userId: 'ops-token',
        members: { alice: 'write' },
        holds: [userClauseHold()],
      });
      await h.request({ ...approveBody, asUserId: 'alice' });
      const row = h.auditRows.find((r) => r.action === 'held_run.approve');
      expect(row).toMatchObject({
        actor: { type: 'service_account', id: 'ops-token' },
        outcome: 'allowed',
      });
      expect(row?.meta).toMatchObject({ approvedAsUserId: 'alice' });
    });
  });
