...
export type RedeliverDeliveries = (
  app: { appId: string; privateKey: string },
  opts: { since: Date; until: Date; dryRun?: boolean },
) => Promise<RedeliverWindowResult>;

/**
 * Body of `POST /sources/:routingKey/redeliver`. Both bounds are required: a
 * replay with no upper bound would resend everything GitHub still holds, which
 * is never what an operator recovering one outage window means.
 */
const redeliverRequestSchema = z.object({
...
export function createSourceRoutes(deps: SourceRouteDeps): Hono<AdminSourcesEnv> {
  ...
  // POST /api/v1/admin/sources/:routingKey/redeliver -- ask GitHub to send this
  // App's deliveries from a time window again. The orchestrator holds the App
  // private key, so it is the only tier that can drive the App-level delivery
  // endpoints; nothing upstream carries that credential. Recovers a window
  // whose deliveries never landed, since GitHub does not retry on its own.
  app.post('/sources/:routingKey/redeliver', async (c) => {
    try {
      const routingKey = c.req.param('routingKey');
      const denied = enforceRoutingKeyScope(c, routingKey);
      if (denied) return denied;

      const parsed = redeliverRequestSchema.safeParse(await c.req.json().catch(() => null));
      if (!parsed.success) {
        return c.json(
          { error: 'Invalid request: since and until must be ISO-8601 timestamps' },
          400,
        );
      }
      ...
      if (since >= until) {
        return c.json({ error: 'Invalid request: since must be earlier than until' }, 400);
      }
      ...
      if (source.provider !== 'github') {
        return c.json(
          {
            error: `Source ${routingKey} is not a GitHub source (provider=${source.provider}); delivery replay is a GitHub App capability.`,
          },
          400,
        );
      }
      ...
      if (!appConfig?.appId) {
        return c.json({ error: `Source ${routingKey} has no stored GitHub App id` }, 400);
      }

      const redeliver = deps.redeliverDeliveries ?? redeliverWindow;
      const result = await redeliver(
        { appId: appConfig.appId, privateKey: source.privateKey },
        { since, until, dryRun: parsed.data.dryRun },
      );
      logger.info('Replayed GitHub delivery window', {
        routingKey,
        since: result.since,
        until: result.until,
      ...
      return c.json({ routingKey, ...result });
    } catch (err) {
      logger.error('Failed to replay GitHub deliveries', { error: toErrorMessage(err) });
      return c.json({ error: toErrorMessage(err) }, 500);
    }
  });
