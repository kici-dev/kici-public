...
import { DEFAULT_FORK_POLICY, EffectivePolicySource } from '../security/trust-policy-gate.js';
...
describe('evaluateSecurityPolicy', () => {
  ...
  it('ignores a fork PR with no stored policy, in every mode', async () => {
    ...
    for (const mode of OrchestratorMode.options) {
      ...
      expect(outcome, `mode ${mode}`).toEqual({
        action: 'ignore',
        forkSwitch: {
          policy: ForkPolicy.enum.ignore,
          source: EffectivePolicySource.enum.unconfigured,
          // Named by meaning, not by re-calling the constant the code reads:
          // `independent` is the one mode with no Platform to own the policy,
          // and so the one whose remedy cannot be the dashboard.
          platformManaged: mode !== OrchestratorMode.enum.independent,
        },
      });
    }
  });
  ...
  it('a thrown read still passes a same-repo PR — the hold is scoped to forks', async () => {
    ...
    // The verdict passes, and the reported switch still names the read failure
    // rather than the absent-row default — the two are different facts.
    expect(outcome).toEqual({
      action: 'pass',
      forkSwitch: {
        policy: ForkPolicy.enum.hold,
        source: EffectivePolicySource.enum['read-failure'],
        platformManaged: true,
      },
    });
  });
  ...
  it('still applies a stored policy in independent mode', async () => {
    ...
    expect(outcome).toEqual({
      action: 'pass',
      forkSwitch: {
        policy: ForkPolicy.enum.allow,
        source: EffectivePolicySource.enum.stored,
        // `independent` — nothing upstream owns this org's policy.
  ...
  it('does not gate a same-repo PR, whatever the stored policy', async () => {
    ...
    for (const value of ForkPolicy.options) {
      ...
      expect(outcome, `forkPolicy ${value}`).toEqual({
        action: 'pass',
        forkSwitch: {
          policy: value,
          source: EffectivePolicySource.enum.stored,
