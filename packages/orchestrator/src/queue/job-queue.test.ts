...
describe('JobQueue', () => {
  describe('enqueue', () => {
    ...
    it('stores the exact-label columns canonical, and the pattern sources verbatim', async () => {
      // The SQL `@>` prefilter compares these columns byte for byte, so the
      // write is where a job's exact labels enter the canonical domain. The
      // pattern columns are deliberately untouched: a regex source cannot be
      // lowercased, so its fold rides on the `i` flag the read forces.
      const db = createMockDb({ countResult: { count: 0 } });
      const queue = new JobQueue(db, { maxDepth: 100, defaultTimeoutMs: 600_000 });
      const pattern = { kind: 'regex' as const, source: '^kici:host:Box-', flags: '' };

      await queue.enqueue(
        makeJobInput({
          runsOnLabels: ['Linux', 'Docker'],
          excludeLabels: ['Spot'],
          runsOnPatterns: [pattern],
        }),
      );

      const arg = db._mocks.values.mock.calls[0][0];
      expect(arg.runs_on_labels).toBe(JSON.stringify(['linux', 'docker']));
      expect(arg.exclude_labels).toBe(JSON.stringify(['spot']));
      expect(arg.runs_on_patterns).toBe(JSON.stringify([pattern]));
    });
    ...
    it('round-trips runsOnPatterns/excludePatterns through enqueue + read', async () => {
      ...
      // string form back into LabelMatcher[], and canonicalizes each one on the
      // way out: the source is preserved byte for byte and the `i` flag carries
      // the case fold, which is why the write above stores the source verbatim.
      ...
      expect(job?.runsOnPatterns).toEqual([{ ...runsOnPatterns[0], flags: 'i' }]);
      expect(job?.excludePatterns).toEqual([{ ...excludePatterns[0], flags: 'i' }]);
    });
  ...
  describe('dequeueForLabels', () => {
    ...
    it('omits the mandatory-labels predicate when agentMandatoryLabels is empty', async () => {
      ...
      // exclude, the pin predicate (pinned_agent_id IS NULL [OR = agent]), the
      // stopped-run guard, and the pattern-free guard (runs_on_patterns = '[]'
      // AND exclude_patterns = '[]') = 7. The `runs_on_labels @> :gate`
      // containment must NOT be added when agentMandatoryLabels is empty (it
      // would be a no-op against an empty JSONB array but leaving it off keeps
      // the query cheap).
    ...
    it('adds the mandatory-labels containment predicate when gate is non-empty', async () => {
      ...
      // Fast-path baseline (7, incl. pin predicate, stopped-run guard and
      // pattern-free guard) + the gate predicate (1) = 8.
      expect(db._mocks.where).toHaveBeenCalledTimes(8);
    });
  ...
  describe('dequeueForLabels two-pass pattern drain', () => {
    ...
    it('matches a stored glob pattern against a differently-cased label', async () => {
      // The pattern columns hold regex SOURCES, which cannot be lowercased
      // without corrupting the pattern, so the fold rides on the `i` flag
      // `canonicalizeMatcher` forces at this read. `flags: ''` is what a lock
      // file compiled before the fold shipped stores.
      const row = makeDbRow({
        id: 'glob-job',
        runs_on_labels: [],
        runs_on_patterns: [{ kind: 'regex', source: '^(?:kici:host:Web\\-[^/]*?)$', flags: '' }],
      });
      const db = createMockDb({
        selectFirstRow: undefined,
        selectRows: [row],
      ...
      const claimed = await queue.dequeueForLabels(['kici:host:web-01']);
      expect(claimed).not.toBeNull();
      expect(claimed!.id).toBe('glob-job');
      expect(claimed!.runsOnPatterns[0].kind === 'regex' && claimed!.runsOnPatterns[0].flags).toBe(
        'i',
      );
    });

    it('excludes on a stored pattern that differs from the agent label only by case', async () => {
      // The forced flag must reach the exclusion arm too, or a job excluding
      // `-Canary$` drains onto exactly the agent it named.
      const row = makeDbRow({
        id: 'excluded-job',
        runs_on_labels: [],
        runs_on_patterns: [boxPattern],
        exclude_patterns: [{ kind: 'regex', source: '-Canary$', flags: '' }],
      });
      const db = createMockDb({
        selectFirstRow: undefined,
        selectRows: [row],
      ...
      expect(await queue.dequeueForLabels(['kici:host:box-canary'])).toBeNull();
    });

    it('folds the exact selector columns a row was written with before the fold', async () => {
      // The SQL `@>` prefilter is mocked away here, so this pins the JS half:
      // whatever case the row holds, the job a caller receives is canonical.
      const row = makeDbRow({
        id: 'cased-job',
        runs_on_labels: JSON.stringify(['Linux', 'Docker']),
        exclude_labels: JSON.stringify(['Spot']),
      });
      const db = createMockDb({ selectRows: [row] });
      const queue = new JobQueue(db, { maxDepth: 100, defaultTimeoutMs: 600_000 });

      const [job] = await queue.getPendingJobs();
      expect(job.runsOnLabels).toEqual(['linux', 'docker']);
      expect(job.excludeLabels).toEqual(['spot']);
    });
  ...
  describe('markDispatched', () => {
    ...
    it('stamps the owning coordinator and the dispatch clock', async () => {
      const db = createMockDb();
      const queue = new JobQueue(db, {
        maxDepth: 100,
        defaultTimeoutMs: 600_000,
      ...
      // `owner_instance_id` is what lets a cluster-wide sweep tell its own rows
      // from a live sibling's; `dispatched_at` is what stops the stale detector
      // measuring a job's queue wait as if it were a dispatch that went
      // unacknowledged.
      expect(db._mocks.set).toHaveBeenCalledWith(
        expect.objectContaining({
          owner_instance_id: 'coord-a',
          dispatched_at: expect.any(Date),
        }),
      );
    });

    it('leaves the owner NULL when the coordinator has no instance identity', async () => {
      const db = createMockDb();
      const queue = new JobQueue(db, { maxDepth: 100, defaultTimeoutMs: 600_000 });

      await queue.markDispatched('job-1', 'agent-7');

      // NULL reads as "unknown owner", never as "not mine".
      expect(db._mocks.set).toHaveBeenCalledWith(
        expect.objectContaining({ owner_instance_id: null }),
      );
    });
  });
  ...
  describe('getDepthBreakdown', () => {
    ...
    it('collapses case-only variants into one byLabel series', async () => {
      // A row an orchestrator wrote before migration 140 canonicalized the
      // table keeps the author's case. Counting it verbatim would split one
      // queue into a `Linux` series and a `linux` series, halving both the
      // gauge and the depth warning that reads it.
      const db = createMockDb({
        selectRows: [
          makeDbRow({
            id: 'a',
