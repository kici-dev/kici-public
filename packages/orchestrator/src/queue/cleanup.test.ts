...
import { canonicalizeLabels, canonicalizeMatcher, type LabelMatcher } from '@kici-dev/engine';
 ...
 *
 * Selectors are taken as plain strings and folded here, the same way
 * `rowToExpiredJobInfo` folds what it reads out of the row.
 */
function expiredJob(
  over: Pick<ExpiredJobInfo, 'id' | 'runId' | 'jobName'> & {
  ...
  return {
    id: over.id,
    runId: over.runId,
    jobName: over.jobName,
    lastProvisioningError: over.lastProvisioningError ?? null,
    runsOnLabels: canonicalizeLabels(over.runsOnLabels ?? ['default']),
