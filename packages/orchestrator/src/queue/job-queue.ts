...
/**
 * SQL fragment: the coordinator that owns `<column>` is NOT alive.
 *
 * "Alive" means a `cluster_instances` row inside the grace window. A
 * currently-connected peer is OR-ed in by the caller as a corroborating
 * short-circuit — it can only ever make a coordinator read *more* alive, so it
 * cannot cause the failure this predicate exists to prevent.
 *
 * @param ownerColumn a fully qualified column reference (`dispatch_queue.owner_instance_id`)
 * @param graceMs how stale a heartbeat may be and still read as live
 */
function ownerNotLiveSql(ownerColumn: string, graceMs: number) {
  return sql<SqlBool>`NOT EXISTS (
    SELECT 1 FROM cluster_instances ci
     WHERE ci.instance_id = ${sql.ref(ownerColumn)}
       AND ci.last_heartbeat_at > NOW() - ${sql.lit(Math.max(0, Math.round(graceMs)))} * INTERVAL '1 millisecond'
  )`;
}
...
const OWNER_COLUMN = 'dispatch_queue.owner_instance_id';

/**
 * SQL fragment: the row's run has not stopped.
 *
 * True when the run's status is neither terminal nor `cancelling`, and also
 * true when no `execution_runs` row exists at all. The missing-row case reads
 * as "not stopped" deliberately: a queue row whose run was never recorded is
 * the expiry sweep's problem, and failing closed here would strand it in the
 * queue forever instead.
 */
function notTerminalRunSql() {
  const stopped = [...TERMINAL_RUN_STATES, ExecutionRunStatus.enum.cancelling];
  return sql<SqlBool>`NOT EXISTS (
    SELECT 1 FROM execution_runs er
     WHERE er.run_id = dispatch_queue.run_id::uuid
       AND er.status IN (${sql.join(stopped.map((v) => sql`${v}`))})
  )`;
}
...
function ownerIsLiveSql(opts: OwnershipPredicateOptions) {
  ...
  const liveIds = [
    ...(opts.instanceId === undefined ? [] : [opts.instanceId]),
    ...(opts.livePeerIds ?? []),
  ];
  const heartbeatLive = sql<SqlBool>`NOT (${ownerNotLiveSql(column, opts.graceMs)})`;
  if (liveIds.length === 0) return heartbeatLive;
  return sql<SqlBool>`(${sql.ref(column)} IN (${sql.join(liveIds.map((id) => sql`${id}`))}) OR ${heartbeatLive})`;
}
 ...
 */
export function recoverableByOwner(opts: OwnershipPredicateOptions) {
  const column = opts.ownerColumn ?? OWNER_COLUMN;
  const self =
    opts.instanceId === undefined
      ? sql<SqlBool>`FALSE`
      : sql<SqlBool>`${sql.ref(column)} = ${opts.instanceId}`;
  return sql<SqlBool>`(
    ${sql.ref(column)} IS NOT NULL
    AND (${self} OR ${ownerNotLiveSql(column, opts.graceMs)})
  )`;
}
 ...
 * A NULL owner passes.
 */
export function writableByOwner(opts: OwnershipPredicateOptions) {
  const column = opts.ownerColumn ?? OWNER_COLUMN;
  return sql<SqlBool>`(
    ${sql.ref(column)} IS NULL
    OR NOT (${ownerIsLiveSql(opts)})
    OR ${
      opts.instanceId === undefined
        ? sql<SqlBool>`FALSE`
        : sql<SqlBool>`${sql.ref(column)} = ${opts.instanceId}`
...
function rowToExpiredJobInfo(r: ExpiryRowShape): ExpiredJobInfo {
  return {
    ...
    // Folded on read for the same reason the dispatch path folds: a row written
    // before labels were canonicalized carries the case its author typed, and
    // the expiry verdict is decided by comparing it against canonical labels.
    runsOnLabels: canonicalizeLabels(
      parseSelectorColumnForExpiry<string>(r.runs_on_labels, 'runs_on_labels'),
    ),
    runsOnPatterns: parseSelectorColumnForExpiry<LabelMatcher>(
      r.runs_on_patterns,
      'runs_on_patterns',
    ).map(canonicalizeMatcher),
    excludeLabels: canonicalizeLabels(
      parseSelectorColumnForExpiry<string>(r.exclude_labels, 'exclude_labels'),
    ),
...
export class JobQueue {
  ...
  private readonly ownershipGraceMs: number;
  ...
  constructor(
    ...
    this.ownershipGraceMs =
      options.ownershipGraceMs ?? instanceLivenessGraceMs(DEFAULT_RECOVERY_GRACE_MS);
  }

  /** Ownership predicate inputs, in the shape both predicate builders take. */
  private ownershipOptions(): OwnershipPredicateOptions {
    return { instanceId: this.instanceId, graceMs: this.ownershipGraceMs };
  }
  ...
  async enqueue(job: QueuedJobInput): Promise<string> {
    ...
    await this.db
      ...
      .values({
        ...
        // The exact-label columns are stored canonical: the SQL `@>` prefilter
        // below compares them byte for byte against an agent's canonical labels.
        ...
        exclude_labels: JSON.stringify(canonicalizeLabels(job.excludeLabels ?? [])),
  ...
  private claimTransition(agentId?: string): {
    ...
    return {
      ...
      last_provisioning_error: null,
      // The coordinator that claimed the row is the one arming its timers, so
      // it is the only one a cluster-wide sweep may let act on it.
      owner_instance_id: this.instanceId ?? null,
      // When the row was actually handed out, as distinct from `created_at`
      // (when it was enqueued). The stale detector reads this: a job that sat
      // behind a busy fleet for three minutes and was dispatched thirty seconds
      // ago has not gone unacknowledged for three minutes.
  ...
  private drainBaseQuery(agentLabels: string[], agentMandatoryLabels: string[], agentId?: string) {
    // The `@>` containment and the `= ANY(...)` exclusion are byte comparisons
    // Postgres performs, so the agent's side is folded here to meet a stored
    // column the write folds. Without this the prefilter is the one comparison
    // in the dispatch path that stays case-sensitive, and a job it drops never
    // reaches the JS post-filter that would have matched it.
    const canonicalAgentLabels: string[] = canonicalizeLabels(agentLabels);
    const canonicalGate: string[] = canonicalizeLabels(agentMandatoryLabels);
    const agentLabelsJson = JSON.stringify(canonicalAgentLabels);
    const mandatoryLabelsJson = JSON.stringify(canonicalGate);
    let query = this.db
      ...
      .where(
        sql<SqlBool>`NOT EXISTS (
          ...
          WHERE e.value = ANY(${sql.val(canonicalAgentLabels)}::text[])
        )`,
      );
    ...
    // Never hand out work for a run that is over. `cancelByRunId` expires the
    // queue rows a cancel can see, but a row can arrive here afterwards — a
    // requeue racing the cancel, a job enqueued by a downstream scheduler — and
    // dispatching it starts a real agent, with real side effects, for a run
  ...
  async dequeueByPinnedAgent(agentId: string, agentLabels?: string[]): Promise<QueuedJob | null> {
    ...
    if (!row) return null;
    const job = this.rowToQueuedJob(row);
    if (agentLabels && !jobPatternsSatisfiedBy(job, canonicalizeLabelSet(agentLabels))) return null;
  ...
  async dequeueById(
    ...
    // Same reason `drainBaseQuery` folds: these are byte comparisons Postgres
    // performs against a stored column the write folds.
    const canonicalAgentLabels: string[] = canonicalizeLabels(agentLabels);
    const canonicalGate: string[] = canonicalizeLabels(agentMandatoryLabels);
    const agentLabelsJson = JSON.stringify(canonicalAgentLabels);
    const mandatoryLabelsJson = JSON.stringify(canonicalGate);
    ...
    const job = this.rowToQueuedJob(row);
    if (!jobPatternsSatisfiedBy(job, canonicalizeLabelSet(agentLabels))) return null;
  ...
  async insertDispatched(
    ...
    const inserted = await this.db
      ...
      .values({
        ...
        pinned_agent_id: job.pinnedAgentId ?? null,
        agent_id: agentId,
        // The row is born `dispatched`, so it never passes through
        // `markDispatched` and these two are the only chance to stamp them.
        // Without the owner, a cluster-wide sweep reads the row as
        // unknown-owner: startup recovery spares it, so a directly-dispatched
        // job whose coordinator died is never recovered by the survivor.
        owner_instance_id: this.instanceId ?? null,
        dispatched_at: new Date(),
      })
      .onConflict((oc) => oc.column('id').doNothing())
  ...
  async cancelByRunId(runId: string): Promise<number> {
    const result = await this.db
      ...
      .where((eb) =>
        eb.or([
          eb('status', '=', DispatchQueueStatus.Pending),
          // A dispatch that was never acknowledged. Expiring only `pending`
          // rows let this one outlive the cancel: the ack timed out, the requeue
          // flipped it back to `pending`, and the next drain dispatched it for a
          // run that already read `cancelled`.
          eb.and([
            eb('status', '=', DispatchQueueStatus.Dispatched),
            eb('ack_deadline', 'is not', null),
          ]),
          // A recovering row is waiting for an agent to reconnect and reclaim
          // it. The run is cancelled, so there is nothing left to reclaim.
          eb('status', '=', DispatchQueueStatus.Recovering),
        ]),
      )
      .executeTakeFirst();
    ...
    for (const row of rows) {
      ...
      const labels = Array.isArray(row.runs_on_labels)
        ...
        : typeof row.runs_on_labels === 'string'
          ...
          : [];
      // Fold here too, even though migration 140 canonicalized every stored row
      // and `enqueue` canonicalizes every new one. This is the last read of
      // `runs_on_labels` that keys anything by the label text, and a row written
      // by an orchestrator that has not yet applied 140 would otherwise split
      // one queue into a `Linux` series and a `linux` series — halving both the
      // gauge and the operator depth warning that reads it.
   ...
   * member.
   */
  async getDispatchedJobOwnersByRunId(
    runId: string,
  ): Promise<Array<{ jobId: string; agentId: string | null; ownerInstanceId: string | null }>> {
    const rows = await this.db
      .selectFrom('dispatch_queue')
      .select(['id', 'agent_id', 'owner_instance_id'])
      .where('run_id', '=', runId)
      .where('status', 'in', [DispatchQueueStatus.Dispatched, DispatchQueueStatus.Recovering])
      .execute();
    return rows.map((r) => ({
      ...
      ownerInstanceId: r.owner_instance_id ?? null,
    }));
  }
   ...
   */
  async markRecovering(jobId: string, deadline?: Date, agentId?: string): Promise<boolean> {
    const setValues: {
      status: DispatchQueueStatus;
    ...
    const result = await this.db
      ...
      .where(writableByOwner(this.ownershipOptions()))
      .executeTakeFirst();
   ...
   */
  async getOrphanedDispatchedJobs(): Promise<
    Array<{ id: string; runId: string; agentId: string | null }>
  > {
    const rows = await this.db
      .selectFrom('dispatch_queue')
      .select(['id', 'run_id', 'agent_id'])
      .where('status', '=', DispatchQueueStatus.Dispatched)
      .where(recoverableByOwner(this.ownershipOptions()))
      .execute();
    return rows.map((r) => ({ id: r.id, runId: r.run_id, agentId: r.agent_id ?? null }));
  }
   ...
   */
  async countSparedDispatchedJobs(): Promise<{ ownedElsewhere: number; unknownOwner: number }> {
    const rows = await this.db
      .selectFrom('dispatch_queue')
      .select(['id', 'owner_instance_id'])
      .where('status', '=', DispatchQueueStatus.Dispatched)
      .where(sql<SqlBool>`NOT (${recoverableByOwner(this.ownershipOptions())})`)
      .execute();
    let unknownOwner = 0;
    for (const row of rows) if (row.owner_instance_id === null) unknownOwner++;
    return { ownedElsewhere: rows.length - unknownOwner, unknownOwner };
  }
  ...
  > {
    const rows = await this.db
      ...
      // Scoped to owned-or-orphaned rows: re-arming a timer for a dispatch a
      // live sibling is watching gives that dispatch two independent deadlines,
      // and the one this coordinator armed knows nothing about the ack the
      // sibling received.
      .where(recoverableByOwner(this.ownershipOptions()))
      .execute();
    return rows
  ...
  > {
    const rows = await this.db
      ...
      // Same scoping as the ack re-arm: a recovery a live sibling is already
      // timing does not need a second timer here, and the expiry this one would
      // fire cannot see the reclaim the sibling handled.
      .where(recoverableByOwner(this.ownershipOptions()))
      .execute();
    return rows.map((r) => ({
      id: r.id,
  ...
  async requeue(jobId: string): Promise<number | null> {
    const row = await this.db
      ...
      .set({
        ...
        ack_agent_id: null,
        agent_id: null,
        // The row is going back on the queue, so nobody owns it and nothing has
        // been dispatched: leaving either stamped would make a sweep act on a
        // pending row and would leave the stale detector measuring a dispatch
        // that was undone.
      ...
      .where('id', '=', jobId)
      .where('status', '=', DispatchQueueStatus.Dispatched)
      // A requeue for a run that has stopped puts the row back on the queue for
      // the next drain to dispatch — a fresh agent running a job for a run that
      // already reads `cancelled`. Reporting "not requeued" is the honest
      // answer: the cancel already terminalized the row.
  ...
  async requeueIfAwaitingAck(
    ...
    const guarded = this.db
      ...
      .where('status', '=', DispatchQueueStatus.Dispatched)
      .where('ack_deadline', 'is not', null)
      // Same reason as `requeue`: a stopped run must not get its job back.
      .where(notTerminalRunSql());
    const row = await (
      expectedAgentId === null
        ? guarded.where('ack_agent_id', 'is', null)
        : guarded.where('ack_agent_id', '=', expectedAgentId)
    )
      .returning('dispatch_attempts')
      .executeTakeFirst();
    this.depthCache = null;
  ...
  async markDispatchedIfRecovering(jobId: string, agentId: string): Promise<boolean> {
    const result = await this.db
      ...
      .set({
        ...
        ack_agent_id: null,
        agent_id: agentId,
        // The reclaim hands the job out again, from THIS coordinator: it is the
        // one holding the agent's socket and arming the row's timers. Leaving
        // the previous owner stamped would point every ownership question at a
        // coordinator that is very likely the dead one the recovery is undoing.
        owner_instance_id: this.instanceId ?? null,
        dispatched_at: new Date(),
      })
      .where('id', '=', jobId)
  ...
  private rowToQueuedJob(row: DispatchQueueItem): QueuedJob {
    ...
    return {
      ...
      // Every selector below is folded here, at the one read every dispatch
      // path shares. A row enqueued before labels were canonicalized still
      // carries the case its author typed, and the pattern columns hold regex
      // sources that cannot be lowercased — `canonicalizeMatcher` carries their
      // fold as the `i` flag instead.
      runsOnLabels: canonicalizeLabels(
        Array.isArray(row.runs_on_labels)
          ? row.runs_on_labels
          : (JSON.parse(row.runs_on_labels) as string[]),
      ),
      jobConfig,
      ...
      excludeLabels: canonicalizeLabels(
        typeof row.exclude_labels === 'string'
          ? (JSON.parse(row.exclude_labels) as string[])
          : Array.isArray(row.exclude_labels)
            ? row.exclude_labels
            : [],
      ),
      runsOnPatterns: parseMatcherColumn(row.runs_on_patterns).map(canonicalizeMatcher),
      excludePatterns: parseMatcherColumn(row.exclude_patterns).map(canonicalizeMatcher),
 ...
 */
function jobPatternsSatisfiedBy(job: QueuedJob, labels: ReadonlySet<CanonicalLabel>): boolean {
