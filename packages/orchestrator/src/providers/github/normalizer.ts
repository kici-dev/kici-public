...
/**
 * `owner/repo` of the pull-request HEAD, from the same payload shape
 * `isForkPullRequest` compares. Undefined when the payload carries no head
 * repository — a deleted fork, or a trimmed payload — which every consumer
 * renders as unresolved rather than guessing the base repository.
 */
function headRepository(pr: PullRequestShape | undefined): string | undefined {
