...
export class UniversalGitWebhookNormalizer implements WebhookNormalizer {
  ...
  normalizeEvent(
    ...
    return {
      ...
      isForkPR,
      // The same value `isForkPR` compares. It is the run row's `head_repository`
      // and the input that decides whether the fork question resolved at all:
      // undefined here means the payload named no head repository, so
      // `resolveRunEventContext` records the fork state as unresolved rather
      // than as the `false` the comparison above collapses to.
