...
describe('LocalLockFileFetcher', () => {
  ...
  it('declares itself uncacheable, because ref carries nothing about the file on disk', () => {
    // fails-when: the flag is dropped or flipped — LockFileCache would then pin
    // the first read under `local:.:HEAD` for the whole TTL, and every later
    // `kici run --local` dispatch would run the previous run's step bodies.
    const fetcher = new LocalLockFileFetcher(testDir);
    expect(fetcher.cacheable).toBe(false);
  });
