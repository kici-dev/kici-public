...
import { checkDeferredCapture } from './provenance/verify-deferred-capture.js';
...
export function createApp(deps: AppDependencies) {
  ...
  if (deps.hostRosterStore) {
    ...
    // present — independent mode without secrets does not expose it.
  ...
  if (deps.secretResolver) {
    const gitCredentialBroker = new GitCredentialBroker({
      secretResolver: deps.secretResolver,
      // The dispatch pipeline's `contextStore` is optional (an independent
      // orchestrator without `independentSecrets` wires none), but the gate is
      // not: a broker with no store would have to resolve qualified references
      // ungated. Build one over the same database instead — with no `contexts`
      // rows every qualified reference is refused as naming a context that does
      // not exist, which is the correct fail-closed reading, while a request
      // with no ref keeps taking the source-credential path untouched.
      contextStore: deps.contextStore ?? new ContextStore(deps.db),
  ...
  const tryDispatchNextQueued = deps.concurrencyQueueManager
    ...
    upgradeWebSocket(() =>
      createAgentWsHandler({
        ...
        onProvenanceDefer: async (record) => {
          ...
          //
          // Cross-check FIRST, and fail closed. The retrier mints a real,
          // server-signed token committing to whatever lands here, so a
          // statement the orchestrator has not checked against its own run row
          // must never reach the outbox. A refusal drops the attestation and
          // leaves the job green — a degradation, where storing it would let a
          // compromised job have the orchestrator vouch for a build that never
          // happened.
          const verdict = await checkDeferredCapture({
            db: deps.db,
            record,
            orchestratorId: deps.config.instanceId,
          });
          if (!verdict.ok) {
            logger.warn('Deferred attestation refused at capture', {
              runId: record.runId,
              jobId: record.jobId,
              reason: verdict.reason,
              detail: verdict.detail,
              hint: 'the agent may predate the canonical build context sent with a job dispatch — upgrade the agent',
            });
            return;
          }
          await new PendingAttestationsRepo(deps.db).insert({
            ...
            // The RECOMPUTED hash, never the agent-supplied one: the later
            // mint stamps this into a server-signed claim.
            statementHash: verdict.statementHash,
            originKind: record.originKind,
          });
        },
        onConcurrencyReport: deps.concurrencyQueueManager
          ? async (agentId, msg) => {
              ...
              if (concurrencyConfig.cancelInProgress) {
                ...
                const activeRuns = await queueManager.getActiveRuns(msg.group, routingKey);
                ...
                for (const oldRunId of activeRuns) {
                  ...
                  await queueManager.releaseSlot(msg.group, routingKey, oldRunId);
                  supersededRunIds.push(oldRunId);
                }
                // Acquire slot for current run.
                await queueManager.acquireSlot(msg.group, routingKey, msg.runId, msg.jobId, {
              ...
              // Queue mode: try to acquire slot. The DB is the arbiter, so
              // the cap holds across a restart and across coordinators.
              const acquired = await queueManager.acquireSlot(
                msg.group,
                routingKey,
                msg.runId,
                msg.jobId,
  ...
  const admitIngest = async (
    ...
    // The relay ingress logs its own shed; this is the same line for direct
    // ingress, so a shed on either path is explicable from the logs rather than
    // only from a counter. Without it an operator watching
    // kici_orch_ingest_shed_total step up on this path has the count and
    // nothing that names the delivery.
    logger.warn('Direct webhook shed by ingest admission control', {
   ...
   *
   * `preAdmitted` skips the admission step for a caller that already holds a
   * grant for this delivery — the overflow replayer, which reserves its slot
   * before it claims the row. It is not a bypass: the slot is held by the
   * caller for the whole of this call, and admitting a second time here would
   * charge the delivery an attempt for a capacity refusal it already survived.
   */
  const runWebhookIngest = async (
    info: WebhookInfo,
    opts?: { captureOnShed?: boolean; preAdmitted?: boolean },
  ): Promise<WebhookIngestOutcome> => {
    const reqId = randomUUID();
    return requestContext.run({ requestId: reqId, routingKey: info.routingKey }, async () => {
      const admit = opts?.preAdmitted ? undefined : await admitIngest(info, opts);
  ...
  // Direct-origin overflow replay: re-inject a captured HTTP delivery through
  // the same ingest closure, under the admission grant the replayer already
  // holds for it (`preAdmitted`). This one stays inline on purpose — the
  // replayer decides the row's next state from the outcome, so it must await
  // the pipeline rather than an acknowledgement.
  const reinjectDirect = (d: OverflowDelivery): Promise<WebhookIngestOutcome> =>
    runWebhookIngest(overflowDeliveryToInfo(d), { captureOnShed: false, preAdmitted: true });
  ...
  if (deps.genericSourceManager && deps.trustStore && deps.adminDeps) {
    app.route(
      ...
      createAdminEventRoutes({
        ...
        clusterSettings: deps.clusterSettings,
        // Read lazily so a reconnect that learns the org id is picked up
        // without re-mounting the routes; `undefined` in independent mode.
  ...
  // masterKeys + auditLogger are required only by the ?reveal=true variant
  ...
  if (deps.adminDeps) {
    app.route(
      ...
      createAdminRunRoutes({
        ...
        masterKeys: deps.masterKeys ?? null,
