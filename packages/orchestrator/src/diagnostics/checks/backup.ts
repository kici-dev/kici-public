  ...
  try {
    ...
    if (!row) {
      return {
        ...
        status: 'fail',
        message:
          'No orchestrator DB backup has ever been recorded — run `kici-admin db backup`, ' +
          'then schedule it with `kici-admin db backup --install-timer`',
        durationMs: Date.now() - start,
      };
    }
    ...
    if (ageHours > thresholdHours) {
      return {
        ...
        status: 'warn',
        message:
          `Last backup was ${ageLabel} ago (> ${thresholdHours}h) — ` +
          'schedule backups with `kici-admin db backup --install-timer`',
        details,
        durationMs: Date.now() - start,
      };
    }
