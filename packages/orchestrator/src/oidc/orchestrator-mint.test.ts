...
/**
 * A tiny fake Kysely returning canned rows, projected to the columns the caller
 * selected. `execution_runs` → the run; a `sources` row supplies the org
 * (resolveOrgId reads customer_id by routing_key); `execution_jobs` → the job.
 * Unmapped tables (generic_webhook_sources, remote_sources) return undefined so
 * resolveOrgId falls through in order.
 *
 * The projection is what makes the production select lists load-bearing: a
 * column the query forgets to name reads `undefined` here exactly as it would
 * from Postgres, rather than arriving anyway because the canned row carries it.
 */
function fakeDb(rows: {
  ...
  const chain = (row: unknown): unknown => {
    ...
    const c: Record<string, unknown> = {
      select: (cols: string | readonly string[]) => {
        columns = typeof cols === 'string' ? [cols] : cols;
        return c;
      },
      where: () => c,
      executeTakeFirst: async () => {
        if (row === undefined || columns === undefined) return row;
        const source = row as Record<string, unknown>;
        return Object.fromEntries(columns.map((col) => [col, source[col]]));
      },
    };
    return c;
  };
...
describe('mintOrchestratorIdToken', () => {
  ...
  it('the fake drops a column the query did not select', async () => {
    // Positive control for the two tests below: they only prove the production
    // select list names `subject_trigger_event` if an unnamed column really is
    // absent from what the query returns.
    const db = fakeDb({ run: { ...RUN, subject_trigger_event: 'pull_request:opened' } });
    const projected = await db
      .selectFrom('execution_runs')
      .select(['run_id'])
      .where('run_id', '=', 'run-1')
      .executeTakeFirst();
    expect(projected).toEqual({ run_id: 'run-1' });
  });

  /**
   * The inherited event reaches the token through two hops the claim builders
   * cannot see: the query's own select list, and the field-by-field claim
   * object built from the returned row. Both are exercised here because the
   * fake projects to the selected columns — omit `subject_trigger_event` from
   * either hop and the subject falls back to the branch shape below.
   */
  it('carries the inherited subject event from the run row into the token', async () => {
    const { signer } = await DbSigner.generate(KEY);
    const db = fakeDb({
      run: { ...RUN, trigger_event: 'rerun', subject_trigger_event: 'pull_request:opened' },
      job: { run_id: 'run-1', job_id: 'job-1', status: 'running' },
      sourceCustomerId: 'org-acme',
    });
    ...
    // The claim still reports what started THIS run.
    expect(claims.event_name).toBe('rerun');
  });

  it('keeps the branch subject for a re-run row that inherited nothing', async () => {
    const { signer } = await DbSigner.generate(KEY);
    const db = fakeDb({
      run: { ...RUN, trigger_event: 'rerun', subject_trigger_event: null },
      job: { run_id: 'run-1', job_id: 'job-1', status: 'running' },
      sourceCustomerId: 'org-acme',
    });
    const { claims } = await mintOrchestratorIdToken(
    ...
    expect(claims.sub).toBe('repo:acme/app:ref:refs/heads/main:workflow:build');
  });
