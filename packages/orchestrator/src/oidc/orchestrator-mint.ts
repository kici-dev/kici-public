...
export async function mintOrchestratorIdToken(
  ...
  const run = await deps.db
    ...
    .select([
      ...
      'provider',
      'local_working_tree',
      // The event-context claims. `ref` above is the branch the run PRESENTS —
      // the BASE branch for a pull request — so without these a fork PR and a
      // trusted push to that same branch mint an identical identity.
      'trigger_event',
      // The event the SUBJECT is derived from. A re-run records
      // `trigger_event: 'rerun'`, which is not pull-request family, so without
      // this column a re-run of a pull request presents the branch-shaped
      // identity a trusted push to the base branch presents.
  ...
  const claims = buildIdTokenClaims(
    {
      ...
      subject_trigger_event: run.subject_trigger_event,
      head_ref: run.head_ref,
      head_repository: run.head_repository,
      is_fork: run.is_fork,
      trust_tier: run.trust_tier,
      trigger_actor_username: run.trigger_actor_username,
    },
    {
      run_id: job.run_id,
      job_id: job.job_id,
