 ...
 */
export function unwrapPrivateJwk(encrypted: string, secretKey: string, oldSecretKey?: string): JWK {
  const value: EncryptedValue = { data: encrypted, keyVersion: 1 };
  try {
    return JSON.parse(decrypt(value, deriveKey(secretKey), SIGNING_KEY_AAD)) as JWK;
  } catch (primaryErr) {
    if (!oldSecretKey) throw primaryErr;
  }
  return JSON.parse(decrypt(value, deriveKey(oldSecretKey), SIGNING_KEY_AAD)) as JWK;
}
 ...
 */
export function unwrapPrivateJwkWithKeyAge(
  encrypted: string,
  secretKey: string,
  oldSecretKey?: string,
): { privateJwk: JWK; usedOldKey: boolean } {
  const value: EncryptedValue = { data: encrypted, keyVersion: 1 };
  try {
    return {
      privateJwk: JSON.parse(decrypt(value, deriveKey(secretKey), SIGNING_KEY_AAD)) as JWK,
      usedOldKey: false,
    };
  } catch (primaryErr) {
    if (!oldSecretKey) throw primaryErr;
  ...
  return {
    privateJwk: JSON.parse(decrypt(value, deriveKey(oldSecretKey), SIGNING_KEY_AAD)) as JWK,
    usedOldKey: true,
  };
}
...
export class DbSigner implements Signer {
  ...
  static async fromPrivateJwk(privateJwk: JWK): Promise<DbSigner> {
  ...
  static async fromEncrypted(
    ...
    const privateJwk = unwrapPrivateJwk(encryptedPrivateJwk, secretKey, oldSecretKey);
