...
describe('resolveEffectivePolicy', () => {
  ...
  it('returns the stored policy whatever the mode', () => {
    for (const mode of OrchestratorMode.options) {
      expect(resolveEffectivePolicy(stored, mode)).toEqual({
        policy: ALLOW_ALL,
  ...
  it('returns the fail-closed policy in every mode with no stored row', () => {
    ...
    for (const mode of OrchestratorMode.options) {
      expect(resolveEffectivePolicy(null, mode)).toEqual({
        policy: FAIL_CLOSED_POLICY,
        source: EffectivePolicySource.enum.unconfigured,
      });
    }
  });
...
describe('forkDropExplanation', () => {
  const IGNORE = ForkPolicy.enum.ignore;

  it('says what happened, what did not, and that the contributor sees none of it', () => {
    const text = forkDropExplanation({
      policy: IGNORE,
      source: EffectivePolicySource.enum.stored,
      platformManaged: true,
    });
    // The row is the only trace the drop leaves, so all three have to be in it.
    expect(text).toContain(`forkPolicy=${IGNORE}`);
    expect(text).toContain('No run was created');
    expect(text).toContain('visible to the contributor');
  });

  it('distinguishes a policy somebody chose from one nobody chose', () => {
    const chosen = forkDropExplanation({
      policy: IGNORE,
      source: EffectivePolicySource.enum.stored,
      platformManaged: true,
    });
    const unchosen = forkDropExplanation({
      policy: IGNORE,
      source: EffectivePolicySource.enum.unconfigured,
      platformManaged: true,
    });

    // The whole point of carrying the source: an operator who never opened the
    // settings and one who deliberately set `ignore` need different next steps,
    // and the identical `ignore` verdict cannot tell them apart on its own.
    expect(unchosen).not.toBe(chosen);
    expect(unchosen).toContain('no stored trust policy');
    expect(unchosen).toContain('Settings > CI trust');
    // And the converse, without which a build that appended the remedy to both
    // would pass every assertion above.
    expect(chosen).not.toContain('Settings > CI trust');
  });

  it('names the remedy the deployment actually has', () => {
    // An independent orchestrator has no Platform, so it has no dashboard to
    // send its operator to — and that operator is the likeliest reader of this
    // row, because they own the fork switch themselves.
    const independent = forkDropExplanation({
      policy: IGNORE,
      source: EffectivePolicySource.enum.unconfigured,
    ...
    // And the converse: a Platform-attached orchestrator must NOT name the
    // local verb, which its own admin route answers with a 409.
    const managed = forkDropExplanation({
      policy: IGNORE,
      source: EffectivePolicySource.enum.unconfigured,
      platformManaged: true,
    });
    expect(managed).not.toContain('kici-admin trust-policy set');
  });

  it('offers no remedy at all for a switch the org chose, in either deployment', () => {
    // A stored `ignore` is a decision, not an omission — telling its author how
    // to set a policy they already set would misread the row.
    for (const platformManaged of [true, false]) {
      const text = forkDropExplanation({
        policy: IGNORE,
        source: EffectivePolicySource.enum.stored,
        platformManaged,
      });
      expect(text, `platformManaged ${platformManaged}`).not.toContain('to change this');
    }
  });
