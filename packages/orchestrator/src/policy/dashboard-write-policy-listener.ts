...
export class DashboardWritePolicyChangeListener {
  ...
  async start(): Promise<void> {
    this.listener = new NotifyListener({
      pool: this.opts.pool,
      channel: DASHBOARD_WRITE_POLICY_CHANNEL,
      onNotification: (msg) => {
      ...
      // Drop the whole cached map: the next read repopulates it from the
      // database, which is the same degradation the drain's error branch
      // already relies on.
      onReconnect: () => invalidateDashboardWritePolicyCache(),
    });
