...
export class LockFileCache {
  ...
  async get(
    ...
    // A fetcher whose store can change under a fixed ref reads through: the
    // key above carries no information about what the file holds now, so a
    // stored entry would pin the first read for the whole TTL.
    if (fetcher.cacheable === false) {
      return this.fetchAndValidate(fetcher, repoIdentifier, ref, credentials, undefined);
    }
  ...
  private async fetchAndValidate(
    ...
    ref: string,
    credentials: unknown,
    /** Where to store the validated lock; `undefined` reads through. */
    ...
    if (cacheKey !== undefined) this.cache.set(cacheKey, lockFile);
    return lockFile;
  }
