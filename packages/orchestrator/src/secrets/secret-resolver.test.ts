...
describe('SecretResolver', () => {
  ...
  // ── resolveNamedInternal (source-scoped direct lookup) ────────────────

  describe('resolveNamedInternal', () => {
    it('returns a named secret by (scope, key) from the default backend', async () => {
      ...
      const resolver = createResolver();
      const result = await resolver.resolveNamedInternal(
        'org-1',
        '__source__/src-123',
        'forgejo-pat',
      );
    ...
    it('throws when explicit backend is missing', async () => {
      const resolver = createResolver();
      await expect(
        resolver.resolveNamedInternal('org-1', '__source__/src-123', 'k', {
    ...
    it('writes a resolve_named audit entry on success', async () => {
      ...
      const resolver = createResolver();
      await resolver.resolveNamedInternal('org-1', '__source__/src-123', 'forgejo-pat');
