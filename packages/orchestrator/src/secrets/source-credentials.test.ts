 ...
 *
 * Exercises the thin glue around SecretResolver.resolveNamedInternal to ensure:
...
describe('resolveSourceCredential', () => {
  it('delegates to resolveNamedInternal with the canonical scope', async () => {
    const resolveNamedInternal = vi.fn().mockResolvedValue('hunter2');
    const resolver = mockResolver({ resolveNamedInternal });
    ...
    expect(resolveNamedInternal).toHaveBeenCalledWith(
      'org-1',
      '__source__/src-123',
      'forgejo-pat',
  ...
  it('passes the credentialRef.store through when set', async () => {
    const resolveNamedInternal = vi.fn().mockResolvedValue('vault-value');
    const resolver = mockResolver({ resolveNamedInternal });
  ...
  it('returns { ok:false, reason:"not_found" } when the key is missing', async () => {
    const resolveNamedInternal = vi.fn().mockResolvedValue(null);
    const resolver = mockResolver({ resolveNamedInternal });
  ...
  it('rethrows unexpected errors that are not backend-missing', async () => {
    const resolveNamedInternal = vi.fn().mockRejectedValue(new Error('unrelated failure'));
    const resolver = mockResolver({ resolveNamedInternal });
  ...
  it('forwards runId / jobId to resolveNamedInternal for audit correlation', async () => {
    const resolveNamedInternal = vi.fn().mockResolvedValue('v');
    const resolver = mockResolver({ resolveNamedInternal });
