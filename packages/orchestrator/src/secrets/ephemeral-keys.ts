...
export const PRIVATE_KEY_AAD = 'ephemeral-private-key';
...
export function decryptPrivateKey(
  ...
  try {
    return Buffer.from(decrypt(encryptedValue, deriveKey(secretKey), PRIVATE_KEY_AAD), 'base64');
  } catch (primaryErr) {
    if (!oldSecretKey) throw primaryErr;
  }
  return Buffer.from(decrypt(encryptedValue, deriveKey(oldSecretKey), PRIVATE_KEY_AAD), 'base64');
}
