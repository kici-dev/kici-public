...
function registerLocalBundle(row: GenericWebhookSource, deps: RegisterSourceBundleDeps): void {
  ...
  // skip; they will not advertise the routing key
 ...
 */
export async function registerAllGenericSourceBundles(
  deps: RegisterAllGenericSourceBundlesDeps,
): Promise<void> {
  const { sourceManager, ...rowDeps } = deps;

  try {
    for (const row of await sourceManager.listLocalSources()) {
      registerProviderBundleForSource(row, rowDeps);
    }
  } catch (err) {
    logger.warn('Failed to register local provider bundles', {
      error: toErrorMessage(err),
    });
  }

  try {
    for (const row of await sourceManager.listUniversalGitSources()) {
      registerProviderBundleForSource(row, rowDeps);
    }
  } catch (err) {
    universalGitRegistrationErrorsTotal.add(1, { reason: 'registration' });
    logger.warn('Failed to enumerate universal-git sources', {
      error: toErrorMessage(err),
    });
  }
}
