 ...
 *     A caller recording that a delivery was merely SEEN (the shed breadcrumb)
 *     passes `onlyIfAbsent` so the conflict leaves the existing row alone.
 */
  ...
  async record(
    ...
    try {
      await this.db
        ...
        .values(row)
        .onConflict((oc) => {
          const conflict = oc.columns(['org_id', 'delivery_id']);
          // A breadcrumb records that the delivery was seen, not how it ended,
          // so it must never overwrite an outcome another writer already
          // reached for the same delivery.
          if (opts?.onlyIfAbsent) return conflict.doNothing();
