...
} from './register-source-bundle.js';
...
export class GenericSourcesChangeListener {
  ...
  async start(): Promise<void> {
    this.listener = new NotifyListener({
      pool: this.opts.pool,
      channel: 'generic_sources_change',
      onNotification: (msg) => {
      ...
      // A reconnect missed an unknown number of routing keys, so re-derive
      // the whole registration set — the same thing cold boot does.
      onReconnect: () =>
        registerAllGenericSourceBundles({
          sourceManager: this.opts.sourceManager,
