...
import { DepCache, depPointerKey, depTarballKey } from './dep-cache.js';
...
describe('siblingsDigest in the pointer key', () => {
  const LOCK = 'lock-hash';
  const SIB = 'sib-digest';

  it('keys exactly as before when the lock carries no digest', () => {
    // Most repos have no in-repo sibling, so their existing pointers stay live.
    expect(depPointerKey(LOCK, 'linux', 'x64')).toBe('deps/linux-x64/lock-hash.hash');
    expect(depPointerKey(LOCK, 'linux', 'x64', undefined)).toBe('deps/linux-x64/lock-hash.hash');
  });

  it('folds the digest into the pointer name when present', () => {
    expect(depPointerKey(LOCK, 'linux', 'x64', SIB)).toBe(
      'deps/linux-x64/lock-hash-sib-digest.hash',
    );
  });

  it('leaves the immutable tarball key untouched', () => {
    // The object keeps its own content addressing; only the pointer moves.
    expect(depTarballKey('abc', 'linux', 'x64')).toBe('deps/linux-x64/abc.tar.gz');
  });
});

describe('siblingsDigest end-to-end through the pointer', () => {
  let storage: InMemoryCacheStorage;
  let cache: DepCache;
  const LOCK = 'lock-hash';
  const tar = Buffer.from('deps bytes');
  ...
  it('misses after a sibling edit, so the deps are rebuilt', async () => {
    await cache.store(LOCK, 'linux', 'x64', tar);
    await cache.publishPointer(LOCK, 'linux', 'x64', DepCache.computeHash(tar), 'sib-v1');
    expect(await cache.has(LOCK, 'linux', 'x64', 'sib-v1')).toBe(true);
    // Same package-manager lock file, edited sibling source.
    expect(await cache.has(LOCK, 'linux', 'x64', 'sib-v2')).toBe(false);
  });

  it('does not confuse a sibling-keyed entry with a digest-free one', async () => {
    await cache.store(LOCK, 'linux', 'x64', tar);
    expect(await cache.has(LOCK, 'linux', 'x64')).toBe(true);
    expect(await cache.has(LOCK, 'linux', 'x64', 'sib-v1')).toBe(false);
  });
});
