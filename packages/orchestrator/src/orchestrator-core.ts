...
import { unroutableFastFailedTotal, dispatchOwnershipSkippedTotal } from './metrics/prometheus.js';
...
import { assertAgentAuthBindSafe, assertAgentReachableStorage } from './storage/loopback-guard.js';
import {
...
import { sweepStuckCancelling } from './cancel/sweep-stuck-cancelling.js';
import {
  PeerRegistry,
  PeerClient,
  createPeerHandler,
...
import {
  registerAllGenericSourceBundles,
  registerProviderBundleForSource,
} from './webhook/register-source-bundle.js';
...
import { warnOnHeldRunLockouts } from './policy/dashboard-write-policy.js';
import {
...
import { ConcurrencyQueueManager, ConcurrencySlotStatus } from './concurrency/index.js';
...
import { InstanceHeartbeat, instanceLivenessGraceMs } from './cluster/instance-heartbeat.js';
...
async function initializeScaler(
   ...
   * event scaler backend calls it on every scale-up / scale-down, including a
   * warm pool's first fill — which is why the caller starts the warm pools
   * only after the emitter is assigned, not inside this function.
   */
  ...
  const factoryCtx: BackendFactoryContext = {
    ...
    isAgentRegistered: (agentId: string) => agentRegistry?.isRegistered(agentId) ?? true,
    logger,
  };
  ...
  // The warm pools are started by the caller once the event emitter is
  // assigned: their first fill spawns at once, and an event backend's spawn
  // emits through that emitter.
  scalerManager.start();
...
interface SecretsInfra {
   ...
   * precedence. Null when no master key is configured. Every master-key-wrapped
   * store reads it from here, so `KICI_SECRET_KEY_FILE` reaches all of them.
   */
...
async function initializeSecrets(
  ...
  const masterKeys = resolveMasterKeys(config)!;
  const masterKey = masterKeys.current;
  const oldMasterKey = masterKeys.old;
  if (oldMasterKey) {
  ...
  return { secretResolver, adminDeps, pgSecretStore, auditLogger, masterKeys };
}
...
export async function mergeUpstreamOutputs(
  ...
  try {
    ...
    // With no master key configured the secrets subsystem is off, so a stored
    // output cannot be unsealed. Name that rather than throwing into the outer
    // catch, which would report it as a merge failure.
    if (hasSecretOutputs && !masterKeys) {
      logger.warn('Upstream secret outputs present but no master key is configured — skipping', {
        runId,
      });
    } else if (hasSecretOutputs && masterKeys) {
      ...
      for (const outputs of Object.values(upstreamSecretOutputs)) {
        for (const [key, encryptedValue] of Object.entries(outputs)) {
          if (!(key in mergedSecrets)) {
            mergedSecrets[key] = unsealSecretOutput(encryptedValue, runId, masterKeys);
          }
        }
      }
    }
...
function buildOnDispatch(
  ...
  return async (agentId: string, job: any) => {
    ...
    const dispatchProvenanceContext = await loadProvenanceContext(db, job.runId, job.id, {
      issuer: config.provenanceSigningIssuer,
      orchestratorId: config.instanceId,
    }).catch((err) => {
      // Best-effort: a failure here costs the deferred path its cross-checkable
      // statement, never the dispatch itself.
      logger.warn('Failed to resolve provenance context for dispatch', {
        runId: job.runId,
        jobId: job.id,
        error: toErrorMessage(err),
      });
    ...
    const dispatchMsg: Record<string, unknown> = {
      ...
      // The mint's own view of this build, so a deferred attestation freezes a
      // statement the orchestrator can cross-check against its own run row.
      // Absent when no provenance issuer is configured — the agent then falls
      // back to its local guess, whose statement the capture check rejects.
      ...(dispatchProvenanceContext && { provenanceContext: dispatchProvenanceContext }),
...
function buildOnSecretOutputs(masterKeys: ResolvedMasterKeys, db: Kysely<Database>) {
  return async (runId: string, jobId: string, secretOutputs: Record<string, any>) => {
    try {
      ...
      const runPrivateKey = decryptPrivateKey(
        keyRow.encrypted_private_key,
        masterKeys.material,
        masterKeys.materialOld,
      );
      ...
      for (const [key, envelope] of Object.entries(secretOutputs)) {
        try {
          ...
          const reEncrypted = sealSecretOutput(plaintext, masterKeys.current, runId);
...
interface ClusterInfra {
  ...
  /**
   * Late-bound stuck-cancelling re-drive, invoked by the leader sweep tick. The
   * cancel path needs the execution tracker and the peer transport, both built
   * after this substrate, so the bootstrap assigns it once they exist.
   */
  stuckCancellingSweepRef: { current: (() => Promise<void>) | null };
  /**
...
function initializeCluster(
  ...
  const stuckCancellingSweepRef: { current: (() => Promise<void>) | null } = {
    current: null,
  };
    ...
    onBecomeLeader: () => {
      ...
      if (!recoverySweepTimerRef.current) {
        recoverySweepTimerRef.current = setInterval(() => {
          ...
          // Re-drive a cancel that never completed. A cancel travels to the
          // owning coordinator over the peer channel, and a peer blip drops it,
          // leaving the run in `cancelling` with its jobs still executing. The
          // workflow-deadline detector already re-scans `cancelling` runs but
          // only ones carrying a workflow timeout; this covers the rest.
          stuckCancellingSweepRef.current?.().catch((err) => {
            logger.error('Stuck-cancelling sweep failed', { error: toErrorMessage(err) });
          });
        }, 10_000);
      }
    },
...
export async function bootstrapOrchestrator(
  ...
  // Schema work runs on its own timeout-free pool: a backfill over a table
  // holding real history legitimately outlives the hot path's statement
  // timeout, and inheriting that cap turns an upgrade into a boot loop.
  if (config.autoMigrate) {
    await withMigrationPool(config.databaseUrl, (migrationDeps) => runMigrations(migrationDeps));
    // Indexes on volume-scaling tables are built concurrently after boot
    // rather than inside a migration, so they never lock the table an
    // upgrade is waiting on. Not awaited: the server serves while they
    // build, and a failure retries on the next boot.
    void buildDeferredIndexes(config.databaseUrl).catch((err) => {
      logger.warn('Deferred index build did not run', { error: toErrorMessage(err) });
    });
  } else {
  ...
  const queue = new JobQueue(db, {
    ...
    // Stamped onto every row this coordinator dispatches, so a cluster-wide
    // sweep can tell its own rows from a live sibling's.
    instanceId: config.instanceId,
    ownershipGraceMs: instanceLivenessGraceMs(
  ...
  // event router built later. The provider throws while it is unassigned, so
  // nothing that can emit may run before the assignment — a demand scale-up
  // cannot (dispatch starts later), and the warm pools' first fill is held
  // back by `startWarmPools()` being called right after it.
  ...
  // Fail fast when agent auth is disabled on a listener anyone can reach. An
  // unauthenticated agent registration on a routable address is a credential-free
  // job-dispatch surface, so this refuses rather than warns.
  assertAgentAuthBindSafe(config);
          ...
          db.selectFrom('concurrency_groups')
            ...
            .then(async (entries) => {
              for (const entry of entries) {
                await concurrencyQueueManagerRef!.releaseSlot(
                  entry.group_key,
                  entry.routing_key,
                  runId,
                );
    ...
    { db, instanceId: config.instanceId },
    {
      cleanup: {
        ...
        handler: createCleanupHandler(dedup, queue, {
          ...
          // The database-side retention tier. Covers an install with no object
          // store, which is the default; with the cold store on, its archivers
          // keep sole ownership of the tables they register.
          retentionWindows: {
            runRetentionDays: config.runRetentionDays,
            auditRetentionDays: config.auditRetentionDays,
            provenanceRetentionDays: config.provenanceRetentionDays,
            heldRunRetentionDays: config.heldRunRetentionDays,
          },
          coldStoreEnabled: readOrchestratorColdStoreConfig().enabled,
  ...
  // errors_total{reason}); the helper catches each outer list-fetch failure.
  // The same helper is the generic-sources listener's reconnect catch-up.
  await registerAllGenericSourceBundles({
  ...
  // A policy that already disables a held-run write on a Platform-attached
  // orchestrator leaves NO surface that can answer a context or reviewer hold —
  // the admin CLI refuses in this mode. The write path refuses that combination, but a
  ...
  // The warm pools' first fill spawns immediately, and on an event backend
  // that spawn emits through the emitter assigned on the line above.
  ...
  // Old key fallback enables the rotation grace window: if KICI_SECRET_KEY_OLD
  // (or the _OLD file) is set, rows encrypted under the previous generation
  // still decrypt until `rotate-key` has completed.
  const masterKeyForStore: Buffer | null = masterKeys?.current ?? null;
  const oldMasterKeyForStore: Buffer | null = masterKeys?.old ?? null;
  ...
  if (adminDeps) {
    ...
    adminDeps.masterKeys = masterKeys;
  }
  ...
  const reconcileSignerOnce = (): Promise<{ signer: Signer } | null> =>
    reconcileOrchestratorSigningKey({
      ...
      // re-encrypted under the current one while KICI_SECRET_KEY_OLD is set, so
      // a deployment stranded by an earlier rotation recovers by restoring the
      // old key once and restarting.
      selfHeal: masterKeys
        ? (row, privateJwk) => selfHealStrandedSigningKey(db, row, privateJwk, masterKeys)
        : undefined,
  ...
  const dashboardEncryptionRepo = masterKeys ? new DashboardEncryptionKeyRepo(db) : undefined;
  ...
  const resolveDashboardEncryptionKey =
    async (): Promise<ResolvedDashboardEncryptionKey | null> => {
      ...
      const reconciled = await reconcileDashboardEncryptionKey({
        ...
        oldSecretKey: masterKeys?.materialOld,
        selfHeal: masterKeys
          ? (row, der) => selfHealStrandedDashboardKey(db, row, der, masterKeys)
          : undefined,
        logWarn: (message, meta) => logger.warn(message, meta),
        audit: ({ kid }) => {
  ...
  const ingestOverflowReplayer = new IngestOverflowReplayer({
    ...
    // The same fairness key + per-org cap the live ingest paths admit on, so a
    // replay reservation competes on exactly the terms a fresh delivery would.
    resolveAdmissionKey: (routingKey) => ingestCapReader.resolve(routingKey),
    ...
    maxAgeMs: config.ingestOverflowMaxAgeMs,
  ...
  // Concurrency group support -- always enabled. The DATABASE arbitrates slots;
  // there is no process-local mirror, so `max` means the same thing across a
  // restart and across every coordinator in the cluster.
  ...
  const { app, wss, tryDispatchNextQueued, reinjectDirect } = createApp({
    ...
    registrationIndex,
    logStorage,
    onSecretOutputs: masterKeys ? buildOnSecretOutputs(masterKeys, db) : undefined,
  ...
  const stuckCancellingAfterMs = config.agentMaxReconnectDelayMs * 4;
  ...
  // 30b. Coordinator heartbeat. Started BEFORE startup recovery: the recovery
  // scan below reads `cluster_instances` to decide whether a row's owner is
  // alive, and a sibling booting at the same moment reads it to decide the same
  // about this instance. A coordinator with no row yet is invisible, so the
  // first beat has to land before anybody scans.
  const instanceHeartbeat = new InstanceHeartbeat({
    db,
    instanceId: config.instanceId,
    intervalMs: config.clusterInstanceHeartbeatMs,
    version: options?.orchestratorVersion,
    getRole: () => cluster.raft.getRole(),
  });
  await instanceHeartbeat.start();

  // 31. Startup recovery
  //
  // Scoped to rows this instance owns or whose owning coordinator is no longer
  // alive. An unfiltered scan of the shared table made every coordinator boot
  // flip every sibling's in-flight row to `recovering` and fail it 120 seconds
  // later, while the agents ran those jobs to completion.
  const dispatchedJobs = await queue.getOrphanedDispatchedJobs();
  const spared = await queue.countSparedDispatchedJobs();
  if (spared.ownedElsewhere > 0 || spared.unknownOwner > 0) {
    logger.warn('Startup recovery spared dispatched jobs this instance does not own', {
      ownedByLiveInstance: spared.ownedElsewhere,
      unknownOwner: spared.unknownOwner,
    });
    dispatchOwnershipSkippedTotal.add(spared.ownedElsewhere, {
      action: 'startup_recovery',
      reason: 'owned_elsewhere',
    });
    dispatchOwnershipSkippedTotal.add(spared.unknownOwner, {
      action: 'startup_recovery',
  ...
  setupGracefulShutdown({
    ...
    steps: [
      ...
      {
        // Drop this coordinator's `cluster_instances` row, not just its timer.
        // A stopped coordinator that leaves its row behind reads as alive to
        // every sibling for the whole grace window, so the rows it owned are
        // spared by their ownership predicates and their recovery waits out the
        // window for no reason. Best-effort, and before the DB is closed.
        name: 'Retiring coordinator heartbeat',
