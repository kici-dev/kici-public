...
describe('DashboardHandler', () => {
  ...
  describe('handleAdminTokensList', () => {
    const buildHandler = () => {
      ...
      return { handler, mockDb, accessLogRecord };
    };

    const request = {
      type: 'dashboard.admin-tokens.list' as const,
      requestId: 'req-tokens-1',
      actor: { type: 'user' as const, sub: 'alice' },
    };

    it('projects the token inventory without the hash and audits the read', async () => {
      const { handler, mockDb, accessLogRecord } = buildHandler();
      mockDb.mocks.selectExecute.mockResolvedValueOnce([
        {
          id: 'tok-1',
          label: 'alice-ops',
          role: 'admin',
          ...
          expires_at: null,
          last_used_at: new Date('2026-09-02T10:00:00Z'),
          revoked: false,
        },
        {
          id: 'tok-2',
      ...
      const res = await handler.handleAdminTokensList(request);

      expect(res.type).toBe('dashboard.admin-tokens.list.response');
      expect(res.error).toBeUndefined();
      ...
      // A token nobody recorded a holder for stays null — that IS the finding.
      expect(res.tokens[1]!.subject).toBeNull();
      // The routing-key scope rides along so the report can tell a scoped
      // admin token from an unscoped one.
      expect(res.tokens[1]!.routingKey).toBe('github:42');

      // Shape guard: no credential material may reach the dashboard plane.
      for (const t of res.tokens) {
        expect(t).not.toHaveProperty('token_hash');
        expect(t).not.toHaveProperty('tokenHash');
      }
      // The projection never asks for the hash column in the first place.
      expect(mockDb.mocks.select).toHaveBeenCalledWith(expect.not.arrayContaining(['token_hash']));

      expect(accessLogRecord).toHaveBeenCalledTimes(1);
      expect(accessLogRecord.mock.calls[0][0]).toMatchObject({
        orgId: 'org-1',
        routingKey: 'github:42',
        action: 'admin_tokens.list.read',
        source: 'platform_proxy',
        outcome: 'allowed',
        target: null,
        actor: { type: 'user', sub: 'alice' },
    ...
    it('filters out revoked tokens at the query level', async () => {
      const { handler, mockDb } = buildHandler();
      mockDb.mocks.selectExecute.mockResolvedValueOnce([]);

      await handler.handleAdminTokensList(request);

      // A revoked token is not a finding, so it never enters the report.
      expect(mockDb.mocks.selectWhere).toHaveBeenCalledWith('revoked', '=', false);
    });

    it('reports an error envelope and an error access_log row on a query failure', async () => {
      const { handler, mockDb, accessLogRecord } = buildHandler();
      mockDb.mocks.selectExecute.mockRejectedValueOnce(new Error('db is down'));

      const res = await handler.handleAdminTokensList(request);

      expect(res.tokens).toEqual([]);
      expect(res.error).toBe('Internal error querying admin tokens');
      expect(accessLogRecord.mock.calls[0][0]).toMatchObject({
        action: 'admin_tokens.list.read',
        outcome: 'error',
      });
