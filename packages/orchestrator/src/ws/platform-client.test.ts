...
describe('PlatformClient', () => {
  ...
  describe('source registration', () => {
    ...
    it('includes configPaths in source.register when the orchestrator knows any', () => {
      // fails-when: the post-auth registration spread drops the field — the
      // dashboard panel would be blank for every freshly connected orchestrator.
      const client = createClient({
        providerSources: [
          { provider: 'github', routingKey: 'github:42', name: 'My App', subtype: 'github_app' },
        ],
        configPaths: { envFile: '/etc/kici/orch.env', scalerConfig: '/etc/kici/scalers.yaml' },
      });
      client.connect();
      const mock = getLatestMock();
      simulateOpen(mock);
      ...
      const sent = getSentMessages(mock);
      const registerMsg = sent.find((m: any) => m.type === 'source.register') as any;
      expect(registerMsg.configPaths).toEqual({
        envFile: '/etc/kici/orch.env',
        scalerConfig: '/etc/kici/scalers.yaml',
      });
    });

    it('includes configPaths when a source is registered at runtime', () => {
      // fails-when: the `sendSourceRegister` spread drops the field — adding a
      // source after boot would blank the paths the Platform already recorded.
      const client = createClient({
        configPaths: { envFile: '/etc/kici/orch.env' },
      });
      const mock = authenticateClient(client);
      mock.sentMessages = [];

      client.sendSourceRegister({
        provider: 'github',
      ...
      const sent = getSentMessages(mock);
      const registerMsg = sent.find((m: any) => m.type === 'source.register') as any;
      expect(registerMsg.configPaths).toEqual({ envFile: '/etc/kici/orch.env' });
    });

    it('includes configPaths when a changed source is re-registered', () => {
      // fails-when: the `updateSources` changed-sources spread drops the field —
      // renaming a source hours after boot would blank the dashboard panel.
      const client = createClient({
        providerSources: [
          {
            provider: 'github',
        ...
        configPaths: { scalerConfig: '/etc/kici/scalers.d' },
      });
      const mock = authenticateClient(client);
      mock.sentMessages = [];
      ...
      const sent = getSentMessages(mock);
      const registerMsg = sent.find((m: any) => m.type === 'source.register') as any;
      expect(registerMsg.configPaths).toEqual({ scalerConfig: '/etc/kici/scalers.d' });
    });

    it('includes configPaths in the empty re-announce after every source is removed', () => {
      // fails-when: the `updateSources` all-removed spread drops the field —
      // an orchestrator that loses its last source would blank the panel.
      const client = createClient({
        providerSources: [
          {
            provider: 'github',
        ...
        configPaths: { composeFile: '/etc/kici/docker-compose.yaml' },
      });
      const mock = authenticateClient(client);
      mock.sentMessages = [];
      ...
      const registerMsg = sent.find((m: any) => m.type === 'source.register') as any;
      expect(registerMsg).toBeDefined();
      expect(registerMsg.sources).toEqual([]);
      expect(registerMsg.configPaths).toEqual({ composeFile: '/etc/kici/docker-compose.yaml' });
    });

    it('omits configPaths from source.register when nothing is known', () => {
      // An empty object would read on the Platform as "reported nothing known"
      // rather than "did not report" — the guard in the spread prevents that.
      const client = createClient({
        providerSources: [
          { provider: 'github', routingKey: 'github:42', name: 'My App', subtype: 'github_app' },
        ],
      ...
      const sent = getSentMessages(mock);
      const registerMsg = sent.find((m: any) => m.type === 'source.register') as any;
      expect(registerMsg.configPaths).toBeUndefined();
    });
    ...
    it('registerSourceAndAwait rejects with the Platform reason rather than timing out', async () => {
      // A rejected key used to leave the waiter pending until its 5 s timeout,
      // so `kici-admin source add` printed "(unavailable)" instead of the
      // reason the Platform gave — for instance that the source's `--org` does
      // not match the authenticated Platform organization.
      const client = createClient({ providerSources: [] });
      const mock = authenticateClient(client);
      ...
      const pending = client.registerSourceAndAwait(
        [
          {
            provider: 'generic',
            routingKey: 'generic:org_wrong:src-1',
            name: 'mismatched org',
            subtype: 'generic_webhook',
          },
        ],
        'generic:org_wrong:src-1',
      );
      ...
      simulateMessage(mock, {
        ...
        rejected: [
          {
            routingKey: 'generic:org_wrong:src-1',
            reason: 'routing key org segment does not match the authenticated organization',
          },
        ],
      });

      await expect(pending).rejects.toThrow(
        /routing key org segment does not match the authenticated organization/,
      );
    });
  ...
  describe('chunked webhook.relay protocol', () => {
    ...
    it('captures the shed relay delivery (meta + assembled body) before the shed ack', async () => {
      ...
      // and the controller's own shed reason — the breadcrumb names it, so a
      // reason the seam never receives is a reason no durable record carries.
      expect(onShedCapture).toHaveBeenCalledTimes(1);
      const [meta, bodyBuf, reason] = onShedCapture.mock.calls[0]!;
      expect(meta.routingKey).toBe('github:12345');
      expect(meta.deliveryId).toBe('del-m-cap');
      expect((bodyBuf as Buffer).toString('utf8')).toBe(body);
      expect(reason).toBe('queue_full');
    ...
    it('does not record a shed for an admitted relay, nor for one verify rejects', async () => {
      // The counterpart to the shed breadcrumb: the seam that writes the
      // durable `shed` record must be unreachable for a delivery that was
      // never shed, or an admitted delivery gains a phantom shed row.
      const onWebhookRelay = vi.fn().mockResolvedValue(undefined);
      const onShedCapture = vi.fn().mockResolvedValue(undefined);
      const release = vi.fn();
      const onAdmit = vi.fn().mockResolvedValue({ admitted: true, release });
      ...
      const acceptedMock = authenticateClient(accepted);
      const body = '{"foo":1}';
      simulateMessage(acceptedMock, makeStartFrame('m-ok', body, 1));
      simulateMessage(acceptedMock, makeChunk('m-ok', 0, body, true));
      await vi.advanceTimersByTimeAsync(100);

      expect(onWebhookRelay).toHaveBeenCalledTimes(1);
      expect(onShedCapture).not.toHaveBeenCalled();
      ...
      const rejectedMock = authenticateClient(rejected);
      simulateMessage(rejectedMock, makeStartFrame('m-bad', body, 1));
      simulateMessage(rejectedMock, makeChunk('m-bad', 0, body, true));
      await vi.advanceTimersByTimeAsync(100);
