...
function makeDeps(over: Partial<BringupApiDeps> = {}): {
  ...
  resolveNamedInternal: ReturnType<typeof vi.fn>;
} {
  const mints = vi.fn(async () => ({ token: 'kat_boottoken', id: 'boot-1' }));
  const records = vi.fn(async () => undefined);
  const resolveNamedInternal = vi.fn(async () => 'PRIVATE-KEY-MATERIAL');
  ...
  const deps = {
    ...
    secretResolver: { resolveNamedInternal } as unknown as BringupApiDeps['secretResolver'],
  ...
  return { deps, mints, records, resolveNamedInternal };
}
...
describe('createEnsureInitRunnerHandler', () => {
  ...
  it('mints a single-use token, resolves the key, audits, and returns the material', async () => {
    const { deps, mints, records, resolveNamedInternal } = makeDeps();
    const handler = createEnsureInitRunnerHandler(deps);
    const result = await handler(OPS, { targetAgentId: FRESH });

    expect(resolveNamedInternal).toHaveBeenCalledWith('__default__', 'prod/bootstrap', 'ssh');
...
describe('createPreBootSendHandler', () => {
  ...
  it('resolves the SSH key + input secret, defaults to dropbear port/command, audits', async () => {
    const { deps, resolveNamedInternal, records } = makeDeps();
    // First resolveNamedInternal call = SSH key (prod/bootstrap/ssh); second = input.
    resolveNamedInternal.mockResolvedValueOnce('SSH-KEY').mockResolvedValueOnce('LUKS-PASSPHRASE');
    ...
    expect(resolveNamedInternal).toHaveBeenNthCalledWith(1, '__default__', 'prod/bootstrap', 'ssh');
    expect(resolveNamedInternal).toHaveBeenNthCalledWith(
      2,
      '__default__',
      'prod/luks',
      'box-00007',
