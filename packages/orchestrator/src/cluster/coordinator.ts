...
export class RunCoordinator {
  ...
  private cancelPeerJob(peerId: string, runId: string, jobId: string, reason: string): void {
    if (!this.cancelJobOnPeer(peerId, runId, jobId, reason)) {
 ...
 *
 * Applies the same gate as `peerAgentMatchesRequiredLabels` in
 * `peer-registry.ts` — coordinator-side capacity scoring must apply the same
 * gate cluster routing uses, otherwise the coordinator would over-count gated
 * agents when ranking peers for an off-gate label set.
 *
 * Both sides compare already-canonical labels: `agent.mandatoryLabels` is
 * folded when the heartbeat lands in `PeerRegistry.updateHeartbeat`, and
 * `requiredLabels` is a job's `runsOnLabels`, folded at its own ingress. So
 * this comparison folds nothing itself; feeding it a raw label from a new
 * source would silently under-count, and the fix is to fold at that source.
 */
