...
describe('PeerRegistry', () => {
  ...
  describe('findPeersWithCapacity', () => {
    ...
    it('matches a peer agent whose labels differ from the requirement only by case', () => {
      registry.addPeer({
        instanceId: 'orch-case',
        connectionId: 'conn-case',
        address: null,
        routingKeys: [],
      });
      registry.updateHeartbeat('orch-case', {
        type: 'peer.heartbeat',
        instanceId: 'orch-case',
        term: 1,
        leaderId: null,
        ...
        capabilities: { s3LogAccess: false },
        // No scaler capacity: the connected-agent subset test is the only path
        // that can select this peer, so the assertion cannot pass on the
        // scaler half that already folded.
        scalerCapacity: [],
        timestamp: Date.now(),
      });

      expect(registry.findPeersWithCapacity([['docker', 'linux']])).toHaveLength(1);
      // The matcher still discriminates: a label the agent does not carry in any
      // case selects nothing, so the assertion above is not vacuously true.
      expect(registry.findPeersWithCapacity([['docker', 'windows']])).toHaveLength(0);
    });

    it('honours a peer-agent mandatory gate that differs from the requirement only by case', () => {
      registry.addPeer({
      ...
      registry.updateHeartbeat('orch-gate', {
        type: 'peer.heartbeat',
        instanceId: 'orch-gate',
        term: 1,
        leaderId: null,
        ...
        agents: [
          {
            agentId: 'a1',
            // The agent's own labels match the requirement exactly, so only
            // the gate's fold decides the outcome.
            labels: ['gpu'],
            activeJobs: 0,
            maxConcurrency: 1,
            platform: 'linux',
      ...
      expect(registry.findPeersWithCapacity([['gpu']])).toHaveLength(1);
      // The gate still bites: a requirement that names neither case of the
      // mandatory label leaves the peer invisible.
      expect(registry.findPeersWithCapacity([['cuda']])).toHaveLength(0);
    });
  });
  ...
  describe('findPeersWithLabels mandatoryLabels gate', () => {
    ...
    it('matches a peer agent whose labels differ from the requirement only by case', () => {
      registry.addPeer({
        instanceId: 'orch-case-labels',
        connectionId: 'conn-case-labels',
        address: null,
        routingKeys: [],
      });
      registry.updateHeartbeat('orch-case-labels', {
        type: 'peer.heartbeat',
        instanceId: 'orch-case-labels',
        term: 1,
        leaderId: null,
        ...
        capabilities: { s3LogAccess: false },
        // No scaler capacity: only the connected-agent subset test in
        // findPeersWithLabels can select this peer.
        scalerCapacity: [],
        timestamp: Date.now(),
      });

      expect(registry.findPeersWithLabels([['docker', 'linux']])).toHaveLength(1);
      expect(registry.findPeersWithLabels([['docker', 'windows']])).toHaveLength(0);
    });

    it('honours a peer-agent mandatory gate that differs from the requirement only by case', () => {
      registry.addPeer({
        instanceId: 'orch-case-gate',
        connectionId: 'conn-case-gate',
        address: null,
        routingKeys: [],
      });
      registry.updateHeartbeat('orch-case-gate', {
        type: 'peer.heartbeat',
        instanceId: 'orch-case-gate',
        term: 1,
        leaderId: null,
      ...
      expect(registry.findPeersWithLabels([['gpu']])).toHaveLength(1);
      expect(registry.findPeersWithLabels([['cuda']])).toHaveLength(0);
    });
  });
