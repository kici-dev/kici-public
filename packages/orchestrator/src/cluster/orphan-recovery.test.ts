...
/**
 * A DB stub for the concurrency-slot reconcile, which is a join + a bulk
 * update rather than one of the three indexed reads the main mock models.
 */
function createReconcileDb(leaked: Array<{ id: string }>) {
  ...
  const db: any = {
    selectFrom: vi.fn(() => {
      ...
      chain.execute = vi.fn(async () => leaked);
      return chain;
    }),
    updateTable: vi.fn(() => {
      const record = { set: {} as Record<string, unknown>, where: [] as unknown[][] };
      updates.push(record);
      const chain: any = {};
...
describe('OrphanRecovery', () => {
  ...
  // ── Concurrency-slot reconcile ──────────────────────────────────

  describe('leaked concurrency slots', () => {
    it('releases an active slot whose run already finished', async () => {
      // A slot outlives its run when the release path did not run — a
      // coordinator crash between "the run finished" and "the slot was
      // released". On a `max: 1` deploy gate that leaked slot blocks every
      // later deploy forever, and nothing else would notice.
      const { db, updates } = createReconcileDb([{ id: 'slot-1' }, { id: 'slot-2' }]);
      ...
      await recovery.releaseLeakedConcurrencySlots();

      expect(updates).toHaveLength(1);
      expect(updates[0].set.status).toBe('completed');
      expect(updates[0].where).toContainEqual(['id', 'in', ['slot-1', 'slot-2']]);
    });

    it('writes nothing when no slot is leaked', async () => {
