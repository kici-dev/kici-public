...
const baseSchema = z.object({
   ...
   * deployment must accept connections from remote agents. Set it to a loopback
   * address for a single-machine plane — which `KICI_AGENT_AUTH=none` then
   * requires (`assertAgentAuthBindSafe`).
   */
  ...
  // ingestOverflowMaxAttempts: replay attempts before a row goes `failed`. Only
  // a GENUINE failure spends one — a verify throw, a pipeline throw, a claim
  // that went stale. A capacity refusal never claims the row (the drain
  // reserves its admission slot first), so an overload of any length costs a
  // buffered delivery no attempts at all.
  ingestOverflowMaxAttempts: z.coerce.number().int().min(1).default(10),
  // ingestOverflowMaxAgeMs: how long a delivery may sit buffered awaiting
  // capacity before it is marked `failed` (`max_age`). This is the retention
  ...
  // a later pass.
  ingestOverflowMaxAgeMs: z.coerce.number().int().min(60_000).default(900_000),
  ...
  // Database-side retention windows, in days. Retention used to be
  // object-store-first: every aging table routed through cold-store
  // archive-then-delete, so with no bucket configured — the default — nothing
  // aged and the operator's own disk filled. These windows prune the warm
  // tables directly. 0 disables each, mirroring dispatchQueueTtlDays.
  //
  // Non-zero defaults on purpose: a 0 default fixes nothing for the operator
  // who never reads the docs, who is exactly the one whose disk fills. The
  // sweep announces itself and deletes nothing for a week first, so the
  // change is loud rather than silent.
  //
  // Terminal runs and their jobs and steps. 90 days matches the CI norm
  // already chosen for stepLogTtlDays, and is more generous than the 30-day
  // warm TTL the storage-layout doc already publishes for these tables.
  runRetentionDays: z.coerce.number().int().nonnegative().default(90),
  // access_log, secret_audit_log and event_log. A year, because these are the
  // rows an operator reaches for when reconstructing what happened.
  auditRetentionDays: z.coerce.number().int().nonnegative().default(365),
  // attestations and pending_attestations. A year, matching the audit window:
  // a build attestation is evidence about a release, not run telemetry.
  provenanceRetentionDays: z.coerce.number().int().nonnegative().default(365),
  // Terminal held_runs only. A pending hold is never pruned, whatever its age.
  heldRunRetentionDays: z.coerce.number().int().nonnegative().default(90),
  ...
  // Interval between `cluster_instances` heartbeat writes. Every ownership
  // predicate in the dispatch plane reads this table to decide whether the
  // coordinator that owns a row is still alive, so this is bootstrap config:
  // it has to be readable before the coordinator can answer that question about
  // itself, which is why it is not a cluster_settings knob.
  clusterInstanceHeartbeatMs: z.coerce.number().default(10_000),
...
export const envDef = defineEnv({
  ...
  envMap: {
    ...
    runRetentionDays: 'KICI_RUN_RETENTION_DAYS',
    auditRetentionDays: 'KICI_AUDIT_RETENTION_DAYS',
    provenanceRetentionDays: 'KICI_PROVENANCE_RETENTION_DAYS',
    heldRunRetentionDays: 'KICI_HELD_RUN_RETENTION_DAYS',
    ...
    ingestOverflowMaxAgeMs: 'KICI_INGEST_OVERFLOW_MAX_AGE_MS',
    ...
    clusterInstanceHeartbeatMs: 'KICI_CLUSTER_INSTANCE_HEARTBEAT_MS',
...
const DEPLOY_IDENTITY_ENV_VARS = [
  ...
  'KICI_DEPLOY_CONFIG_FILE',
  'KICI_DEPLOY_COMPOSE_FILE',
];
...
/**
 * `KICI_*` names that belong to the config-reload overlay
 * (`config/env-overlay.ts`) rather than to the startup schema, mapped to the
 * advice the boot-time validator prints when it rejects one.
 *
 * Each name is documented in the operator configuration reference, so an
 * operator can plausibly set one — and the Levenshtein suggester finds no
 * near miss for any of them, so the rejection reads "no close match in the
 * schema" and names nothing to try instead.
 *
 * These names stay rejected. Only the message changes.
 */
const OVERLAY_ONLY_ENV_VAR_ADVICE: Record<string, string> = {
  KICI_SERVER_PORT: 'not a startup variable — set KICI_PORT',
  KICI_SERVER_BASE_PATH: 'not a startup variable — set KICI_BASE_PATH',
  KICI_SERVER_LOG_LEVEL: 'not a startup variable — set KICI_LOG_LEVEL',
  KICI_INSTANCE_ID: 'not a startup variable — set KICI_CLUSTER_INSTANCE_ID',
  KICI_INSTANCE_MODE: 'not a startup variable — set KICI_MODE',
  KICI_NODE_ENV: 'not a startup variable — set NODE_ENV',
  // cluster.autoRotateCredentials is declared in the startup schema but has no
  // env-var mapping and no reader, so there is no equivalent to redirect to.
  KICI_CLUSTER_AUTO_ROTATE_CREDENTIALS:
...
export function loadConfig(scope: ConfigScope = ConfigScope.enum.runtime): AppConfig {
  ...
  if (!packaging) {
    validateUnknownKiciVars(
      ...
      {
        // KICI_CONFIG selects the local YAML file `loadLocalConfig`
        // (config/loader.ts) reads on every config reload, and the one
        // `kici-admin scaler` reads. The startup schema never reads that file,
        // so it is path selection rather than a config field — and only this
        // service consumes it, so it is allowlisted here rather than
