...
describe('orchestrator loadConfig', () => {
  ...
  describe('newly-validated env vars (drift catcher)', () => {
    ...
    it('accepts the stamped config-path env vars at boot', () => {
      // fails-when: either name is missing from DEPLOY_IDENTITY_ENV_VARS — the
      // unknown-KICI_* validator rejects it and the orchestrator will not boot.
      process.env.KICI_DEPLOY_MODE = 'systemd';
      process.env.KICI_DEPLOY_CONFIG_FILE = '/etc/kici/orch.env';
      process.env.KICI_DEPLOY_COMPOSE_FILE = '/etc/kici/orch-compose.yaml';
      expect(() => loadConfig()).not.toThrow();
    });

    it('still rejects an unregistered KICI_DEPLOY_* name', () => {
      // The positive control for the test above. Without it that assertion
      // would pass just as happily against a validator that accepts anything.
      process.env.KICI_DEPLOY_BOGUS = 'x';
      expect(() => loadConfig()).toThrow(/KICI_DEPLOY_BOGUS/);
    });
  ...
  // Pins the claim in docs/operator/orchestrator/configuration.md § Overview:
  // "the orchestrator starts from environment variables ... no YAML file and
  //  no database row takes part."
  describe('the startup config does not read the local YAML file', () => {
    let yamlDir: string;
    let yamlPath: string;

    beforeEach(async () => {
      yamlDir = await mkdtemp(join(tmpdir(), 'kici-startup-yaml-'));
      yamlPath = join(yamlDir, 'orchestrator.yaml');
      await writeFile(
        yamlPath,
        [
          'database:',
          "  url: 'postgresql://from-yaml/kici'",
          'server:',
          '  port: 4999',
          "  basePath: '/yaml-only'",
          '',
    ...
    // fails-when: loadConfig() grows a YAML read — the file's `/yaml-only`
    //   basePath and port 4999 would replace the built-in defaults asserted
    //   here. The resolveLocalConfig() half is what stops a mis-pathed or
    //   unparsable fixture from passing this for the wrong reason.
    it('ignores a local YAML file and returns the built-in defaults', async () => {
      process.env.KICI_CONFIG = yamlPath;
      ...
      // The same file IS read by the reload path, so the fixture is not inert.
      const local = await resolveLocalConfig();
      expect((local.local as { server: { basePath: string } }).server.basePath).toBe('/yaml-only');
      expect(local.port).toBe(4999);
    });

    // breaks-if-wrong: the same two settings supplied through the environment
    //   must still reach the startup config. Without this, an implementation
    //   that returned constants would pass the assertion above.
    it('reads the same settings from the environment', () => {
      process.env.KICI_BASE_PATH = '/from-env';
      process.env.KICI_PORT = '4999';
    ...
    // fails-when: KICI_CONFIG is dropped from this loader's `extraKnown` — the
    //   documented way to point the reload path at a non-default file stops
    //   the service instead. The allowlist is per-service on purpose: only the
    //   orchestrator reads the file the variable names.
    it('starts when KICI_CONFIG is set in the environment', () => {
      process.env.KICI_CONFIG = yamlPath;
      expect(() => loadConfig()).not.toThrow();
    });

    // breaks-if-wrong: accepting KICI_CONFIG must stay path selection. A
    //   near-miss of the name is still a typo the boot check has to catch.
    it('still refuses to start on a near-miss of KICI_CONFIG', () => {
      process.env.KICI_CONFIGX = yamlPath;
      expect(() => loadConfig()).toThrow(/Unknown KICI_\* env var/);
    });
  });

  // Each of these names is documented in the operator configuration reference
  // and belongs to the reload overlay, not the startup schema. They stay
  // rejected; the rejection now names what to set instead.
  describe('a rejected overlay-only name names its startup equivalent', () => {
    ...
    // fails-when: the advice table loses an entry — the message falls back to
    //   "no close match in the schema", which names nothing to try instead.
    //   Asserting only /Unknown KICI_\* env var/ would pass either way.
    for (const [overlayName, startupName] of cases) {
      it(`${overlayName} names ${startupName}`, () => {
        process.env[overlayName] = 'x';
        expect(() => loadConfig()).toThrow(
          new RegExp(`${overlayName} {4}\\(not a startup variable — set ${startupName}\\)`),
        );
      });
    }

    // cluster.autoRotateCredentials is in the startup schema with no env
    // mapping and no reader, so there is nothing to redirect to.
    it('KICI_CLUSTER_AUTO_ROTATE_CREDENTIALS says there is no equivalent', () => {
      process.env.KICI_CLUSTER_AUTO_ROTATE_CREDENTIALS = 'true';
      expect(() => loadConfig()).toThrow(/has no startup equivalent/);
    });

    // breaks-if-wrong: the typo catcher is load-bearing and must not be
    //   widened. A genuinely unknown name is still rejected, and its message
    //   carries no redirect clause.
    it('still rejects a genuinely unknown name, with no redirect clause', () => {
      process.env.KICI_NOPE = '1';
      let message = '';
      try {
        loadConfig();
      } catch (err) {
      ...
      expect(message).toMatch(/Unknown KICI_\* env var/);
      expect(message).toMatch(/KICI_NOPE/);
      expect(message).not.toMatch(/not a startup variable/);
    });
  });
...
describe('orchestrator bind host', () => {
  const originalEnv = process.env;
  ...
  it('defaults to every interface, matching the Platform', () => {
    // A real deployment must accept remote agents, so flipping this default to
    // loopback would be a silent availability regression on upgrade.
    expect(loadConfig().host).toBe('0.0.0.0');
  });

  it('reads KICI_HOST', () => {
    process.env.KICI_HOST = '127.0.0.1';
