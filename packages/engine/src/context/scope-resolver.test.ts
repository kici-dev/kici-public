...
describe('resolveSecretsForContext', () => {
  const makeBinding = (
    id: string,
    envId: string,
    scopePattern: string,
  ...
  it('returns empty record for empty bindings', () => {
    const result = resolveSecretsForContext([], [], decryptFn);
  ...
  it('returns empty record for non-matching secrets', () => {
    ...
    const result = resolveSecretsForContext(bindings, secrets, decryptFn);
    expect(result).toEqual({});
  });
...
describe('resolveSecretsForContext with prefixed scopes', () => {
  const makeBinding = (
    id: string,
    envId: string,
    scopePattern: string,
...
describe('resolveSecretsForContext — per-host scoping', () => {
  const makeBinding = (id: string, scopePattern: string, hostPattern = '**'): ContextBinding => ({
    id,
    orgId: 'org1',
    contextId: 'env1',
    scopePattern,
  ...
  it('templated scope resolves each host its own subtree (one binding, N hosts)', () => {
    ...
    const r2 = resolveSecretsForContext(bindings, secrets, id, f2);
    const r3 = resolveSecretsForContext(bindings, secrets, id, f3);
  ...
  it('host_pattern gates which hosts a binding applies to', () => {
    ...
    expect(resolveSecretsForContext(bindings, secrets, id, f2).PG).toBe('pg-secret');
    const web = { agentId: 'web-1', host: 'web-1', labels: ['role:web'] };
    expect(resolveSecretsForContext(bindings, secrets, id, web).PG).toBeUndefined();
  });
  ...
  it('a per-host binding overrides a fleet-wide one for a colliding key', () => {
    ...
    expect(resolveSecretsForContext(bindings, secrets, id, f2).WG).toBe('host2');
  });
  ...
  it('without hostFacts, only ** bindings resolve and templated scopes are skipped', () => {
    ...
    const out = resolveSecretsForContext(bindings, secrets, id);
