...
describe('lock dynamic-field shapes', () => {
  // fails-when: the inline-expression reader is exported from the lock types again
...
describe('LockDynamicJobFn with gitCredentials', () => {
  it('round-trips a generator credential map through JSON', () => {
    const declared: LockDynamicJobFn = {
      _type: 'dynamic',
      source: { file: 'test.ts', index: 0 },
      group: 'reports',
      gitCredentials: { default: { kind: 'token', tokenSecret: 'ci:FORGE_PAT' } },
    };
    // The lock is written and read as JSON, so the map must survive verbatim —
    // the credential relay compares a request against it field by field.
    const roundTripped = JSON.parse(JSON.stringify(declared)) as LockDynamicJobFn;
    expect(roundTripped.gitCredentials).toEqual({
      default: { kind: 'token', tokenSecret: 'ci:FORGE_PAT' },
    });
  });

  it('leaves the key absent on a generator that declares none', () => {
    const undeclared: LockDynamicJobFn = {
      _type: 'dynamic',
      source: { file: 'test.ts', index: 0 },
    };
    // Absent, not `undefined`: an always-present key would change the emitted
    // object shape and move every workflow's contentHash.
    expect('gitCredentials' in undeclared).toBe(false);
  });
});
