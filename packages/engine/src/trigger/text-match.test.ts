...
describe('compileSafeRegex — flag handling', () => {
  it.each(['d', 'i', 'm', 's', 'u', 'v'])('unwraps a /pattern/%s entry', (flag) => {
    const re = compileSafeRegex(`/release-\\d+/${flag}`);
    expect(re?.flags).toBe(flag);
    expect(re?.test('release-12')).toBe(true);
  });

  it('strips g and y so a reused instance cannot alternate', () => {
    const re = compileSafeRegex('/release-\\d+/gy');
    expect(re?.flags).toBe('');
    expect(re?.test('release-1')).toBe(true);
    expect(re?.test('release-2')).toBe(true);
  });

  it('treats a bare pattern as flagless', () => {
    expect(compileSafeRegex('release-\\d+')?.flags).toBe('');
  });
});
