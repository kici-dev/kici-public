...
describe('sourceRegistrationSchema configPaths', () => {
  const base = {
    type: 'source.register' as const,
    messageId: 'm1',
    sources: [],
  ...
  it('accepts a registration that omits configPaths (older orchestrators)', () => {
    expect(sourceRegistrationSchema.parse(base).configPaths).toBeUndefined();
  });

  it('round-trips the reported paths', () => {
    const configPaths = {
    ...
    expect(sourceRegistrationSchema.parse({ ...base, configPaths }).configPaths).toEqual(
      configPaths,
    );
  });

  it('accepts a partially reported configPaths', () => {
    const parsed = sourceRegistrationSchema.parse({
      ...base,
      configPaths: { envFile: '/etc/kici/kici-orchestrator.env' },
    });
    expect(parsed.configPaths).toEqual({ envFile: '/etc/kici/kici-orchestrator.env' });
  });
});
