...
// --- Admin-token list request/response ---
//
// The orchestrator's `admin_tokens` rows, projected for the Platform's RBAC
// drift report. The report compares each token's recorded holder against the
// org's dashboard membership; it never converges the two layers, and the
// orchestrator never asks the Platform whether a token may act.
 ...
 */
export const dashboardAdminTokenSummarySchema = z.object({
  id: z.string(),
  label: z.string(),
...
export const dashboardAdminTokensListRequestSchema = z.object({
...
export const dashboardAdminTokensListResponseSchema = z.object({
  type: z.literal('dashboard.admin-tokens.list.response'),
  requestId: z.string(),
  tokens: z.array(dashboardAdminTokenSummarySchema),
  error: z.string().optional(),
});
export type DashboardAdminTokensListResponse = z.infer<
  typeof dashboardAdminTokensListResponseSchema
>;
  ...
  jobs: z
    .array(
      z.object({
        ...
        // Optional: an orchestrator that predates it sends nothing, and the
        // CLI renders the job's duration as unknown rather than failing.
