...
import {
  ...
  dashboardAdminTokenSummarySchema,
...
describe('REST response schemas (CLI reuse)', () => {
  ...
  /** The minimum an orchestrator row needs to parse, with no optional fields set. */
  const diagnosticsOrchestratorFixture = () => ({
  ...
  it('diagnosticsInfrastructureResponseSchema accepts an orchestrator with no configPaths', () => {
    const ok = diagnosticsInfrastructureResponseSchema.parse({
      orchestrators: [diagnosticsOrchestratorFixture()],
      alerts: [],
    });
    expect(ok.orchestrators[0].configPaths).toBeUndefined();
  });

  it('diagnosticsInfrastructureResponseSchema carries configPaths with null members', () => {
    const ok = diagnosticsInfrastructureResponseSchema.parse({
      orchestrators: [
        {
          ...diagnosticsOrchestratorFixture(),
          configPaths: {
    ...
    expect(ok.orchestrators[0].configPaths).toEqual({
      envFile: '/etc/kici/kici-orchestrator.env',
      scalerConfig: null,
      composeFile: null,
    });
  ...
  it('diagnosticsInfrastructureResponseSchema rejects an omitted configPaths member', () => {
    const result = diagnosticsInfrastructureResponseSchema.safeParse({
      orchestrators: [
        {
          ...diagnosticsOrchestratorFixture(),
          configPaths: { envFile: '/etc/kici/kici-orchestrator.env' },
        },
      ],
      alerts: [],
    });
...
describe('dashboard.admin-tokens.list', () => {
  const summary = {
    id: 'tok-1',
    label: 'alice-ops',
    role: 'admin',
  ...
  it('round-trips the request and registers it as a dashboard request type', () => {
    const req = dashboardAdminTokensListRequestSchema.parse({
      type: 'dashboard.admin-tokens.list',
      requestId: 'q1',
      actor: { type: 'user', sub: 'u1' },
    });
    expect(req.requestId).toBe('q1');
    expect(DASHBOARD_REQUEST_TYPE_SET.has('dashboard.admin-tokens.list')).toBe(true);
  });

  it('round-trips the response, carrying the routing-key scope beside the role', () => {
    const res = dashboardAdminTokensListResponseSchema.parse({
      type: 'dashboard.admin-tokens.list.response',
      requestId: 'q1',
      tokens: [summary, { ...summary, id: 'tok-2', subject: null, routingKey: 'github:42' }],
    });
    expect(res.tokens).toHaveLength(2);
    expect(res.tokens[1]!.subject).toBeNull();
    expect(res.tokens[1]!.routingKey).toBe('github:42');
  });

  it('never carries the token hash — the field is absent from the parsed summary', () => {
    // The load-bearing assertion. A hash is a credential verifier: relaying it
    // would put an offline-crackable value on the dashboard plane for a report
    // that has no use for it. Zod strips unknown keys, so a hash supplied on
    // the wire cannot survive the parse; adding a hash field to the schema
    // would make it survive, and this test is what fails in that case.
    const parsed = dashboardAdminTokenSummarySchema.parse({
      ...summary,
      token_hash: 'deadbeef',
      tokenHash: 'deadbeef',
    });
    expect(Object.keys(parsed)).not.toContain('token_hash');
    expect(Object.keys(parsed)).not.toContain('tokenHash');
    expect(Object.keys(dashboardAdminTokenSummarySchema.shape)).not.toContain('token_hash');
    expect(Object.keys(dashboardAdminTokenSummarySchema.shape)).not.toContain('tokenHash');

    const res = dashboardAdminTokensListResponseSchema.parse({
      type: 'dashboard.admin-tokens.list.response',
      requestId: 'q1',
      tokens: [{ ...summary, token_hash: 'deadbeef' }],
    });
