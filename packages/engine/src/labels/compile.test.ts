...
describe('toLabelMatcher — stateful regex flags', () => {
  it('drops g and y from an author RegExp', () => {
    expect(toLabelMatcher(/^linux-.*/g, 'job runsOn')).toEqual({
      kind: 'regex',
      source: '^linux-.*',
      flags: '',
    });
    expect(toLabelMatcher(/^linux-.*/gy, 'job runsOn')).toMatchObject({ flags: '' });
  });

  it('preserves a non-stateful flag', () => {
    expect(toLabelMatcher(/^linux-.*/gi, 'job runsOn')).toMatchObject({ flags: 'i' });
  });
});
