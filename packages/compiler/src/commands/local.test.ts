...
describe('kici local commands', () => {
  ...
  it('localLogsCommand prints the path on stdout and the rotation policy on stderr', async () => {
    ...
    const note = errSpy.mock.calls.flat().join('\n');
    expect(note).toContain('50 MB');
    expect(note).toContain('/tmp/x/orchestrator.log.1');
    expect(note).toContain('/tmp/x/orchestrator.log.pg');
  });
