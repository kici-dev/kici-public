...
describe('collectIdentity', () => {
  ...
  it('folds in every orchestrator the probe saw, not just the first', () => {
    ...
    expect(id.orchestrators[0]).toEqual({
      ...
      mode: 'platform',
      connected: true,
      configPaths: { envFile: null, scalerConfig: null, composeFile: null },
    });
  ...
  it('records each orchestrator config path the probe saw', () => {
    ...
    expect(identity.orchestrators[0].configPaths).toEqual({
      envFile: '/etc/kici/orch.env',
      scalerConfig: '/etc/kici/scalers.yaml',
      composeFile: null,
    });
  ...
  it('records null config paths for an orchestrator that reported none', () => {
    // fails-when: the mapping drops the key on absence — a reader could not
    // then tell "reported nothing" from "the collector forgot to look".
    const identity = collectIdentity(okProbe());
    expect(identity.orchestrators[0].configPaths).toEqual({
      envFile: null,
      scalerConfig: null,
      composeFile: null,
