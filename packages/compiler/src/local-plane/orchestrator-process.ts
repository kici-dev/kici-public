...
export function spawnOrchestratorProcess(
  ...
  // dev-signed identity vs hybrid's Platform attachment + own issuer) are
  // layered on below.
  ...
  if (opts.attach) {
    ...
    // SecretResolver, and OIDC/attestation mint with the orchestrator's own
    // signing key under the plane's own issuer (db custody, wrapped by the
    // plane's KICI_SECRET_KEY). The dev-signed identity envs are deliberately
    // NOT set — the choke point prefers orchestrator-owned signing.
