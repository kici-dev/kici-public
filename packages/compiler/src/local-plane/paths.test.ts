...
describe('local-plane paths', () => {
  ...
  it('names the Postgres log beside the orchestrator log', () => {
    // fails-when: pgLogFile is composed anywhere other than `<logFile>.pg`.
    // pg_ctl -l and the rotation must target the same byte-for-byte path, or
    // rotation silently does nothing to the file Postgres actually writes.
    process.env.KICI_CONFIG_DIR = '/tmp/cfgx';
    const p = planePaths();
    expect(p.logFile).toBe(path.join('/tmp/cfgx', 'local', 'orchestrator.log'));
    expect(p.pgLogFile).toBe(path.join('/tmp/cfgx', 'local', 'orchestrator.log.pg'));
  });
