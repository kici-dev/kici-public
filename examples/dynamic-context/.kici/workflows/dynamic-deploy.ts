...
/**
 * Dynamic context + dynamic env vars from the triggering event.
 *
 * Demonstrates that `job.context` and `job.env` accept callbacks that
 ...
 * context and a `prod` deploy target; anything else falls back to staging.
...
export default workflow('dynamic-deploy', {
  ...
  jobs: [
    job('deploy', {
      ...
      context: (event) => (event.targetBranch === 'main' ? 'production' : 'staging'),
