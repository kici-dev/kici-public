...
Runnable templates and example workflows, one subdirectory per topic:
...
| [`hetzner-autoscale/`](./hetzner-autoscale/)               | The host-side reaper for Hetzner autoscaling: a timer runs [`hetzner-autoscale/reap.ts`](./hetzner-autoscale/reap.ts) to delete every labelled server older than a TTL, the backstop when a teardown workflow never ran. It talks to the API through [`hetzner-autoscale/hetzner-client.ts`](./hetzner-autoscale/hetzner-client.ts) and runs with plain `node`. |
