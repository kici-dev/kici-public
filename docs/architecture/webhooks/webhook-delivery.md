...
One inbound webhook has a fixed budget of 8 seconds, measured from the moment
the request arrives — not per candidate. GitHub closes a delivery that has not
been answered within 10 seconds and counts it a failure, with manual
redelivery as the only remedy. Without a total budget, an org with several
unresponsive orchestrators spends 5 seconds per candidate. It answers long
after the provider stopped listening, and loses a delivery the buffer would
have replayed.

Each attempt therefore gets whatever is left of the budget, up to the 5-second
per-candidate ceiling; when the budget is spent the Platform stops trying
candidates, skips the cross-instance hop, and buffers.

A single delivery is also bounded in memory. The candidate loop re-sends the
whole body to each orchestrator it tries, so the cumulative bytes are capped.
A socket that has not drained its previous frames is skipped rather than
written to again.

### Step 3: Orchestrator processes webhook
...
### Step 4: Orchestrator dispatches jobs
...
5. The build agent clones the repo, runs `npm ci`, packs the `.kici/` source tarball (`source/v2/{orgId}/{sourceTarDigest}.tar.gz`, with a `{contentHash}.hash` pointer) and — if missing — the deps tarball (`deps/{platform}-{arch}/{depsHash}.tar.gz`, addressed by its own content hash, plus a `{lockfileHash}.hash` pointer), and uploads both via pre-signed PUT URLs
...
3. **Issues a clone token via the registration's bundle.** Calls `regBundle.cloneTokenProvider.createCloneToken(reg.repoIdentifier, reg.providerContext)`. **Fail-fast on error**: if issuance throws (revoked installation, expired app key, etc.), increment `kici_cross_source_errors_total{reason="clone_token"}` and skip the registration. Do **not** fall back to the inbound generic bundle — silently swapping bundles would leak credentials across providers. The token is only a check and is not kept. The run's stored credentials carry the registration's provider context alone. Each job's clone auth is minted at dispatch through the bundle of the job's routing key, which is the registration's.
4. **Delegates to `dispatchMatchedWorkflow()`.** Synthesizes a per-registration `WorkflowDispatchContext` and calls `dispatchMatchedWorkflow()` — the **same** helper the same-source path uses for every matched workflow. The helper uniformly handles static jobs, dynamic-fn workflows (`__dynamic__` eval jobs), `__init__` two-phase init dispatch (for static jobs with dynamic context / env / concurrencyGroup fields), `__build__` build coordinator integration with bundle + dep cache lookup, per-job context evaluation, protection rules, secrets resolution, sandbox capability grant resolution (per-job `sandbox` requests checked against the org's allow-list; a denied grant records a failed run), held runs, concurrency groups, and check run reporter wiring. The cross-source shell threads provenance (`crossSource: true`, `inboundRoutingKey`, `inboundEventName`, `workflowRepoUrl`, `workflowRef`, `workflowSha`, `workflowRepoIdentifier`) into every dispatched `jobConfig` via a wrapped dispatcher override (`ctx.extraJobConfig`), and uses effective overrides to ensure:
   ...
   - `providerContext` = the registration's context alone, with no clone token in it
...
### Ingest admission shed (Step 3)

**Trigger:** The orchestrator's ingest admission controller refuses a slot — event-loop lag is above the shed threshold, or a global / per-org concurrency cap is full. Relayed deliveries never queue for a slot, because the Platform awaits the ACK synchronously against a tight deadline.

**Result:** The orchestrator ACKs `shed_retry_later` and the Platform answers **429** with `Retry-After`. The shed happens before signature verification and before the pipeline, so the orchestrator writes an `event_log` breadcrumb with status `shed` naming the shed reason — a shed delivery stays distinguishable from one that never arrived. The breadcrumb is written only when absent, so a later replay upgrades the same row to its real outcome. When the durable ingest queue is on, the delivery is additively captured for automatic replay once capacity recovers. On the Platform's own replay path a shed leaves the delivery buffered for a later tick rather than draining it.

> Source: `packages/orchestrator/src/webhook/ingest-admission.ts`, `packages/orchestrator/src/webhook/shed-breadcrumb.ts`

### Invalid signature (Step 3)
...
### No orchestrator connected -- local instance (Step 2)
...
The cross-instance message carries a **pointer to the body, not the body
itself**: the receiving instance is told a short-lived key to read the bytes
from, and consumes that key exactly once. Every instance multiplexes its
subscriptions onto one connection. A large payload published inline would push
that connection past its output buffer. That costs the instance its whole
cross-instance plane — every relay, every acknowledgement, every dashboard
message — rather than just the one delivery. If the pointer has already
expired when the message is read, the receiving instance does not acknowledge:
the originating instance times out and buffers the delivery for replay.

### No orchestrator connected -- any instance (Step 2)
...
### ACK timeout -- single connection (Step 2)
...
### ACK timeout -- all connections (Step 2)
...
### No agent with matching labels (Step 4)
...
### Agent disconnect during execution (Step 5)
