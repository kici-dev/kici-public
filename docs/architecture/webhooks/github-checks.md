...
- **whole-run cancellation** (`cancel/cancel-run.ts`), which marks the pending
  and queued rows nobody is working `cancelled` — and, when no agent held
  outstanding work, every remaining non-terminal row too. A row whose agent was
  sent the cancel is left alone: the agent unwinds and reports its own terminal
  status, so that job goes through the tracker and its check run does resolve;
