...
description: Job dispatch, cancel, registration, log streaming, job and step status, heartbeats, cache upload, event emit, scaler credential claim, execution status forwarding
---
...
Ten orchestrator-to-agent fields describe a failure in free text: `error` on `event.emit.response`, `agent.api.response`, `artifacts.upload.response`, `artifacts.download.response` and `scaler.claim-credentials.response`, and `reason` on `step.approval-resolved`, `artifacts.upload.complete.ack`, `auth.failure`, `job.cancel` and `job.concurrency.ack`.
...
| upstreamJobStatuses      | Record<string, ExecutionJobStatus>      | No       | Terminal status of each upstream job, keyed by job name (per-child for fan-out). Powers `ctx.needs.<job>.status`.                                                                                                                                                                                                                                                         |
| upstreamInvokeResults    | Record<string, InvokeResult[]>          | No       | Per-invoke-gate results for any upstream gate this job `needs`, keyed by gate job name. One entry per run the gate triggered, carrying that run's non-secret declared outputs. Powers `ctx.needs['<gate>'].result` on a standard downstream job. Absent on orchestrators that predate it — the agent then resolves the gate need through the fan-out group shape instead. |
...
| containerRegistryAuth    | ContainerRegistryAuth                   | No       | Registry credentials for pulling this job's container image, already resolved by the orchestrator (the lock file carries secret **names**; the agent never resolves them itself). Absent on orchestrators that predate it — the agent then pulls anonymously.                                                                                                             |
...
| orgId                    | string                                  | No       | Org id that owns this run. Namespaces the user-facing cache so one tenant can never read another's entries.                                                                                                                                                                                                                                                               |
| repoId                   | string                                  | No       | Repo identifier (e.g. `owner/repo`). Second namespacing level for the user-facing cache.                                                                                                                                                                                                                                                                                  |
| cacheRefScope            | enum                                    | No       | Cache write scope for this job: `shared` lets it write the org-shared default-branch cache, `isolated` confines writes to a per-run scope while still allowing shared-scope reads. Absent is treated as `isolated` by the agent (fail-closed).                                                                                                                            |
...
| password      | string | Yes      | Resolved registry password or token           |
| serveraddress | string | Yes      | Registry host the credentials authenticate to |

InvokeResult: `{ repo, workflow, runId, status, outputs }` -- one summoned run's identity, terminal status, and declared outputs.
...
| warmPool        | boolean          | No       | When set, the orchestrator pre-spawned this agent for a warm pool rather than for a specific queued job. An agent that sees this flag must not arm any idle-shutdown timer on register: the orchestrator's warm-pool reaper owns its lifetime and either dispatches a job to it or destroys it. Absent for a job-bound spawn, for a static agent, and on orchestrators that predate warm pools. An agent that ignores the flag arms the short `KICI_SCALER_IDLE_TIMEOUT` timer as usual. |
...
| properties     | Record<string, string \| number \| boolean> | No       | Agent-reported typed host-vars (the `KICI_PROPERTIES` bag), shallow-merged into the host roster's host properties with agent-reported keys winning. Absent means none |
...
- A requeued job that no connected agent takes on the immediate re-dispatch is not stranded: a periodic per-coordinator sweep re-attempts delivery of every still-pending job onto a connected idle agent whose labels match, through the same atomic claim a normal dispatch uses. So a re-dispatch that transiently missed an eligible agent — because the agent was momentarily busy, or connected to a different coordinator — still lands the job on the next sweep rather than waiting for the job to expire.
...
| depsHash        | string                   | No       | SHA-256 of the dep tarball, so the orchestrator can sign a content-addressed URL. Deps uploads only. Absent on an older agent, which then gets a lockfile-keyed URL      |
| sourceTarDigest | string                   | No       | SHA-256 of the source tarball, so the orchestrator can sign a content-addressed URL. Source uploads only. Absent on an older agent, which then gets a lockfile-keyed URL |
| siblingsDigest  | string                   | No       | In-repo `workspace:` sibling closure digest; part of the dependency pointer key                                                                                          |
...
| sourceTarDigest | string                    | No       | SHA-256 of the source tarball (for agent-side integrity checking)               |
| siblingsDigest  | string                    | No       | In-repo `workspace:` sibling closure digest; part of the dependency pointer key |
...
A `fleet.bundle.chunk` whose `requestId` names a collection the orchestrator is still waiting on from that agent is exempt from the connection's rate limiter, the same way a heartbeat is. The agent streams its whole bundle as one burst, and `maxBytes` lets that bundle exceed the limiter's byte burst many times over. A throttled frame would leave a gap the assembler rejects as out of order. A chunk that names no such collection is rate-limited like any other frame.

## Connection rate limiting

Every agent connection carries a per-connection rate limiter on message count and byte throughput. Heartbeats and solicited fleet bundle chunks are exempt.

### Orchestrator -> Agent

#### rate.limit.warning

Sent instead of processing a frame the limiter refused. The frame is dropped; the agent should wait before sending more. A sustained overrun closes the connection instead of warning.

| Field        | Type                   | Required | Description                                                        |
| ------------ | ---------------------- | -------- | ------------------------------------------------------------------ |
| type         | `"rate.limit.warning"` | Yes      | Message discriminator                                              |
| retryAfterMs | number                 | No       | Estimated wait until the limiter admits another frame of that size |

## Scaler credential-claim messages

These messages carry the event-scaler provisioning claim. The event scaler emits a `kici.scaler.scale-up` event carrying a single-use `claimCode` -- never a token. The provisioning workflow forwards that code to the instance it boots. The agent exchanges the code over this round-trip for a freshly minted ephemeral agent token, then registers with it. A workflow can also exchange the code itself and deliver the token to the instance. The token is therefore minted lazily, only when a real provision claims it, and it appears in the response frame only -- never in the persisted event log.

The workflow-facing surface is `ctx.kici.scaler.claimAgentCredentials(claimCode)`; see [Event scaler](../../operator/orchestrator/event-scaler.md).

> Authoritative source: `packages/engine/src/protocol/messages/orchestrator-agent.ts`

### Agent -> Orchestrator

#### scaler.claim-credentials

Exchanges a single-use claim code for ephemeral agent credentials. The claim code is the authorization, so a fresh agent can send this message before it authenticates or registers. An already-registered agent can also send it.

| Field     | Type                         | Required | Description                                     |
| --------- | ---------------------------- | -------- | ----------------------------------------------- |
| type      | `"scaler.claim-credentials"` | Yes      | Message discriminator                           |
| requestId | string                       | Yes      | Correlates the response                         |
| claimCode | string                       | Yes      | Single-use code from the scale-up event payload |

> Authoritative source: `packages/engine/src/protocol/messages/orchestrator-agent.ts` -- `scalerClaimCredentialsSchema`

### Orchestrator -> Agent

#### scaler.claim-credentials.response

The minted credentials for a claimed provision, or an error. Exactly one of `credentials` / `error` is present.

| Field       | Type                                               | Required | Description                                              |
| ----------- | -------------------------------------------------- | -------- | -------------------------------------------------------- |
| type        | `"scaler.claim-credentials.response"`              | Yes      | Message discriminator                                    |
| requestId   | string                                             | Yes      | Correlates to the originating `scaler.claim-credentials` |
| credentials | `{ agentToken, agentId, orchestratorUrl, labels }` | No       | Minted credentials (on success)                          |
| error       | string                                             | No       | Failure description (on failure)                         |

`error` carries the deliberate claim verdicts verbatim -- the code is unknown, already consumed, or past its `claimTtlSeconds` window -- because each names only the caller's own claim. It also carries a fixed string when the orchestrator has no event scaler configured. Any other failure is a safe fixed string, never raw exception text.

The claim is consumed before the token is minted, so a concurrent replay of the same code sees it consumed and cannot double-mint.

> Authoritative source: `packages/engine/src/protocol/messages/orchestrator-agent.ts` -- `scalerClaimCredentialsResponseSchema`
...
Each method carries a required role -- `read` or `write` -- checked against the calling agent's own roles before the handler runs. Several are registered only when the orchestrator has the dependency the method needs. An agent that calls one on an orchestrator which did not register it gets the ordinary "unknown method" error, never a partial result.

| Method                     | Role    | Registered when                                                                                     | Purpose                                                                           |
| -------------------------- | ------- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `infrastructure.list`      | `read`  | Always                                                                                              | Lists the orchestrator's configured scaler backends                               |
| `inventory.query`          | `read`  | A host roster is configured                                                                         | Queries the cluster's host roster (backs `ctx.kici.inventory`)                    |
| `inventory.get`            | `read`  | A host roster is configured                                                                         | Reads one host's roster entry                                                     |
| `host.requestReboot`       | `write` | A host roster is configured                                                                         | Marks a host reboot-pending so the imminent disconnect is expected, not a failure |
| `host.cancelReboot`        | `write` | A host roster is configured                                                                         | Clears the reboot-pending flag                                                    |
| `kici.ensureInitRunner`    | `write` | A host roster is configured **and** the token store, secret resolver and access log are all present | Bring-up: ensures the init runner exists on a target host                         |
| `kici.preBootSend`         | `write` | Same as above                                                                                       | Bring-up: returns the scoped transport material for pushing a fresh box           |
| `kici.presignAgentPackage` | `write` | Same as above                                                                                       | Bring-up: pre-signs the agent package download                                    |
| `kici.agentVersionStatus`  | `write` | Same as above                                                                                       | Fleet upgrade: reports a host's target and staged agent version                   |
| `kici.restageAgent`        | `write` | Same as above                                                                                       | Fleet upgrade: re-stages a host's agent once availability allows                  |
| `oidc.token.request`       | `read`  | An identity-token mint is available (see below)                                                     | Mints a short-lived build-provenance ID token bound to the calling job            |
| `git.credential.request`   | `read`  | A secret resolver is configured                                                                     | Mints a scoped, short-lived git credential for one network operation              |

The bring-up and roster methods take `write` because they change host state; every read-only lookup and every mint bound to the caller's own job takes `read`.
...
| Field    | Type   | Required | Description                                 |
| -------- | ------ | -------- | ------------------------------------------- |
| jobId    | string | Yes      | The job the calling agent is executing      |
| audience | string | Yes      | Requested token audience (1-255 characters) |

Result -- either a minted token, or a deferral:

| Field     | Type   | Required | Description                                             |
| --------- | ------ | -------- | ------------------------------------------------------- |
| token     | string | Yes      | The minted short-lived JWT                              |
| expiresIn | number | Yes      | Token lifetime in seconds                               |
| jti       | string | Yes      | Token identifier, used to correlate the mint downstream |

| Field    | Type   | Required | Description                                                                                      |
| -------- | ------ | -------- | ------------------------------------------------------------------------------------------------ |
| deferred | `true` | Yes      | The mint failed transiently; the agent freezes the statement and reports it for later fulfilment |
| code     | enum   | Yes      | `unavailable` or `failed`                                                                        |

A permanent rejection is thrown rather than returned, so it never appears as a deferral.

> Authoritative source: `packages/engine/src/protocol/messages/oidc-token-relay.ts`

#### git.credential.request

Mints a git credential for a single network operation. The agent's credential helper calls this on every fetch and push, so a token is always minted seconds before use. That is what lets an arbitrarily long job keep pushing past the point where a credential captured at checkout would have expired.

A repository must be named explicitly: wildcards are rejected by the schema. A write grant is therefore always a specific repository written down in the workflow source and reviewable in a diff.

Authorization runs in two layers before anything is minted. Job ownership comes first -- an agent may only request credentials for a job it was actually dispatched, resolved from server truth rather than from the parameters. Then the repository fence, which applies to **write** only. Every named repository must sit inside the organisation that owns the job's source repository. One out-of-organisation name refuses the whole request, rather than minting a token that covers both. A read needs no fence, because naming a repository the credential's own installation does not cover yields nothing.
...
| ------------ | ---------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| jobId        | string                 | Yes      | The job the calling agent is executing                                                                                                           |
| repositories | string[]               | Yes      | Every repository the credential must cover, most-specific first. A list because one app installation token can name several repositories at once |
| ref          | GitCredentialRef       | No       | Which credential to mint from. Absent falls back to the source credential the orchestrator already holds for the job                             |
| permissions  | Record<string, string> | No       | Requested permissions, passed through verbatim for the forge to accept or reject                                                                 |

GitCredentialRef is one of three shapes, each naming its material as either a qualified `<context>:<secret-name>` reference or a runtime value, exactly one per pair:

| kind    | Fields                                     |
| ------- | ------------------------------------------ |
| `app`   | `appId*`, `installationId*`, `privateKey*` |
| `token` | `token*`, optional `user`                  |
| `ssh`   | `privateKey*`                              |

The schema rejects a reference that looks like the credential itself rather than the name of a secret holding it. Pasting a key or token where a name belongs is the easy, silent mistake, so it is caught on the wire as well as in the workflow source.

Result:

| Field     | Type           | Required | Description                                                                                              |
| --------- | -------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| kind      | enum           | Yes      | `basic` (HTTPS) or `ssh`                                                                                 |
| user      | string         | No       | Basic-auth username. Omitted for SSH                                                                     |
| secret    | string         | Yes      | The minted credential                                                                                    |
| grant     | object         | Yes      | `{ scoped: false }`, or `{ scoped: true, permissions }` naming exactly what the credential was scoped to |
| expiresAt | string or null | Yes      | Expiry timestamp, or null for a credential with no expiry                                                |

> Authoritative source: `packages/engine/src/protocol/messages/git-credential-relay.ts`
...
| workflowRepoIdentifier | string               | No       | Repository that DEFINES the workflow, when that is not `repoIdentifier` — a global workflow authored in one repository and dispatched against another. Absent for every per-repository run                                                                    |
| isGlobalEvalRound      | boolean              | No       | True when the run records a global evaluation round rather than a workflow. Mirrored upstream so a re-run of a round re-evaluates the original event instead of resolving a workflow out of the acted-on repository's lock file. Absent means an ordinary run |
...
| jobKind        | enum                    | No       | What this job **is**, orthogonal to its status: `standard` (ordinary job), `gate` (an invoke gate), `proxy` (a per-run child mirroring one summoned run). Absent reads as `standard`, so an older peer that ignores it is unaffected |
| summonedRunId  | string                  | No       | For a `proxy` job, the id of the summoned run it mirrors                                                                                                                                                                             |
...
| repoIdentifier         | string         | No       | Repository identifier (e.g., `owner/repo`)                                   |
| workflowRepoIdentifier | string         | No       | Repository that DEFINES the workflow, when that is not `repoIdentifier`      |
| sha                    | string         | No       | Commit SHA                                                                   |
