...
`dynamicJob` is polymorphic. The function form is unchanged (event-only, dispatched at webhook time). An options-object form `{ needs, generate, gitCredentials }` marks a result-aware generator when it declares `needs`; without `needs` it is dispatched at webhook time like the function form. The options form is also the only place a generator declares `gitCredentials` — the named git credentials every job it generates inherits. The lock file's dynamic entry carries that map. The orchestrator copies it onto each generated child from the lock entry alone, never from the generated job definitions the agent's dynamic-eval reply returns, so job code cannot declare its own authorization.
