...
- **The workflow install gate.** A `registries:` or `installEnv:` protection rule returned hold, wait, or queue. Release comes from a reviewer approving, a wait timer expiring, or a concurrency slot freeing up, and the run resumes into a fresh dispatch that flips it back to `pending`. The resumed dispatch checks again every install context the approval did not cover. If one no longer admits the run, or a named context now matches a different row, the run fails before any job starts.
...
A hold recorded for a run that already finished is not forwarded either. A run leaves a terminal status in one case. It failed before a job it had already dispatched reported in, and that job then reports progress. The tracker reopens such a run to `running` only after the job report was applied to the job row, so a replay of a report the row already records never reopens a finished run. A run that failed on a build failure is never reopened. Each reopen raises the run's status generation (`execution_runs.status_epoch`), which every forward carries as `statusEpoch`. The Platform applies a frame's status by generation, so a frame that arrives late or belongs to a generation the run has left cannot move the run back ([late status frames](../protocol/orchestrator-agent.md#late-status-frames)).
