...
Five special job types skip the step sandbox, because they never execute customer workflow steps. The agent checks for them in this order:
...
The agent's `JobRunner.runJob()` manages the outer lifecycle. Customer code runs in a child process, never in the agent: step bodies in the step runner, and every module load and evaluation in the eval child. Both children are forked with a sanitized environment that carries no agent credential, and the agent removes its own credentials from `process.env` after boot.
