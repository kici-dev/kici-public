...
### Two children, and no in-process route

Customer-authored code runs in one of two child processes. The agent process
itself never loads a workflow module.

- **The step runner** (`workflow-runner.js`) is the only execution path for
  customer **step** code. It runs under the selected backend — container,
  bare-metal (optionally bubblewrap), or Firecracker — and its IPC surface
  carries the job's relays: `event.emit`, the agent API, the user cache,
  provenance, artifacts, git write grants, and step approvals.
- **The eval child** (`eval-runner.js`) is the only execution path for customer
  **evaluation** code: the workflow module load, a workflow `filter`, the
  dynamic `env` / `environment` / `concurrencyGroup` / `matrix` functions, a
  `DynamicJobFn`, and a global eval round. The agent forks it with the same
  sanitized environment builder the step runner uses, so it holds no agent
  credential.

The eval child has its own IPC union rather than the step runner's, and it is
deliberately narrower: `ready`, `log.line`, `eval.result`, `eval.error`, and one
relay for `ctx.kici`. Customer code that runs in a child has arbitrary code
execution _in that child_, so it can send any message the agent-side handler for
that child accepts. Sharing the step runner's union would hand a `matrix`
function reachability to the cache, provenance, artifact, git-grant and
step-approval relays. The `ctx.kici` relay is present because a `DynamicJobFn`
and a global-eval generator are already handed that API; nothing else is.

The eval request carries no secrets: no evaluation surface consumes them.
...
- The rules above sit on the `forward` hook, so they govern what a container reaches **through** the host. A packet addressed to one of the host's own addresses arrives on the `input` hook instead and crosses no forward rule, so a second per-container rule set on that hook governs what the container reaches **on** the host: the orchestrator's own port, DNS on the bridge gateway, and the object-storage endpoint the orchestrator directed the agent at. Everything else host-destined is dropped. Per-label-set `networkPolicy.hostAccess` replaces that set

### Nested job containers

A job that names a `container:` image runs in a container the agent creates, and that image supplies its own `ENTRYPOINT` — which starts the moment the container starts. So the drops for these containers are keyed on the network, not on a container:

- Job containers are attached to a dedicated bridge network (`kici-jobs`, subnet 172.31.0.0/16)
- One nftables rule set matches `ip saddr 172.31.0.0/16` and blocks the same RFC1918 ranges and cloud metadata endpoints. It is installed when the agent prepares the network, before any job container is created, and covers every container that later joins the bridge
- The set is installed once per host, so a host running many jobs holds one set rather than one per job. The install is skipped only when the chain already carries every rule in the set: a partial set, which a failed install leaves behind, is reinstalled rather than read as covered
- The same two-hook split applies: a job container reaches DNS on the bridge gateway and no other host service. That set is installed for the whole subnet before any job container exists, and again per container at spawn. It takes no argument a workflow could supply, so a job cannot widen its own host reachability
- A per-container rule set is still added at spawn and removed at teardown. Its forward half carries no policy today; it is the place a future per-job allowlist would be keyed, and because rules are inserted at the chain head, a per-container `accept` is evaluated ahead of the subnet `drop`. A repair puts a fresh subnet set at the head, above the per-container rules of jobs already running — harmless while those rules carry no policy, since both sets then hold the same verdicts
- The bridge may already exist under this name with a different subnet. The agent then installs the drops for the range that bridge actually uses and warns, rather than refusing the network — refusing would put the job container on the runtime's default bridge with no filtering at all
- A host that cannot program nftables gets neither the rules nor the bridge: the agent warns and the job container keeps the runtime's default bridge with unfiltered egress
...
- Per-VM `input`-hook rules govern what the VM reaches on the host itself, on the same defaults and the same `networkPolicy.hostAccess` override as the container backend
...
| Every host address and port not in `hostAccess` | Prevent access to services running beside the agent or the orchestrator, which the forward rules above never see |
