...
4. **Build-context cross-check.** The statement's build context must match the
   ...
   deferred ones included: a deferred token carries the same `repository` /
   `ref` / `sha` / `workflow_ref` claims a live one does, because deferral marks
   the mint timing and does not change what the claims say. A deferred
   attestation must additionally satisfy the statement-hash binding below —
   both, never either.
...
1. **Freeze at build time.** The agent builds the statement from the **build
   ...
   A job's checkout ref is a pull request's HEAD branch, where the claim is the
   BASE branch. A global workflow's clone ref is not the `<name>@<sha>` workflow
   reference. An agent that receives no context falls back to that local view,
   and its capture is refused at step 2.

2. **Cross-check, then capture to a durable outbox.** On a transient mint
   failure the agent reports the frozen envelope, ephemeral public key, and a
   `statement_hash` to the orchestrator. The orchestrator then checks that
   statement against its own run row: it recomputes the hash, compares the build
   context against the claims the later mint will carry, and confirms the subject
   digest. Only then does it record a row in the cluster-shared
   `pending_attestations` outbox, storing its OWN recomputed hash. A statement
   that fails any check is dropped. A permanent rejection (a genuinely bad
...
- **Statement-hash binding.** The deferred identity token commits to the frozen
  statement by hash, so the identity cannot be re-bound to a different artifact
  at retry time. A verifier recomputes the hash and hard-fails on a mismatch.
  This is a binding **in addition to** the build-context cross-check, not
  instead of it. The hash proves the statement was not swapped; the cross-check
  proves the statement agrees with the run the token names. A hash-only rule
  accepted a statement naming a release commit the build never touched, because
...
- **Checked before it is stored.** The orchestrator cross-checks a captured
  ...
  it has not checked. `kici_orch_deferred_attestation_rejected_total{reason}`
  counts the refusals.
- **The mint-timing marker is read from the signed claim only.** The frozen
  statement also records an `attestationOrigin` in its internal parameters, but
