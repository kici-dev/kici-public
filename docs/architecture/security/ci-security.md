...
The no-policy state is not necessarily brief. The Platform pushes `trust_policy.update` on every authenticated handshake, and for an organization with no stored row it pushes its own defaults — whose fork switch is `ignore` too. So an organization that has never opened **Settings > CI trust** holds `ignore` for as long as nobody chooses otherwise, and every fork pull request it receives is dropped.
...
Leaving nothing on the pull request is deliberate. It is what stops an unvetted contributor driving outbound credentialed calls. But it also means a maintainer sees a pull request CI never touched, and no reason anywhere.

So the drop is explained on the surfaces an **operator** reads. The delivery's event-log row carries a message naming the fork switch, whether the organization chose it, and what did not happen. `kici-admin trust-policy show` warns while the switch is `ignore`. The dashboard's CI trust page shows a notice for as long as the saved policy drops.
...
An organization whose policy row was never created receives the push too, carrying the Platform's own defaults. Its orchestrator applies the documented `ignore` fork switch. The identity links, `ci_trust` levels and team memberships ride in the same message, so a commenter still resolves and `/kici approve` still works.
