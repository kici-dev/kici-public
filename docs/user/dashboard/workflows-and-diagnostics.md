...
3. **Infrastructure tree** -- hierarchical view of orchestrators, their scalers, and agents. Refreshes every 10 seconds. Each orchestrator row shows:
   ...
   - An **info popover** on the row lists that orchestrator's registration details. It ends with a **Config files** block when the orchestrator reports where its own config files live. The block names the env file, the scaler config, and the compose file, each with a copy button. A path the orchestrator did not report gets no row. An offline orchestrator reports none at all, so the block does not appear for it. Neither does a worker peer: it runs on its own host, and its heartbeat carries no config paths. Every path is a host path on the machine that runs the orchestrator, so it may not resolve on yours.
