...
| `options.filter`      | `FilterFn`                                                             | no       | Pre-dispatch predicate deciding whether the workflow applies to the event's source repo. A `false` result suppresses the workflow's jobs -- none is dispatched and none is reported as skipped. See [Global workflows](../global-workflows.md). |
...
| `options.approval`    | `ApprovalConfig`                                                       | no       | Pause for a manual human approval before the whole workflow dispatches. See [Approval gates](../approvals.md).                                                                                                                                  |
...
| `options.runsOnAll`            | `RunsOnAllInput`                                                                 | yes (or `runsOn`)     | Host fan-out -- one pinned execution per roster host matching the predicate. Mutually exclusive with `runsOn`. See [runsOnAll host fan-out](./runs-on-all.md).                                                                                                                    |
...
| `options.needs`                | `Array<Job \| string \| DynamicGroupRef \| { name; when? } \| { group; when? }>` | no                    | Job dependencies (must complete first) -- see [Job dependencies (`needs`)](#job-dependencies-needs)                                                                                                                                                                               |
...
| `options.invoke`               | `InvokeConfig`                                                                   | no                    | Run the source repo's opt-in workflows and gate on them, built with `invokeSource()`. An invoke job needs no `runsOn` and is mutually exclusive with `steps` / `run`. See [Invoking a source repo's own workflows](../global-workflows.md#invoking-a-source-repos-own-workflows). |
| `options.checkout`             | `boolean`                                                                        | no (default: `true`)  | When `false`, agent skips git clone. Useful for deploy/notify jobs.                                                                                                                                                                                                               |
| `options.gitCredentials`       | `GitCredentialMap`                                                               | no                    | Named git credentials every step of the job can use. Values name secrets, never secret material. See [Git credentials](../patterns/git-credentials.md).                                                                                                                           |
| `options.container`            | `string \| ContainerConfig`                                                      | no                    | Docker image for job execution. String form is the image name; object form builds from a `dockerfile` instead and adds `env` and registry `auth`. All steps run inside the container. See [Container jobs](../container-jobs.md).                                                 |
...
| `options.context`              | `string \| ((event) => string \| Promise<string>)`                               | no                    | Bound context for this job -- the secret / variable scope it resolves against. Static string or async/dynamic function -- see [Contexts](../contexts.md) and [Dynamic values](../dynamic-values.md).                                                                              |
...
| `options.concurrencyGroup`     | `string \| ((event) => string \| Promise<string>)`                               | no                    | Concurrency group name. Defaults to the first bound context's name -- see [Concurrency](../concurrency.md).                                                                                                                                                                       |
...
| `options.cache`                | `CacheInput`                                                                     | no                    | Declarative cache: restored before steps, saved after the job on a key miss. See [Caching](./caching.md).                                                                                                                                                                         |
| `options.approval`             | `ApprovalConfig`                                                                 | no                    | Pause for a manual human approval before this job dispatches. See [Approval gates](../approvals.md).                                                                                                                                                                              |
...
Case folding covers labels and hostnames only. An **agent ID** stays an opaque identifier and compares exactly, which is what keeps a per-host secret binding on `prod-01` away from an agent named `PROD-01` — see [per-host secret scoping](../../operator/security/secrets.md#per-host-secret-scoping).
...
| ----------------- | --------------------------- | -------- | ------------------------------------------------------------------------------------------------------- |
| `run`             | `(ctx) => Promise<TResult>` | yes      | The step body. With a `check` facet, `run` is the apply function (see below).                           |
| `outputs`         | `OutputSchema`              | no       | Zod schema for the step's outputs, validated at run time. See [Step output types](#step-output-types).  |
| `continueOnError` | `boolean`                   | no       | When `true`, the job proceeds even if this step fails. The step is still recorded as failed.            |
| `timeout`         | `number`                    | no       | Step wall-clock timeout in milliseconds. Unset, the agent default applies. See [Timeouts](#timeouts).   |
| `retry`           | `number \| RetryConfig`     | no       | Re-run a thrown step with backoff. See [Retries](#retries).                                             |
| `cache`           | `CacheInput`                | no       | Declarative cache: restored before the step, saved after it on a key miss. See [Caching](./caching.md). |
| `rules`           | `Rule[]`                    | no       | Step-level conditions, evaluated on the agent.                                                          |
| `onCancel`        | `HookInput`                 | no       | Hook that runs when the step is cancelled.                                                              |
| `cleanup`         | `HookInput`                 | no       | Hook that always runs after the step (success, failure, or cancel).                                     |
| `approval`        | `ApprovalConfig`            | no       | Pause for a manual human approval before this step, or on drift. See [Approval gates](../approvals.md). |
