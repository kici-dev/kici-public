...
interface StepContext<TInputs = Record<string, unknown>> {
  ...
  isTestRun: boolean;
  /** The job's own checked-out repository (present for every job that checks out); `withWrite` opens a write window for it — see [Git credentials](../patterns/git-credentials.md) */
  repo?: {
    identifier: string;
    path: string;
    ref?: string;
