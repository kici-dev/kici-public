...
- **Test before pushing** — `kici preview pr:open` previews which workflows would fire for a given event; `kici run <event> --local` executes a workflow against your laptop without any orchestrator deployment. See [Getting started](../getting-started.md). To run your local working tree through _this_ orchestrator without a git push, use `kici run remote` (step 5 above) — it works out of the box here thanks to the bundled SeaweedFS store. See the [testing guide](../testing-guide.md).
...
- **Run isolated** — Firecracker microVM execution for untrusted code or per-job isolation. See [Firecracker host setup](../../operator/orchestrator/firecracker/host-setup.md).
...
**Orchestrator logs show `Platform auth failed` immediately after start.** The `KICI_PLATFORM_TOKEN` in `.env` doesn't match what you minted at app.kici.dev. Mint a fresh one (the old one stays revokable from the dashboard) and update `.env`, then `docker compose restart orchestrator`.
...
<!-- kici-lint-allow-github-runner: troubleshooting note deliberately contrasts KiCI labels with GitHub's ubuntu-latest -->
