...
Read by the rotated-file logger (`packages/core/src/logger.ts`) before any service config loads. Setting `KICI_LOG_DIR` enables the JSON file transport; the instance-id env vars only act as a filename suffix (so multiple processes can share one `KICI_LOG_DIR`).
...
| `KICI_TMPDIR`               | no       |         | string                 |         | Base directory for KiCI-created temporary files (repo clones, build scratch, deploy render dirs). Defaults to the operating system temp directory. Set it to a path on a volume with free space when the default temp filesystem is small.                                                                                                  |
