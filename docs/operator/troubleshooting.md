...
- `shed` — the ingest admission controller refused the delivery under load
  before the pipeline ran. The row's `error_message` names the shed reason
  (`loop_overload`, `codel_drop`, `queue_full`, `max_sojourn`). The orchestrator
  captures the delivery in its durable overflow queue and replays it once
  capacity recovers. So `shed` is a snapshot, not a terminal outcome: a
  successful replay overwrites the row with the real result. A row that stays
  `shed` means the replay never converged.
...
- `shed`: no action while the orchestrator is under load — the replay clears it.
  A row that stays `shed` means the overflow queue never drained it; read the
  matching `ingest_overflow_buffer` row's `last_error`.
- `duplicate`: expected on provider retries; no action.
- `received`: no action when the event type has no trigger. When a run
