...
**Source tarball** is keyed by the workflow's `contentHash` — `SHA-256(compileSchemaVersion + ":" + treeDigest [+ "\0" + assetDigest])`, where `treeDigest` is a digest over every file under `.kici/` except the paths declared in `.kici/.kiciignore` (see [files the content hash skips](../user/lock-file-and-drift.md#files-the-content-hash-skips-kicikiciignore)):

Key format: `source/v2/{orgId}/{sourceTarDigest}.tar.gz`, with a `source/v2/{orgId}/{contentHash}.hash` pointer

The tarball is **platform-agnostic** — raw `.kici/` source files are identical across architectures, so one entry serves linux-x64, linux-arm64, darwin-arm64, and win32-x64 agents alike. `contentHash` changes when any file under `.kici/` that `.kici/.kiciignore` does not exclude changes, or any file listed in `hashFiles([...])` changes.
...
| ------------ | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | -------------------------- |
| Source cache | `source/v2/{orgId}/{sourceTarDigest}.tar.gz`                                 | Raw `.kici/` directory minus `node_modules/` (deterministic gzip tar)                                        | Any non-ignored `.kici/` file or any `hashFiles` asset changes; `COMPILE_SCHEMA_VERSION` bump               | Shared (platform-agnostic) |
...
- Source tarball hash (the SHA-256 of the tarball bytes) equals across builders for the same `.kici/` source. The cache key itself, however, uses the workflow `contentHash` (a digest of the `.kici/` tree + optional asset digest) — so the cache key is independent of tar encoding and stable even across portability edge cases.
