...
Mode: hybrid (written by `kici-admin orchestrator install`)
  ...
  Deduplication prevents double-processing, so a Platform outage costs no push.
  Requires KICI_PLATFORM_URL and KICI_PLATFORM_TOKEN — the same pair `platform`
  requires, and nothing more. Per-source webhook secrets for direct ingestion
  live in the orchestrator DB
  (kici-admin source add ... → orchestrator-side sources table rows).
  Set KICI_WEBHOOK_PUBLIC_URL to advertise the direct route; until you do, the
  route is served but nothing posts to it.
...
| ------------- | ------------------------------------------------------ | ------------------ | --------------- | ----------------------------------------------------------------- |
| `hybrid`      | Standard deployment; resilience against a relay outage | Yes                | Yes             | Platform API key + per-source secrets                             |
| `platform`    | Relay only, when you cannot expose an ingress          | Yes                | No              | Platform API key                                                  |
...
Relay only. Webhooks flow through the KiCI Platform relay and nowhere else. Pick
this when you cannot expose an ingress; otherwise prefer hybrid, which adds the
direct route at no extra configuration cost.
...
Receives webhooks from both the Platform relay and direct per-source webhook
endpoints. This is what `kici-admin orchestrator install` writes, because it costs
nothing over `platform` and removes the relay as a single point of failure: GitHub
does not retry a delivery the Platform failed to accept, so a relay-only
orchestrator loses every push in an outage window.
