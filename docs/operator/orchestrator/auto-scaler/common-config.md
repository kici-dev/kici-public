...
globalMaxAgents: 50 # Optional. Cap across all backends of THIS orchestrator. Default: 50.
...
labelSets:
  - labels: ['linux', 'container'] # Required. Label set (min 1 label, no 'kici:' prefix).
    ...
    networkPolicy: # Optional. Network isolation policy for this label set.
      ...
      hostAccess: # Optional. What the agent may reach ON the orchestrator host.
        - '5000' # Replaces the default set — see "Network policy" below.
...
`globalMaxAgents` binds **per orchestrator instance**, so an N-coordinator
cluster can run up to N times its value. The exception is a scaler of type
`event`, whose `maxAgents` is claimed cluster-wide. Both count caps are
enforced at admission: an agent that has been admitted but whose provisioning
has not started yet still counts, so a burst of queued jobs cannot overshoot
the cap.

In addition to the agent-count cap (`maxAgents` per-scaler, `globalMaxAgents` per-orchestrator), the auto-scaler enforces three optional CPU+memory caps. They stack: a spawn must pass every cap that applies to it.
...
labelSets:
  - labels: [default]
    ...
    networkPolicy:
      ...
      hostAccess: # What the agent may reach on the orchestrator host
        - '5000' # A host-local registry mirror, on any host address
```
...
| `hostAccess` | string[] | see below | What the agent may reach on the orchestrator's own host |

### `hostAccess`: what an agent reaches on the host

`allowlist` and `denyAll` govern what an agent reaches **through** the host —
the internet, and other machines on your network. `hostAccess` governs what it
reaches **on** the host: the orchestrator's own port, a registry mirror you run
beside it, an object store, a metrics sink.

The two are separate because the operating system delivers the two kinds of
traffic on different hooks. A packet addressed to one of the host's own
addresses never traverses the forwarding path the other two fields filter. So
before `hostAccess` existed, an agent reached every port on every address of its
orchestrator host.

Each entry is `<cidr|address|*>` with an optional `:<port|*>`, and a bare number
is a port:
...
addresses, and a name resolved when the rule is written goes stale the next time
it moves, with nothing to tell you.

**Defaults.** With no `hostAccess`, an agent reaches the orchestrator's port,
DNS on its bridge gateway, and the object-storage endpoint the orchestrator
directed it at — nothing else on the host. A nested **job** container, which
runs your workflow's own steps, reaches DNS and nothing else; it talks to its
agent over the container runtime rather than the network.

Setting `hostAccess` **replaces** the default outright rather than adding to it,
so a label set that names a registry mirror must also name the orchestrator's
port and DNS:

```yaml
networkPolicy:
  hostAccess:
    - '10143' # the orchestrator's agent port
    - '172.30.0.1:53' # DNS on the agent network's gateway
    - '5000' # the registry mirror this label set needs
```

An empty list (`hostAccess: []`) denies every host service.
...
| Firecracker                | Per-VM nftables (IP saddr)        | Yes (via NAT)   | Plus bridge port isolation, so one VM cannot reach another  |
| Bare-metal, container mode | Per-container nftables (IP saddr) | Yes (via NAT)   | Same isolated network and rules as the container backend    |
| Bare-metal, process mode   | None (not supported)              | Yes             | Agents run as child processes with full host network access |

The container backend applies per-container nftables rules using the container's IP address on the isolated `kici-agent-net` network. Each container gets its own set of rules (RFC1918 block + allowlist + optional denyAll) that are created during spawn and cleaned up during destroy. Bare-metal container mode joins the same network and gets the same rules.

The Firecracker backend applies per-VM nftables rules matching on the VM's source address. Rules are created after TAP device setup and cleaned up before TAP device deletion during destroy.

Bare-metal **process** mode has no network isolation. Agents run as child processes with full host filesystem and network access. If a label set has `networkPolicy` configured, a warning is logged at startup. This mode is intended for trusted environments only.

### Where a policy sits in the rule order

A per-agent rule is always evaluated before the host baseline, which the chain
reaches through a `jump` as its last rule. Two consequences worth stating,
because both were false in earlier releases:

- **`denyAll: true` denies.** The per-agent drop is reached before the
  baseline's blanket internet accept. An operator upgrading from a release
  where `denyAll` silently granted full egress will see previously-working
  outbound calls start to fail — which is the configured policy taking effect.
- **An `allowlist` entry inside a denied range is reachable.** The accept
  precedes the RFC1918 drops, so allowlisting a `10.x` registry works.
