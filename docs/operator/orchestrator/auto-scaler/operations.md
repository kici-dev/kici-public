...
- **Bare-metal backend in a container** is unusual. Because child processes inherit the orchestrator container's filesystem and namespaces, "bare-metal" agents launched from inside a container are container-local processes, not host processes — which defeats the usual reason to pick bare-metal (host hardware / GPU access). If you need agents on the host, run the orchestrator on the host.
...
- Labels are case-insensitive: `["Linux"]` matches `["linux"]`. This holds at every step — backend selection, the `mandatoryLabels` gate, and the match against a spawned agent's own labels.

**Labels are stored and displayed lowercase.** The orchestrator folds every label it receives, from the scaler config and from a registering agent alike. A pool declaring `["Docker"]` registers its agents with `docker`, and that is the form `kici-admin agent list`, the dashboard, and `ctx.kici.inventory[…].labels` report. Hostnames fold the same way, so `kici:host:Build-Box-01` is stored and shown as `kici:host:build-box-01`. Agent IDs never fold.
...
| `kici_orch_scaler_cpus_used`                      | Gauge   | `scaler`, `scalerType`, `machinePool` | Current CPU reservations summed by scaler / pool. `scaler="__global__"` is the orchestrator-wide total; pool rows reflect the ledger. `scalerType` is the backend type (`container`, `bare-metal`, `firecracker`, `event`), or `__global__` on the total row                                             |
...
**Solution:** Check your workflow's `runsOn` labels against the scaler config. Remember: all job labels must be present in the scaler's label set (subset matching). Matching normalizes labels (sorted, deduplicated, lowercased) before comparison, so a difference in case is never the cause.

```bash
# Check what label sets are configured
grep -A2 "labels:" /etc/kici/scalers.yaml

# The labels an agent actually registered with
