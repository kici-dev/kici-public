...
Under `db` custody the private signing key is stored AES-256-GCM-wrapped with
the orchestrator's master key (`KICI_SECRET_KEY`) — the same posture as scoped
secrets. So a master-key rotation is a signing-key event, even though the
signing key itself does not change:

- **`kici-admin rotate-key` re-encrypts `orchestrator_signing_keys`** along with
  every other master-key-wrapped store. Its summary reports the count as
  `Re-encrypted N provenance signing keys.` — a zero there on a deployment with
  signing enabled means the sweep did not reach the key the next boot has to
  unwrap. The sweep covers **every** row, not only the active one, so a retired
  or revoked `kid` keeps unwrapping and its historical bundles keep verifying.
- **A key still sealed under the previous master key self-heals at boot** while
  `KICI_SECRET_KEY_OLD` (or `KICI_SECRET_KEY_FILE_OLD`) is configured: the
...
check that proves the key survived the rotation.
