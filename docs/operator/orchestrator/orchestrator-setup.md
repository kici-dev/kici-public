...
#    skip it and write a stub env file you edit by hand. The mode defaults to
#    hybrid; pass --mode platform to take the relay-only shape instead.
...
exactly as `platform` does and additionally serves this orchestrator's own ingress
route, which verifies the webhook signature before it acts on a delivery. It needs
no configuration beyond what `platform` needs, so an orchestrator nothing posts to
directly behaves the same as a relay-only one. Set
[`KICI_WEBHOOK_PUBLIC_URL`](./github-ingress.md) when you are ready to have GitHub
deliver straight to it, and a Platform outage stops costing you pushes. To opt out
of the direct route, pass `--mode platform`.
...
| ---------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| `KICI_MODE`                  | Operating mode: `hybrid`, `platform`, `observed`, `independent`    | `hybrid` (written by `install`; an unset variable falls back to `platform`) |
...
- **Orchestrator-owned signing (all modes):** with `KICI_ORCHESTRATOR_PROVENANCE_ISSUER` and a provisioned signing key (see [signing keys](signing-keys.md)), the orchestrator mints and signs the token locally under its own issuer — no Platform involvement.
- **No minter:** an orchestrator with no signing key configured returns an error for token requests (outside the [local dev plane](local-dev-plane.md), which dev-signs under the `kici-local` issuer).
