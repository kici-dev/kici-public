...
### The chain layout, and why provisioning never deletes the table

The table holds two filter chains, and the split is what lets the host network
be repaired while VMs are running:

- **`forward`** is the hooked chain. It carries one rule per running VM,
  inserted at the head, and ends in a single `jump baseline`.
- **`baseline`** is a plain chain with no hook, holding the six host rules
  above. Nothing but the provisioner writes to it.

Provisioning is therefore **idempotent and non-destructive**: it flushes and
refills `baseline`, removes from `forward` only the rules no running VM owns,
...
Every VM's TAP is additionally enslaved with `isolated on`, so VMs can reach
the gateway but not each other. The orchestrator refuses to start when the host
kernel rejects the flag, rather than running VMs that can see one another.
