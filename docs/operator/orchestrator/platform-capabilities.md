...
- **Organizations, teams, and member roles.** The multi-tenant control plane —
  ...
  The dashboard's [orchestrator-token reconciliation report](../security/rbac-two-layers.md#reconciling-the-two-layers)
  is part of this: it reads your orchestrator's admin tokens and compares each
  one against the holder your organization knows. Self-hosted substitute:
  `kici-admin token list`, which shows each token's role, source scope, and
  recorded holder — there is no membership to compare it against.
  Member CI trust reaches your orchestrator on the Platform's next push, so a
  role change takes effect there a moment after it takes effect on the
  Platform — and not at all while the connection is down. Check the lag with
  `kici-admin trust-policy directory` or the
  `kici_orch_trust_directory_age_seconds` metric. See
