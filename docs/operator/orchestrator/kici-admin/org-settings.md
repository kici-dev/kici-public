...
- `ignore` — the orchestrator drops the event. It creates no run and posts no check status, and the contributor sees nothing on the pull request. This is what an org with no stored policy applies. `show` prints a warning under this value, and the delivery's event-log row records the reason for each drop.
