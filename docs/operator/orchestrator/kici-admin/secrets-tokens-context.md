...
```bash
kici-admin token create <label> --role <role> [--routing-key <key>] [--expires <value>] [--subject <id>]
...
- `--expires` optionally sets an expiry: a duration from now (`30d`, `12h`, `45m`) or an ISO-8601 datetime. Omit it for a non-expiring token; `token list` shows `never` in the `Expires` column and marks a past expiry with `[expired]`.
- `--subject` optionally records who the token is for.

#### Recording the holder with `--subject`

`--subject` takes the OIDC subject or the email address of the person the token
is issued to:

```bash
kici-admin token create alice-ops --role admin --subject alice@example.com
```
...
A token created without `--subject` is listed as `unlinked` and reported as
`unlinked` in the drift report. That is a finding, not an error: nobody can say
whose token it is. Re-issue it with a subject and revoke the old one.
...
| `--subject <id>`      |         | Record the intended holder of this token -- an OIDC subject or an email address. Advisory metadata: the orchestrator cannot verify it and never reads it when authorizing a request. It is what lets the dashboard RBAC drift report say whose token this is; a token created without it is reported as unlinked.          |
