...
**Quiescing before an upgrade:**

```bash
kici-admin orchestrator drain [--wait] [--timeout <seconds>] [--status]
kici-admin orchestrator resume
```

`drain` quiesces the coordinator before an upgrade or host reboot: new jobs are parked in the pending queue instead of being dispatched, the peer heartbeat advertises `draining: true` so sibling orchestrators stop rerouting work here, and the scaler stops spawning fresh agents. Jobs already executing on agents run to completion. It goes through the admin HTTP API and needs the `orchestrator.drain` permission (owner + admin; never auditor).

- `--wait` blocks until in-flight jobs finish (the reported `jobsRunning` reaches 0), bounded by `--timeout <seconds>` (default 300).
- `--status` reports the current drain state without changing it.
- `resume` undoes the drain and starts dispatching again. The drain flag is in-memory only, so a restarted orchestrator comes back accepting and re-dispatches the parked backlog on its own — `resume` is only needed when you drained a process you then decided to keep running.
...
| `--run-retention-days`                              |         | Clear only Run history retention (days)               |
| `--audit-retention-days`                            |         | Clear only Audit log retention (days)                 |
| `--provenance-retention-days`                       |         | Clear only Provenance retention (days)                |
| `--held-run-retention-days`                         |         | Clear only Terminal held-run retention (days)         |
...
| `--run-retention-days <value>`                              |         | Run history retention (days) (integer >= 0)                   |
| `--audit-retention-days <value>`                            |         | Audit log retention (days) (integer >= 0)                     |
| `--provenance-retention-days <value>`                       |         | Provenance retention (days) (integer >= 0)                    |
| `--held-run-retention-days <value>`                         |         | Terminal held-run retention (days) (integer >= 0)             |
...
| `--mode <mode>`         | `hybrid`            | Operating mode written to the env file (platform, hybrid, independent, observed). Naming it with --env-file overwrites the copied file's own KICI_MODE; omitting it leaves that value alone |
...
| `--skip-backup`                    |         | Skip the pre-upgrade database dump (loud; no recovery path)                                                                                   |
| `--backup-dir <path>`              |         | Where the pre-upgrade dump goes (default <instanceDir>/backups)                                                                               |
| `--no-drain`                       |         | Do not quiesce the coordinator first; in-flight jobs will fail                                                                                |
| `--drain-timeout <seconds>`        | `300`   | Seconds to wait for in-flight jobs to finish                                                                                                  |
