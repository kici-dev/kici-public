...
| Shared DB                      | `kici-admin config seed`, `set`, `delete`, `export`, `diff`, `history` and `rollback`; `kici-admin rotate-key`; and the cluster join flow, which copies `storage` and `secrets.key` into the joining instance's YAML file | orchestrator startup, the reload path |
...
| `KICI_OIDC_LEGACY_PR_SUB`                              | no       | false                     | union                                        |         |                                                                                                                                                                                                                                             |
