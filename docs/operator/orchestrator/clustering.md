...
Every orchestrator loads its own scaler config file and builds its own backends from it. Nothing distributes that file, so each one decides for itself what it can provision. The state behind those decisions is split: some of it is shared through the cluster database, and the rest stays in the process that holds it.
...
On boot, an orchestrator rehydrates only the rows stamped with its own instance id. This is what stops it counting a peer's reservations against its own caps and refusing spawns it has capacity for.
