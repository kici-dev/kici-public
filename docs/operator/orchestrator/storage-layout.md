...
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `source/v2/{orgId}/{sourceTarDigest}.tar.gz`                          | Platform-agnostic source tarball (`.kici/` minus `node_modules/`), addressed by its OWN content hash and scoped to the owning org; one entry shared across linux-x64 / linux-arm64 / etc.                                                                                           | `packages/orchestrator/src/cache/source-cache.ts`                         |
| `source/v2/{orgId}/{contentHash}.hash`                                | Pointer resolving a workflow content hash to the digest of the tarball built from it — the only mutable object on the source side, replaced wholesale so concurrent builders cannot leave a half-written pair.                                                                      | `packages/orchestrator/src/cache/source-cache.ts`                         |
| `deps/{platform}-{arch}/{depsHash}.tar.gz`                            | Platform-specific dependency tarball, addressed by its OWN content hash so the object is immutable and a pointer can never resolve to different bytes than it names.                                                                                                                | `packages/orchestrator/src/cache/dep-cache.ts`                            |
| `deps/{platform}-{arch}/{lockfileHash}[-{siblingsDigest}].hash`       | Pointer resolving a lock file — plus the in-repo `workspace:` sibling closure it depends on, when it has one — to the content hash of the tarball built from it. The only mutable object in the scheme, replaced wholesale so concurrent builders cannot leave a half-written pair. | `packages/orchestrator/src/cache/dep-cache.ts`                            |
...
| `KICI_CACHE_TTL_DAYS`              | no (`30`)               | Days of inactivity before an entry is evicted (touch-on-read). Overridable per cluster at runtime via `kici-admin cluster-settings set --cache-ttl-days`.                                                                                                                                                                                                                                                                                |
...
| `KICI_CHECK_RUN_TRACKING_TTL_DAYS` | no (`7`)                | `check_run_tracking` **row** retention (days, Postgres — not object storage). The cleanup sweep deletes rows untouched for longer than this; the window is inactivity age, not run completion, so a check-run status update arriving after its run was pruned can still resolve which check run to update. `0` disables pruning. Overridable per cluster at runtime via `kici-admin cluster-settings set --check-run-tracking-ttl-days`. |
| `KICI_CACHE_MAX_TARBALL_BYTES`     | no (`524288000`)        | Max dep-tarball size; build fails if exceeded. Overridable per cluster at runtime via `kici-admin cluster-settings set --cache-max-tarball-bytes`.                                                                                                                                                                                                                                                                                       |
...
| -------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ---------------------------------------------------------- |
| `source/v2/{orgId}/{sourceTarDigest}.tar.gz` | tens of KB to a few MB | none (raw `.kici/` minus node_modules)                                                  | 1× `PutObject` per cache MISS (per unique workflow content)         | 1× `GetObject` per execution job dispatch (cache HIT path) |
...
| `deps/{...}/{lockfileHash}.hash`             | 64 bytes (SHA-256 hex) | --                                                                                      | 1× `PutObject` per dep MISS (companion to dep tarball)              | 1× `GetObject` per dep download (integrity check)          |
...
The orchestrator reads only the key layouts listed above. Objects written under an earlier layout are never referenced again. The cache TTL is enforced lazily, on access, so the product never removes them either. A retired `cache/` entry goes only through over-quota eviction; a retired `source/` or `deps/` object stays until an operator removes it. `kici-admin cache purge-legacy` is that removal path, alongside a bucket lifecycle rule the operator manages (dry run by default, `--yes` to delete — see [kici-admin: inspection & recovery](./kici-admin/inspection-recovery.md#cache----object-storage-cache-maintenance-direct-storage-access)). The command recognises these shapes:

| Retired key                                                             | Replaced by                                                                               |
| ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `cache/<orgId>/<repoId>/<scope>/<key>.tar.gz` (+ `.hash`, `.size`)      | `cache/<orgId>/<repoId>/<scope>/<key>-<discriminator>.tar.gz` (+ sidecars)                |
| `source/{contentHash}.tar.gz`                                           | `source/v2/{orgId}/{sourceTarDigest}.tar.gz` + `source/v2/{orgId}/{contentHash}.hash`     |
| `deps/{platform}-{arch}/{lockfileHash}.tar.gz` with a same-stem `.hash` | `deps/{platform}-{arch}/{depsHash}.tar.gz` + `deps/{platform}-{arch}/{lockfileHash}.hash` |

A retired dependency tarball shares its key shape with a current one; the command tells them apart by the same-stem `.hash` sidecar, which only the retired layout wrote beside a tarball. The sidecar itself is not removed: its key is a valid current pointer key, the reader already treats it as a dangling pointer, and the next build for that lock file overwrites it. A dependency tarball stored before integrity tracking has no `.hash` sidecar at all, so the command does not detect it; such an object stays until an operator removes it by hand.

Source: `packages/orchestrator/src/cache/legacy-prefixes.ts`.
...
The warm TTL column below applies **only when the cold store is configured**. The archivers do archive-then-delete, and they run only with `KICI_COLD_STORE_ENABLED=true` and a bucket set. Without an object store, these tables are pruned by the database retention windows in the next section instead.

| Table              | Warm TTL (cold store on) | Without the cold store                 | What lives there                                                          |
| ------------------ | ------------------------ | -------------------------------------- | ------------------------------------------------------------------------- |
| `execution_runs`   | 30 days                  | `KICI_RUN_RETENTION_DAYS` (90 days)    | Completed run metadata (status, timings, parent_run_id) for terminal runs |
| `execution_jobs`   | 30 days                  | with their run                         | Per-job metadata (status, agent, queue/run timings)                       |
| `execution_steps`  | 30 days                  | with their run                         | Per-step execution details (status, exit code, duration, log pointer)     |
| `secret_audit_log` | 30 days                  | `KICI_AUDIT_RETENTION_DAYS` (365 days) | Secret access / rotation / write events                                   |
...
The orchestrator prunes aging rows from the database itself, so an install that runs no object store still bounds its own disk. Four windows, in days:

| Setting                          | Default | Covers                                                                               |
| -------------------------------- | ------- | ------------------------------------------------------------------------------------ |
| `KICI_RUN_RETENTION_DAYS`        | 90      | Terminal runs (`success`, `failed`, `cancelled`) and the jobs and steps beneath them |
| `KICI_AUDIT_RETENTION_DAYS`      | 365     | `access_log`, `secret_audit_log`, `event_log`                                        |
| `KICI_PROVENANCE_RETENTION_DAYS` | 365     | `attestations`, `pending_attestations`                                               |
| `KICI_HELD_RUN_RETENTION_DAYS`   | 90      | Terminal `held_runs` — a pending hold is never pruned                                |

Set any window to `0` to disable it. Each is also a cluster setting, so you can change it on a running cluster:

```bash
kici-admin cluster-settings set --run-retention-days 180
kici-admin cluster-settings show
```

Two rules govern how this interacts with the cold store:

- **One owner per table.** With the cold store on, the six tables above are the archivers' alone and the database tier leaves them untouched. It still covers `attestations`, `pending_attestations` and terminal `held_runs`, which no archiver registers.
- **Nothing is deleted for the first week.** The first sweep with a non-zero window records the date and deletes nothing, logging how many rows it would have removed. Deletion starts seven days later. Run `kici-admin diagnose` to see the effective windows and the date the first deletion is due.

Deletes are batched at 5,000 rows per statement, so no single statement holds locks on a large table.

### What database retention does not remove

The sweep deletes database rows. It does not delete the objects some of those rows point at:

- `event_log.payload_key` — the gzipped webhook body.
- `attestations.storage_key` — the stored attestation.

If you run an object store for those payloads but do not run the cold store, pruning a row leaves its object in the bucket with nothing left to reference it. The database shrinks and the bucket does not. Set a bucket lifecycle rule for those prefixes, or enable the cold store, which archives the rows instead of deleting them and keeps every payload reachable.

A run that is still queued, running, held or cancelling is never pruned, whatever its age.
