...
| `deps/{platform}-{arch}/{depsHash}.tar.gz`                            | Platform-specific dependency tarball, addressed by its OWN content hash so the object is immutable and a pointer can never resolve to different bytes than it names.                                                                                          | `packages/orchestrator/src/storage/s3.ts`                                 |
| `deps/{platform}-{arch}/{lockfileHash}.hash`                          | Pointer resolving a lock file to the content hash of the tarball built from it — the only mutable object in the scheme, replaced wholesale so concurrent builders cannot leave a half-written pair.                                                           | `packages/orchestrator/src/storage/s3.ts`                                 |
