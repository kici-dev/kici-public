...
6. The agent stops itself once it goes idle, and the orchestrator emits a `kici.scaler.scale-down` event when that agent disconnects. A provision whose agent never registered is swept by the [orchestrator-side backstop](#orchestrator-side-backstop) instead.
