...
| Resources                                   | 15 typed resources (`runs`, `secrets`, `environments`, `workflows`, `members`, `api_keys`, `webhooks`, `org-settings`, `audit`, `event_log`, `ci_trust`, `support`, …) | Coarse — admin tokens get every write |
...
- [Role-based access control (architecture)](../../architecture/security/rbac.md) — the control-plane permission model: 15 resources, 5 access levels, custom roles, and enforcement.
