...
Under `ignore` it prints a warning too, because that value is the one that leaves nothing else to find: no run, no check, and nothing on the pull request. The warning names the consequence, and says where each individual drop is recorded — the `kici_orch_fork_events_ignored_total` counter, and the delivery's own event-log row, which `kici-admin event-log show <delivery-id>` prints in full. The `event-log list` table has no error column, so use it to find the delivery and `show` to read the reason. When no policy is stored, the warning also says so and names the command or dashboard page that sets one.
...
- **The approval is recorded as the admin token unless you name a member.** By default the token answers as itself, which satisfies a hold that names nobody but no `approvers:` clause, since those name a user or a team and the token is neither. Pass `--as <user-id>` to answer as a member of the stored approval directory — the one `kici-admin trust-policy directory-set` writes — and the clause matches. The id must already be registered; an unregistered one is refused, because a subject the operator merely typed would satisfy any clause by naming it. Either way the decision is written to the access log as `held_run.approve` or `held_run.reject`, and an `--as` decision carries **both** facts: the admin token stays the actor, and the member it acted as is recorded alongside it.
