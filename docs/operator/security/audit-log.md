...
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **always**     | Every allowed event records a row.                                                                                                                                                 | All mutations (`run.cancel`, `run.rerun`, `secret.set`, `secret.delete`, `context.create`, `held_run.approve`, `attestation.retry`, etc.), all sensitive reads (`secret.reveal`, `secret.list.read`, `event_log.detail.read`, `event_log.payload.read`, `run.payload.read`, `access_log.list.read`, `admin_tokens.list.read`, `context_var.list.read`, `source_override.list.read`), and `archive_chunk`. |
| **sample**     | Hash-based stable sampling on `(actor key, requestId)` so a single user's poll trace is coherent — either every poll lands in the sample or none do. Denied/error always recorded. | `run.detail.read` (5%), `event_log.list.read` (5%), `run.orch_logs.read` (10%), `step.logs.read` (10%), `context.list.read` (10%), `registration.list.read` (10%), `global_workflows.get.read` (10%), `context.get.read` (20%), `context.history.read` (20%), `held_run.list.read` (20%), `context_binding.list.read` (20%), `backend.list.read` (20%), `backend.get.read` (20%).                         |
...
| **Tenant-plane mutations (non-secret)**       | `context.*` (non-read), `context_var.*` (non-read), `context_binding.set`, `source_override.*`, `backend.sync*`, `backend.test`, `registration.disable`, `registration.delete`, `run.cancel`, `attestation.retry` (deferred-attestation outbox drain / re-arm — owner/admin only), etc. | 180 days         | 730 days (2y)     | Long enough to investigate bugs, short enough to not bloat S3 indefinitely.           |
| **Tenant-plane reads (high volume, sampled)** | `run.detail.read`, `run.orch_logs.read`, `step.logs.read`, `event_log.list.read`, `context.list.read`, `registration.list.read`, `backend.*.read`, `held_run.list.read`                                                                                                                 | 30 days          | 180 days (6m)     | Sampled at write time per the policy table — surviving rows kept for short forensics. |
| **Tenant-plane reads (sensitive)**            | `secret.list.read`, `source_override.list.read`, `context_var.list.read`, `access_log.list.read`, `admin_tokens.list.read`, `event_log.detail.read`, `event_log.payload.read`, `run.payload.read`                                                                                       | 180 days         | 730 days (2y)     | Lower volume, higher forensic value.                                                  |
