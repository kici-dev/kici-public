...
Once the target resolves, the command reads the service platform from that install's manifest. A Compose install is managed with Compose even on a systemd host, and a systemd install is managed with systemd even where a container runtime is present. `--platform <type>` overrides the manifest and forces one driver. Only `install` detects the platform from the host, because it is the command that writes the manifest.

Instance discovery is wider than the operation: `--name` and the candidate list scan every driver the host can run, so a Compose instance is listed and resolvable on a systemd host.

A driver that cannot read its own registry is skipped instead of reporting nothing, and the command says so on stderr, naming the platform and the reason. If the container daemon is down or hung, Compose instances do not appear in the candidate list. Their index entries are left untouched: an unreadable registry is not evidence that the instances are gone. This applies to every platform — an unreadable systemd unit directory, launchd job or Windows service registry is skipped on the same terms. Fix what the warning names and re-run the command to see those instances again.
...
1. **Operating mode** -- Hybrid (the default: hosted Platform relay plus this orchestrator's own direct webhook ingress), Platform (relay only), Observed, or Independent
...
Both paths write `KICI_MODE=hybrid`. Hybrid accepts every Platform-relayed webhook
exactly as `platform` does and additionally serves the orchestrator's own ingress
route, which verifies the webhook signature before it acts on a delivery. It requires
no configuration beyond what `platform` requires, so an orchestrator nothing posts to
directly behaves the same as a relay-only one. Pass `--mode platform` to opt out.
...
| ----------------------- | --------------------------------------------------------------------------------------- | ------------------------- |
| `--platform <type>`     | Force platform: `systemd`, `launchd`, `windows`, `compose`                              | Detected from the host    |
| `--mode <mode>`         | Operating mode written to the env file: `hybrid`, `platform`, `observed`, `independent` | `hybrid`                  |
...
Every lifecycle command resolves its target through the same priority chain: `--instance-dir` > `--name` > a manifest in the current working directory. Without any of those, the command refuses and prints the candidate list of installed instances on the host. The examples below use `--instance-dir`; they work identically if you `cd` into the deploy folder first and drop the flag. The service platform comes from the resolved install's manifest — see [Targeting operating commands](#targeting-operating-commands).
...
Example output, for an install whose env file names a scaler config. An install
that names none prints the `Env file:` line alone:
...
### What an upgrade does before it stops the service

An orchestrator upgrade takes two steps before anything stops:

1. **A database dump.** It writes `pre-upgrade-<from>-<to>-<timestamp>.dump` and its manifest into `<instance-dir>/backups`. Use `--backup-dir <path>` to put it elsewhere. If the dump cannot be taken — `pg_dump` missing, a version mismatch with the server, or an unresolvable database URL — the upgrade refuses and names the cause. `--skip-backup` proceeds without one; it prints a banner, because an upgrade with no dump has no recovery path.
2. **A drain.** It puts the coordinator into drain and waits for in-flight jobs to finish, up to `--drain-timeout <seconds>` (300 by default). On timeout it refuses and reports how many jobs are still running. `--no-drain` skips the wait and accepts that the restart fails them. After a successful upgrade no resume step is needed — the drain flag lives in memory, so the restarted process comes back accepting.

   A timeout refusal leaves the coordinator **draining**, so the jobs you are waiting for finish with nothing queueing behind them. It accepts no new work until the upgrade restarts it, or until you run `kici-admin orchestrator resume`.

   The drain needs the admin API. If the CLI has no `--url` / `--token` configured it cannot reach it, and the upgrade warns and continues rather than refusing.

Neither step applies to an agent upgrade: an agent has no database and no drain.
...
kici-admin db migrate --to <recorded-head>
kici-admin orchestrator upgrade --rollback
```

Or let the upgrade do it in one step:

```bash
kici-admin orchestrator upgrade --rollback --migrate-down
```

`--migrate-down` reverts the schema through the still-running newer service — the only binary that carries those migrations' `down()` functions — and only then moves the version pointer. It is opt-in rather than automatic: a `down()` can be lossy, and running one unprompted while an operator is already handling an incident is not a default worth having.

If the switch does loop and you have no dump, start the older version with `KICI_AUTO_MIGRATE=false`. That boots only if every newer migration was purely additive.
...
| `--platform <type>`     | Force platform (`systemd`, `launchd`, `windows`, `compose`). Defaults to the platform in the install manifest                                                                                              |
...
| `--skip-backup`         | Skip the pre-upgrade database dump. Prints a banner; leaves the upgrade with no recovery path                                                                                                              |
| `--backup-dir <path>`   | Where the pre-upgrade dump goes (default `<instance-dir>/backups`)                                                                                                                                         |
| `--no-drain`            | Do not quiesce the coordinator first. In-flight jobs fail when the service restarts                                                                                                                        |
| `--drain-timeout <s>`   | Seconds to wait for in-flight jobs to finish before refusing (default 300)                                                                                                                                 |
| `--migrate-down`        | On a rollback whose database is ahead, revert the schema to the target version's recorded head first, through the still-running newer service                                                              |
