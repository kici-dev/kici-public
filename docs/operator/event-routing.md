...
The orchestrator reads these defaults from `KICI_`-prefixed environment variables at startup. Two of them also carry a live fleet-wide override in `cluster_settings`, set with `kici-admin cluster-settings set`; when that override is unset, the startup value applies. See [config management](orchestrator/config-management.md#changing-a-running-orchestrator).

| Setting                         | Env var                                                | Live override                                       | Default   | Description                                                                                                            |
| ------------------------------- | ------------------------------------------------------ | --------------------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------- |
| `maxChainDepth`                 | `KICI_EVENT_ROUTER_MAX_CHAIN_DEPTH`                    | --                                                  | `10`      | Maximum event chain depth before circuit breaker trips                                                                 |
| `rateLimitPerWorkflowPerMinute` | `KICI_EVENT_ROUTER_RATE_LIMIT_PER_WORKFLOW_PER_MINUTE` | `--event-router-rate-limit-per-workflow-per-minute` | `100`     | Maximum events per event name per minute (note: keyed by event name, not workflow name, despite the config field name) |
| `eventTtlSeconds`               | `KICI_EVENT_ROUTER_EVENT_TTL_SECONDS`                  | `--event-router-event-ttl-seconds`                  | `604800`  | Event retention in seconds (7 days)                                                                                    |
| `cleanupIntervalMs`             | `KICI_EVENT_ROUTER_CLEANUP_INTERVAL_MS`                | --                                                  | `3600000` | Cleanup interval for expired events (1 hour)                                                                           |
| `maxDispatchAttempts`           | `KICI_EVENT_ROUTER_MAX_DISPATCH_ATTEMPTS`              | `--event-router-max-dispatch-attempts`              | `5`       | Delivery attempts an event gets before it lands in the dead-letter queue (`kici-admin event-dlq`)                      |
| `leaseDurationMs`               | `KICI_EVENT_ROUTER_LEASE_DURATION_MS`                  | --                                                  | `60000`   | How long one dispatch attempt holds its lease on the event row; an expired lease makes the event eligible for retry    |
| `retryBaseBackoffMs`            | `KICI_EVENT_ROUTER_RETRY_BASE_BACKOFF_MS`              | --                                                  | `5000`    | Base delay of the exponential backoff between attempts (full jitter)                                                   |
| `retryMaxBackoffMs`             | `KICI_EVENT_ROUTER_RETRY_MAX_BACKOFF_MS`               | --                                                  | `300000`  | Ceiling on the backoff between attempts (5 minutes)                                                                    |
| `retryScanIntervalMs`           | `KICI_EVENT_ROUTER_RETRY_SCAN_INTERVAL_MS`             | --                                                  | `10000`   | How often the router scans for events whose retry delay has elapsed                                                    |
...
`orgId` is one value in three places: the segment in this URL, the organization segment of the routing key, and the organization that owns the source on the Platform. Pass your Platform organization id to `--org`. A source created under a different value is refused when the orchestrator is Platform-connected, and the error names the correct id.
