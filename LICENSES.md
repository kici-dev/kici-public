...
| `kici` (unscoped wrapper) | yes           | Apache-2.0    | Developer CLI entry point. Wraps `@kici-dev/compiler`; installing it also pulls the AGPL-3.0-only `engine`, `agent` and `orchestrator` for `kici run --local`. |
...
**Installing the `kici` CLI pulls both tiers.** `npm i kici` installs eight KiCI
packages: the five Apache-2.0 ones above, plus `@kici-dev/engine`,
`@kici-dev/agent` and `@kici-dev/orchestrator`, which are AGPL-3.0-only. The
compiler depends on them because `kici run <event> --local` executes a workflow
on your machine using the same engine and agent the server plane uses. This
imposes nothing on you — the AGPL's source-disclosure obligation (§13) is
triggered by offering the software to third parties over a network, not by
installing or running it. Your own workflow files import only `@kici-dev/sdk`,
which is Apache-2.0.
